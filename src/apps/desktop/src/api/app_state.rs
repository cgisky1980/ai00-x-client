...
impl AppState {
    pub async fn new_async(
        ...
        let config_service = config::get_global_config_service().map_err(|e| {
            Ai00XError::config(format!("Failed to get global config service: {}", e))
        })?;
        ...
        ai_rules::initialize_global_ai_rules_service()
            .await
            .map_err(|e| {
                Ai00XError::service(format!("Failed to initialize AI rules service: {}", e))
            })?;
        ...
        let worker_host_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("resources")
            .join("worker_host.js");
        let js_worker_pool = JsWorkerPool::new(path_manager.clone(), worker_host_path)
            .ok()
            .map(Arc::new);
        ...
        // SSH data is split into:
        // - ssh_connections_dir() = <exe_dir>/data/profile/ssh/  (sync-able, no passwords)
        // - ssh_secrets_dir()      = <exe_dir>/data/ssh_secrets/   (AES password vault, not synced)
        let ssh_connections_dir = path_manager.ssh_connections_dir();
        let ssh_secrets_dir = path_manager.ssh_secrets_dir();
        ...
        let manager = SSHConnectionManager::new(ssh_connections_dir, ssh_secrets_dir);
        ...
        *remote_terminal_manager_clone.write().await = Some(tm_for_state);
        ...
        // Stage 5.12: 初始化全局 P2pDownloader（v9 P2P 重构）
    ...
    pub async fn set_remote_workspace(
        ...
        *self.remote_workspace.write().await = Some(workspace.clone());
        ...
        if let Ok(manager) = self.get_ssh_manager_async().await {
            let core_workspace = ai00_x_core::service::remote_ssh::RemoteWorkspace {
    ...
    pub async fn unregister_remote_workspace_entry(&self, connection_id: &str, remote_path: &str) {
        let rp = ai00_x_core::service::remote_ssh::normalize_remote_workspace_path(remote_path);
        if let Ok(manager) = self.get_ssh_manager_async().await {
        ...
        if let Some(state_manager) =
            ai00_x_core::service::remote_ssh::get_remote_workspace_manager()
        {
            state_manager
        ...
        let clear_slot = slot
            ...
            .map(|w| {
                w.connection_id == connection_id
                    && ai00_x_core::service::remote_ssh::normalize_remote_workspace_path(
                        &w.remote_path,
                    ) == rp
            })
            .unwrap_or(false);
        if clear_slot {
            *slot = None;
            if let Some(m) = ai00_x_core::service::remote_ssh::get_remote_workspace_manager() {
