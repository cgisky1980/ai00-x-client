...
use ai00_x_core::infrastructure::storage::StorageOptions;
use ai00_x_git::{
    GitAddParams, GitCommitParams, GitDiffParams, GitLogParams, GitPullParams, GitPushParams,
    GitService,
};
use ai00_x_git::{GitBranch, GitCommit, GitOperationResult, GitRepository, GitStatus};
use log::{error, info};
...
) -> Result<ai00_x_git::GitGraph, String> {
    info!(
...
pub async fn git_list_worktrees(
    _state: State<'_, AppState>,
    request: GitRepositoryRequest,
) -> Result<Vec<ai00_x_git::GitWorktreeInfo>, String> {
...
) -> Result<ai00_x_git::GitWorktreeInfo, String> {
...
pub async fn git_merge_branch(
    ...
    info!(
        "Merging branch '{}' in '{}' (target: {:?})",
        request.branch, request.repository_path, request.target_branch
    );

    GitService::merge_branch(
        &request.repository_path,
        &request.branch,
    ...
    .map_err(|e| {
        error!(
            "Failed to merge branch: path={}, branch={}, target={:?}, error={}",
            request.repository_path, request.branch, request.target_branch, e
        );
...
#[tauri::command]
pub async fn git_has_conflicts(
    _state: State<'_, AppState>,
    request: GitRepositoryRequest,
) -> Result<bool, String> {
    info!("Checking conflicts in '{}'", request.repository_path);

    GitService::has_conflicts(&request.repository_path)
        .await
        .map_err(|e| {
            error!(
                "Failed to check conflicts: path={}, error={}",
                request.repository_path, e
            );
...
pub async fn git_abort_merge(
    ...
    info!("Aborting merge in '{}'", request.repository_path);

    GitService::abort_merge(&request.repository_path)
        .await
        .map_err(|e| {
            error!(
                "Failed to abort merge: path={}, error={}",
                request.repository_path, e
            );
