            ...
            match tool.call(&request.input, &context).await {
                Ok(results) => {
                    let combined_result = if results.len() == 1 {
                        match &results[0] {
                            ai00_x_core::agent::tools::framework::ToolResult::Result {
                                data,
                                ..
                            } => Some(data.clone()),
                            ai00_x_core::agent::tools::framework::ToolResult::Progress {
                                content,
                                ..
                            } => Some(content.clone()),
                            ai00_x_core::agent::tools::framework::ToolResult::StreamChunk {
                                data,
                                ..
                            } => Some(data.clone()),
                        }
                        ...
                        ai00_x_core::agent::tools::framework::ToolResult::Result { data, .. } => {
                            data.clone()
                        }
                        ai00_x_core::agent::tools::framework::ToolResult::Progress { content, .. } => content.clone(),
                        ai00_x_core::agent::tools::framework::ToolResult::StreamChunk { data, .. } => data.clone(),
                                        }).collect::<Vec<_>>()
                                    }))
                    };
...
pub async fn submit_user_answers(
    ...
    use ai00_x_core::agent::tools::user_input_manager::get_user_input_manager;
