...
use ai00_x_core::service::remote_connect::{
...
pub fn cleanup_on_exit() {
    ai00_x_core::service::remote_connect::ngrok::cleanup_all_ngrok();
...
async fn restore_saved_bots() {
    use ai00_x_core::service::remote_connect::bot;
...
fn detect_mobile_web_dir() -> Option<String> {
    if let Ok(dir) = std::env::var("AI00_X_MOBILE_WEB_DIR") {
        let p = std::path::Path::new(&dir);
        if p.join("index.html").exists() {
            log::info!("Using AI00_X_MOBILE_WEB_DIR: {dir}");
            return Some(dir);
        }
        log::warn!("AI00_X_MOBILE_WEB_DIR set but index.html not found: {dir}");
    }
...
fn detect_from_exe() -> Option<String> {
    ...
    if cfg!(target_os = "linux") {
        candidates.push(exe_dir.join("../lib/ai00-x/mobile-web/dist"));
        candidates.push(exe_dir.join("../lib/ai00-x/mobile-web"));
        candidates.push(exe_dir.join("../share/ai00-x/mobile-web/dist"));
        candidates.push(exe_dir.join("../share/ai00-x/mobile-web"));
        candidates.push(exe_dir.join("../share/com.ai00-x.desktop/mobile-web/dist"));
        candidates.push(exe_dir.join("../share/com.ai00-x.desktop/mobile-web"));
    }
...
fn detect_default_gateway_ip() -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        let output = ai00_x_core::util::process_manager::create_command("route")
            .args(["-n", "get", "default"])
            .output()
            .ok()?;
    ...
    #[cfg(target_os = "linux")]
    {
        let output = ai00_x_core::util::process_manager::create_command("ip")
...
pub async fn remote_connect_get_methods() -> Result<Vec<ConnectionMethodInfo>, String> {
    ...
    let infos = methods
        ...
        .map(|m| match m {
            ...
            ConnectionMethod::Ai00XServer => ConnectionMethodInfo {
                id: "ai00_x_server".into(),
                name: "Ai00-X Server".into(),
                available: true,
                description: "Official Ai00-X relay".into(),
            },
...
fn parse_connection_method(
    ...
    match method {
        ...
        "ai00_x_server" => Ok(ConnectionMethod::Ai00XServer),
