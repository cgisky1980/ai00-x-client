...
use ai00_x_core::service::snapshot::{
...
pub async fn rollback_to_turn(
    ...
    {
        use ai00_x_core::agent::coordination::get_global_coordinator;
    ...
    if request.delete_turns {
        let workspace_path = PathBuf::from(&request.workspace_path);
        {
            ...
            if let Some(coordinator) = get_global_coordinator() {
                ...
                {
                    warn!(
                        "Rollback agent context failed: session_id={}, turn_index={}, error={}",
                        request.session_id, request.turn_index, e
                    );
                }
            } else {
                warn!("Global coordinator not initialized, skipping agent context rollback");
            }
        }
