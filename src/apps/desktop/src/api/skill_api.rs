...
use ai00_x_core::agent::tools::implementations::skills::{
    SkillData, SkillInfo, SkillLocation, SkillRegistry,
};
use ai00_x_core::agent::workspace::RemoteWorkspaceFs;
use ai00_x_core::infrastructure::get_path_manager_arc;
use ai00_x_core::service::remote_ssh::workspace_state::is_remote_path;
use ai00_x_core::service::remote_ssh::{get_remote_workspace_manager, RemoteWorkspaceEntry};
...
pub async fn add_skill(
    ...
    let target_dir = if level == "project" {
        if let Some(workspace_root) = workspace_root_from_input(workspace_path.as_deref()) {
            ...
            workspace_root.join(".ai00-x").join("skills")
        } else {
