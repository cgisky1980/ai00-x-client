...
use ai00_x_core::infrastructure::events::{emit_global_event, BackendEvent};
use ai00_x_core::miniapp::{
    ...
    fn from(d: MiniAppSourceDto) -> Self {
        MiniAppSource {
            ...
            esm_dependencies: d
                .esm_dependencies
                .into_iter()
                .map(|x| ai00_x_core::miniapp::EsmDep {
                    name: x.name,
                    version: x.version,
                    url: x.url,
                })
            ...
            npm_dependencies: d
                .npm_dependencies
                .into_iter()
                .map(|x| ai00_x_core::miniapp::NpmDep {
                    name: x.name,
                    version: x.version,
                })
                .collect(),
...
pub async fn miniapp_runtime_status(state: State<'_, AppState>) -> Result<RuntimeStatus, String> {
    ...
    Ok(RuntimeStatus {
        ...
        kind: Some(match info.kind {
            ai00_x_core::miniapp::RuntimeKind::Bun => "bun".to_string(),
            ai00_x_core::miniapp::RuntimeKind::Node => "node".to_string(),
        }),
...
fn validate_model(
    model: Option<&str>,
    ai_perms: &ai00_x_core::miniapp::AiPermissions,
