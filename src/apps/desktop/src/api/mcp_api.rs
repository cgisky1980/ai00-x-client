...
use ai00_x_core::service::mcp::auth::{
    has_stored_oauth_credentials, MCPRemoteOAuthSessionSnapshot,
};
use ai00_x_core::service::mcp::config::MCPConfigService;
use ai00_x_core::service::mcp::protocol::{
    MCPPrompt, MCPResource, PromptsGetResult, ResourcesReadResult,
};
use ai00_x_core::service::mcp::MCPServerType;
use ai00_x_core::service::runtime::{RuntimeManager, RuntimeSource};
...
async fn load_mcp_resources(
    mcp_service: &ai00_x_core::service::mcp::MCPService,
    server_id: &str,
    refresh: bool,
...
pub async fn get_mcp_servers(state: State<'_, AppState>) -> Result<Vec<MCPServerInfo>, String> {
    ...
    for config in configs {
        ...
        let (command, command_available, command_source, command_resolved_path) =
            if transport == ai00_x_core::service::mcp::MCPServerTransport::Stdio {
...
pub async fn get_mcp_tool_ui_uri(
    ...
    let registry = ai00_x_core::agent::tools::registry::get_global_tool_registry();
...
// --- MCP Skill Info API (progressive tool disclosure) ---

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct MCPSkillInfoResponse {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SetMCPSkillDescriptionRequest {
    pub server_id: String,
    pub description: String,
}

#[tauri::command]
pub async fn get_mcp_skill_info(
    state: State<'_, AppState>,
    server_id: String,
) -> Result<MCPSkillInfoResponse, String> {
    let cached_servers = ai00_x_core::service::mcp::adapter::tool::get_cached_mcp_servers().await;
    let cached = cached_servers
        .iter()
        .find(|s| s.server_id == server_id)
        .ok_or_else(|| format!("MCP server '{}' not cached", server_id))?;

    let mcp_service = state
        .mcp_service
        .as_ref()
        .ok_or_else(|| "MCP service not initialized".to_string())?;
    let server_status = mcp_service
        .server_manager()
        .get_server_status(&server_id)
        .await
    ...
    let skill_name = format!("mcp--{}", server_id);
    let skill_content = format!(
        "MCP server '{}' — {} tools available.\nActivate by calling Skill with command: \"{}\"",
        cached.server_name, cached.tool_count, skill_name
    );

    Ok(MCPSkillInfoResponse {
        skill_key: format!("project::ai00-x::{}", skill_name),
        skill_name,
        skill_content,
        server_name: cached.server_name.clone(),
        tool_count: cached.tool_count,
        tool_names: cached.tool_names.clone(),
        server_status,
    })
}

#[tauri::command]
pub async fn get_mcp_tools_preview(server_id: String) -> Result<Vec<MCPToolPreview>, String> {
    let cached_servers = ai00_x_core::service::mcp::adapter::tool::get_cached_mcp_servers().await;
    let cached = cached_servers
        .iter()
        .find(|s| s.server_id == server_id)
    ...
    let previews = cached
        ...
        .map(|full_name| {
            let short_name = full_name
                .strip_prefix(&format!("mcp__{}__", server_id))
                .unwrap_or(full_name)
                .to_string();
            MCPToolPreview {
                ...
                input_schema_summary: String::new(),
            }
        })
        .collect();
...
#[tauri::command]
pub async fn set_mcp_skill_description(
    request: SetMCPSkillDescriptionRequest,
) -> Result<(), String> {
    let workspace_root = ai00_x_core::service::workspace::get_global_workspace_service()
        .and_then(|svc| svc.try_get_current_workspace_path())
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_else(|| ".".to_string());

    let skill_dir = std::path::PathBuf::from(&workspace_root)
        .join(".ai00-x")
        .join("skills")
        .join(format!("mcp--{}", request.server_id));

    tokio::fs::create_dir_all(&skill_dir)
        .await
        .map_err(|e| e.to_string())?;

    let existing = if skill_dir.join("SKILL.md").exists() {
        tokio::fs::read_to_string(skill_dir.join("SKILL.md"))
            .await
            .unwrap_or_default()
    } else {
        String::new()
    ...
    let new_content = if existing.is_empty() {
        ...
        existing.replace(
            &format!("description: MCP server '{}' — ", request.server_id),
            &format!("description: {}", request.description),
        )
    };

    tokio::fs::write(skill_dir.join("SKILL.md"), &new_content)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn regenerate_mcp_skill(
    state: State<'_, AppState>,
    server_id: String,
) -> Result<(), String> {
    let mcp_service = state
    ...
    if status != ai00_x_core::service::mcp::MCPServerStatus::Connected
        && status != ai00_x_core::service::mcp::MCPServerStatus::Healthy
    {
        return Err(format!(
