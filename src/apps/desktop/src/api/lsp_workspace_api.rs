...
use ai00_x_core::service::lsp::types::CompletionItem;
use ai00_x_core::service::lsp::{
...
pub async fn lsp_open_workspace(
    ...
    let emitter: Arc<dyn ai00_x_core::infrastructure::events::EventEmitter> =
...
) -> Result<Vec<ai00_x_core::service::lsp::types::InlayHint>, String> {
    let workspace_path = PathBuf::from(&request.workspace_path);
    let manager = get_workspace_manager(workspace_path)
        .await
        .map_err(|e| format!("Workspace not found: {}", e))?;
...
pub async fn lsp_list_workspaces() -> Result<Vec<String>, String> {
    use ai00_x_core::service::lsp::get_all_workspace_paths;
...
pub async fn lsp_detect_project(
    ...
    use ai00_x_core::service::lsp::project_detector::ProjectDetector;
