...
use ai00_x_core::service::config::RouterConfig;
...
impl SmartRouter {
    ...
    pub fn resolve_model_ref(decision: &RoutingDecision, config: &RouterConfig) -> String {
        match decision.source {
            DecisionSource::Model => tier_model_ref(&config.tier_models, &decision.route),
            DecisionSource::TrivialAck => tier_model_ref(&config.tier_models, &decision.route),
    ...
    async fn classify(
        ...
        let probs = tokio::time::timeout(
            ...
            // capture=false：设置页预览为无状态测试，不污染进化样本库。
            engine.classify(classify_input, prev_tier, false),
        )
        .await
...
/// Maps a route tier to its configured model reference (core's
/// `RouterTierModels` no longer ships a `model_for` helper).
fn tier_model_ref(models: &RouterTierModels, route: &RouteClass) -> String {
    match route {
        RouteClass::R0 => models.r0.clone(),
        RouteClass::R1 => models.r1.clone(),
        RouteClass::R2 => models.r2.clone(),
        RouteClass::R3 => models.r3.clone(),
    }
}
