...
/// If unset or non-zero: write the exact JPEG passed to OCR into `computer_use_debug` under the app data dir (see implementation). Set `AI00_X_COMPUTER_USE_OCR_DEBUG=0` to disable.
fn ocr_debug_save_enabled() -> bool {
    !matches!(
        std::env::var("AI00_X_COMPUTER_USE_OCR_DEBUG"),
        Ok(v) if v == "0" || v.eq_ignore_ascii_case("false")
    )
}

/// Same directory as agent `screenshot` debug (`workspace/.ai00-x/computer_use_debug`), when PathManager is available.
fn computer_use_ocr_debug_dir() -> PathBuf {
    if let Ok(pm) = try_get_path_manager_arc() {
        return pm.ai00x_home_dir().join("computer_use_debug");
    }
    dirs::home_dir()
        .map(|h| h.join(".ai00-x").join("computer_use_debug"))
...
fn normalize_query(text_query: &str) -> Ai00XResult<String> {
...
pub fn crop_shot_to_ocr_region(
    ...
    let dyn_img = image::load_from_memory(&shot.bytes)
        .map_err(|e| Ai00XError::tool(format!("OCR crop: decode JPEG: {}", e)))?;
    ...
    .map_err(|e| Ai00XError::tool(format!("OCR crop: encode JPEG: {}", e)))?;
...
mod macos {
    ...
    use ai00_x_core::agent::tools::computer_use_host::ComputerScreenshot;
    use ai00_x_core::util::errors::{Ai00XError, Ai00XResult};
    use objc2::msg_send;
    ...
    fn recognize_text_observations(
        jpeg_bytes: &[u8],
    ) -> Ai00XResult<Vec<Retained<VNRecognizedTextObservation>>> {
    ...
    fn ns_error_to_ai00x(err: Retained<NSError>) -> Ai00XError {
        let desc = err.localizedDescription().to_string();
        Ai00XError::tool(format!("macOS Vision OCR failed: {}", desc))
    }
...
mod windows_backend {
    ...
    fn w<T>(r: windows::core::Result<T>) -> Ai00XResult<T> {
        r.map_err(|e| Ai00XError::tool(format!("Windows OCR: {}", e)))
    }
    ...
    pub fn find_text_matches(
        ...
        let result = (|| -> Ai00XResult<Vec<OcrTextMatch>> {
            ...
            let _store: u32 = block_on_async(writer.StoreAsync())?;
            let _flush: bool = block_on_async(writer.FlushAsync())?;
            w(writer.DetachStream())?;

            // 2. Decode JPEG to SoftwareBitmap
            let decoder: BitmapDecoder = block_on_async(BitmapDecoder::CreateAsync(&stream))?;
            let software_bitmap = block_on_async(decoder.GetSoftwareBitmapAsync())?;
            ...
            let ocr_result = block_on_async(engine.RecognizeAsync(&software_bitmap))?;
            let lines: windows_collections::IVectorView<windows::Media::Ocr::OcrLine> =
                w(ocr_result.Lines())?;
            let line_count = w(lines.Size())?;

            let mut raw_matches = Vec::new();
            for line in &lines {
                let words: windows_collections::IVectorView<windows::Media::Ocr::OcrWord> =
                    w(line.Words())?;
                for word in &words {
    ...
    pub fn find_text_matches(
        ...
        let mut api = match TessApi::new(None, "eng") {
            ...
            Err(_) => {
                ...
                api.ok_or_else(|| Ai00XError::tool(
        ...
        let boxa = api
            ...
            .ok_or_else(|| {
                Ai00XError::tool("Linux OCR: Tesseract did not return word regions.".to_string())
            })?;
