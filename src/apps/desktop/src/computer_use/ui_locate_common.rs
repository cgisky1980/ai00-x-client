...
use ai00_x_core::agent::tools::computer_use_host::{UiElementLocateQuery, UiElementLocateResult};
use ai00_x_core::util::errors::{Ai00XError, Ai00XResult};
use screenshots::display_info::DisplayInfo;

pub fn validate_query(q: &UiElementLocateQuery) -> Ai00XResult<()> {
    let t = q
        .title_contains
        .as_ref()
...
fn global_xy_to_native_with_display(d: &DisplayInfo, gx: f64, gy: f64) -> Ai00XResult<(u32, u32)> {
...
pub fn global_to_native_center(gx: f64, gy: f64) -> Ai00XResult<(u32, u32)> {
    let d = DisplayInfo::from_point(gx.round() as i32, gy.round() as i32)
        .map_err(|e| Ai00XError::tool(format!("DisplayInfo::from_point: {}", e)))?;
