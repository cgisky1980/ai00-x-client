...
use ai00_x_core::agent::tools::computer_use_host::{
...
use ai00_x_core::agent::tools::computer_use_optimizer::ComputerUseOptimizer;
use ai00_x_core::util::errors::{Ai00XError, Ai00XResult};
...
impl MacPointerGeo {
    ...
    fn full_pixel_center_to_global_f64(&self, cx: f64, cy: f64) -> Ai00XResult<(f64, f64)> {
...
impl PointerMap {
    ...
    fn map_image_to_global_f64(&self, x: i32, y: i32) -> Ai00XResult<(f64, f64)> {
        if self.image_w == 0
            || self.image_h == 0
            || self.content_w == 0
            || self.content_h == 0
    ...
    fn map_normalized_to_global_f64(&self, x: i32, y: i32) -> Ai00XResult<(f64, f64)> {
...
impl DesktopComputerUseHost {
  ...
  return (unix id of p as text) & "|" & (name of p) & "|" & (try (bundle identifier of p as text) on error "" end try)
    ...
    fn ensure_input_automation_allowed() -> Ai00XResult<()> {
        #[cfg(target_os = "macos")]
        {
            if macos::ax_trusted() {
                return Ok(());
            ...
            return Err(Ai00XError::tool(format!(
                "macOS Accessibility is not enabled for this executable. System Settings > Privacy & Security > Accessibility: add and enable Ai00-X. Development builds use the debug binary at: {}",
                exe
            )));
        }
        #[cfg(not(target_os = "macos"))]
    ...
    fn with_enigo<F, T>(f: F) -> Ai00XResult<T>
    where
        F: FnOnce(&mut Enigo) -> Ai00XResult<T>,
    {
        Self::ensure_input_automation_allowed()?;
        let settings = Settings::default();
        let mut enigo =
            Enigo::new(&settings).map_err(|e| Ai00XError::tool(format!("enigo init: {}", e)))?;
        f(&mut enigo)
    }
    ...
    fn run_enigo_job<F, T>(job: F) -> Ai00XResult<T>
    where
        F: FnOnce(&mut Enigo) -> Ai00XResult<T> + Send,
        T: Send,
    {
        #[cfg(target_os = "macos")]
        {
    ...
    #[cfg(target_os = "macos")]
    fn post_mouse_moved_cg_global(x: f64, y: f64) -> Ai00XResult<()> {
        ...
        let source =
            CGEventSource::new(CGEventSourceStateID::CombinedSessionState).map_err(|_| {
                Ai00XError::tool("CGEventSource create failed (mouse_move)".to_string())
            })?;
        let pt = CGPoint { x, y };
        let ev = CGEvent::new_mouse_event(source, CGEventType::MouseMoved, pt, CGMouseButton::Left)
            .map_err(|_| Ai00XError::tool("CGEvent MouseMoved failed".to_string()))?;
    ...
    #[cfg(target_os = "macos")]
    fn smooth_mouse_move_cg_global(x1: f64, y1: f64) -> Ai00XResult<()> {
    ...
    #[cfg(not(target_os = "macos"))]
    fn smooth_mouse_move_enigo_abs(x1: f64, y1: f64) -> Ai00XResult<()> {
        const MIN_DIST: f64 = 2.5;
        const MIN_STEPS: usize = 8;
        const MAX_STEPS: usize = 85;
        const MAX_DURATION_MS: u64 = 400;
        ...
        Self::run_enigo_job(|e| {
            let (cx, cy) = e.location().map_err(|err| {
                Ai00XError::tool(format!("smooth_mouse_move: pointer location: {}", err))
            })?;
            let x0 = cx as f64;
            let y0 = cy as f64;
            let dx = x1 - x0;
            ...
            if dist < MIN_DIST {
                return e
                    ...
                    .map_err(|err| Ai00XError::tool(format!("mouse_move: {}", err)));
            }
            let duration_ms = (70.0 + dist * 0.28).min(MAX_DURATION_MS as f64) as u64;
            let steps = ((dist / 5.5).ceil() as usize).clamp(MIN_STEPS, MAX_STEPS);
            let step_delay = Duration::from_millis((duration_ms / steps as u64).max(1));
    ...
    fn map_button(s: &str) -> Ai00XResult<Button> {
        match s.to_lowercase().as_str() {
            "left" => Ok(Button::Left),
            "right" => Ok(Button::Right),
            "middle" => Ok(Button::Middle),
            _ => Err(Ai00XError::tool(format!("Unknown mouse button: {}", s))),
        }
    }

    fn map_key(name: &str) -> Ai00XResult<Key> {
        ...
        Ok(match n.as_str() {
            ...
            _ => {
                return Err(Ai00XError::tool(format!("Unknown key name: {}", name)));
            }
        })
    }

    fn encode_jpeg(rgb: &RgbImage, quality: u8) -> Ai00XResult<Vec<u8>> {
        let mut buf = Vec::new();
        ...
        .map_err(|e| Ai00XError::tool(format!("JPEG encode: {}", e)))?;
        Ok(buf)
    }
    ...
    fn raw_shot_from_rgb_crop(
        ...
        native_w: u32,
        native_h: u32,
    ) -> Ai00XResult<ComputerScreenshot> {
    ...
    fn ocr_full_primary_display_region() -> Ai00XResult<OcrRegionNative> {
        let screen = Screen::from_point(0, 0)
            .map_err(|e| Ai00XError::tool(format!("Screen capture init (OCR raw): {}", e)))?;
    ...
    fn ocr_region_square_around_point(cx: f64, cy: f64, half: u32) -> Ai00XResult<OcrRegionNative> {
        let hh = half as f64;
    ...
    fn screenshot_raw_native_region(region: OcrRegionNative) -> Ai00XResult<ComputerScreenshot> {
        ...
        let rgba = screen
            .capture()
            .map_err(|e| Ai00XError::tool(format!("Screenshot failed (OCR raw): {}", e)))?;
    ...
    fn crop_rgb(src: &RgbImage, x0: u32, y0: u32, w: u32, h: u32) -> Ai00XResult<RgbImage> {
        let (sw, sh) = src.dimensions();
        if x0.saturating_add(w) > sw || y0.saturating_add(h) > sh {
            return Err(Ai00XError::tool("Tile crop out of bounds.".to_string()));
        }
    ...
    fn screenshot_sync_tool_with_capture(
    ...
    ) -> Ai00XResult<(ComputerScreenshot, PointerMap, Option<ComputerUseNavFocus>)> {
        ...
        let (
            ...
            let Some(new_rect) = intersect_navigation_rect(expanded, full_rect) else {
                return Err(Ai00XError::tool("Quadrant crop out of bounds.".to_string()));
            };
        ...
        let (pointer_image_x, pointer_image_y) = {
            let pointer_loc = Self::run_enigo_job(|e| {
                e.location()
                    .map_err(|err| Ai00XError::tool(format!("pointer location: {}", err)))
            });
            match pointer_loc {
    ...
    fn permission_sync() -> ComputerUsePermissionSnapshot {
        #[cfg(target_os = "macos")]
        {
            let platform_note = if cfg!(debug_assertions) && !macos::ax_trusted() {
                Some(
                    "Development build: grant Accessibility to target/debug/ai00-x-desktop (path appears in errors if mouse fails)."
                        .to_string(),
                )
            } else {
                None
    ...
    fn computer_use_guard_verified_ui(&self) -> Ai00XResult<()> {
        let s = self
            .state
            .lock()
            .map_err(|e| Ai00XError::tool(format!("lock: {}", e)))?;
        if s.click_needs_fresh_screenshot {
            return Err(Ai00XError::tool(STALE_CAPTURE_TOOL_MESSAGE.to_string()));
        }
        Ok(())
    }
    ...
    fn resolve_screenshot_capture(
        ...
        mouse_x: f64,
        mouse_y: f64,
    ) -> Ai00XResult<(image::RgbaImage, Screen)> {
        ...
        let screen = Screen::from_point(mx, my)
            .or_else(|_| Screen::from_point(0, 0))
            .map_err(|e| Ai00XError::tool(format!("Screen capture init: {}", e)))?;
        let rgba = screen.capture().map_err(|e| {
            Ai00XError::tool(format!(
                "Screenshot failed (on macOS grant Screen Recording for Ai00-X): {}",
                e
            ))
        })?;
        Ok((rgba, screen))
...
#[cfg(target_os = "macos")]
mod macos {
    use super::{Ai00XError, Ai00XResult};
    ...
    pub fn quartz_mouse_location() -> Ai00XResult<(f64, f64)> {
        unsafe {
...
impl DesktopComputerUseHost {
    ...
    async fn mouse_click_at_current_pointer(&self, button: &str) -> Ai00XResult<()> {
        let button = button.to_string();
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                let b = Self::map_button(&button)?;
                e.button(b, Direction::Click)
                    .map_err(|err| Ai00XError::tool(format!("click: {}", err)))
            })
        })
        .await
        .map_err(|e| Ai00XError::tool(e.to_string()))??;
...
impl ComputerUseHost for DesktopComputerUseHost {
    async fn permission_snapshot(&self) -> Ai00XResult<ComputerUsePermissionSnapshot> {
    ...
    async fn request_accessibility_permission(&self) -> Ai00XResult<()> {
        #[cfg(target_os = "macos")]
        {
    ...
    async fn request_screen_capture_permission(&self) -> Ai00XResult<()> {
        #[cfg(target_os = "macos")]
        {
    ...
    async fn screenshot_peek_full_display(&self) -> Ai00XResult<ComputerScreenshot> {
        let (shot, _map, _) = tokio::task::spawn_blocking(|| {
            let screen = Screen::from_point(0, 0)
                .map_err(|e| Ai00XError::tool(format!("Screen capture init (peek): {}", e)))?;
            let rgba = screen
                .capture()
                .map_err(|e| Ai00XError::tool(format!("Screenshot failed (peek): {}", e)))?;
            Self::screenshot_sync_tool_with_capture(
    ...
    async fn ocr_find_text_matches(
        &self,
        text_query: &str,
        region_native: Option<ai00_x_core::agent::tools::computer_use_host::OcrRegionNative>,
    ) -> Ai00XResult<Vec<ai00_x_core::agent::tools::computer_use_host::OcrTextMatch>> {
        ...
        let desktop_matches: Vec<super::screen_ocr::OcrTextMatch> =
            tokio::task::spawn_blocking(move || {
                super::screen_ocr::find_text_matches(&shot, &query)
            })
            .await
            .map_err(|e: tokio::task::JoinError| Ai00XError::tool(e.to_string()))??;
        Ok(desktop_matches
            .into_iter()
            .map(
                |m| ai00_x_core::agent::tools::computer_use_host::OcrTextMatch {
                    text: m.text,
    ...
    async fn accessibility_hit_at_global_point(
        ...
        gx: f64,
        gy: f64,
    ) -> Ai00XResult<Option<ai00_x_core::agent::tools::computer_use_host::OcrAccessibilityHit>>
    {
        #[cfg(target_os = "macos")]
        {
    ...
    ) -> Ai00XResult<UiElementLocateResult> {
        Self::ensure_input_automation_allowed()?;
        #[cfg(target_os = "macos")]
        {
            return tokio::task::spawn_blocking(move || {
    ...
    async fn open_app(
        &self,
        app_name: &str,
    ) -> Ai00XResult<ai00_x_core::agent::tools::computer_use_host::OpenAppResult> {
        use ai00_x_core::agent::tools::computer_use_host::OpenAppResult;
        let name = app_name.to_string();

        #[cfg(target_os = "macos")]
        {
            let result = tokio::task::spawn_blocking(move || -> Ai00XResult<OpenAppResult> {
...
tell application "System Events" to get unix id of first process whose frontmost is true"#,
                    ...
                    .output()
                    .map_err(|e| Ai00XError::tool(format!("open_app osascript: {}", e)))?;
        ...
        #[cfg(target_os = "windows")]
        {
            let result = tokio::task::spawn_blocking(move || -> Ai00XResult<OpenAppResult> {
                let output = std::process::Command::new("cmd")
                    .args(["/c", "start", "", &name])
                    .output()
                    .map_err(|e| Ai00XError::tool(format!("open_app: {}", e)))?;
                Ok(OpenAppResult {
                    app_name: name,
    ...
    fn map_image_coords_to_pointer_f64(&self, x: i32, y: i32) -> Ai00XResult<(f64, f64)> {
        let s = self
            .state
            .lock()
            .map_err(|e| Ai00XError::tool(format!("lock: {}", e)))?;
    ...
    fn map_image_coords_to_pointer(&self, x: i32, y: i32) -> Ai00XResult<(i32, i32)> {
        let (gx, gy) = self.map_image_coords_to_pointer_f64(x, y)?;
        Ok((gx.round() as i32, gy.round() as i32))
    }

    fn map_normalized_coords_to_pointer_f64(&self, x: i32, y: i32) -> Ai00XResult<(f64, f64)> {
        let s = self
            .state
            .lock()
            .map_err(|e| Ai00XError::tool(format!("lock: {}", e)))?;
    ...
    fn map_normalized_coords_to_pointer(&self, x: i32, y: i32) -> Ai00XResult<(i32, i32)> {
        let (gx, gy) = self.map_normalized_coords_to_pointer_f64(x, y)?;
        Ok((gx.round() as i32, gy.round() as i32))
    }

    async fn mouse_move_global_f64(&self, gx: f64, gy: f64) -> Ai00XResult<()> {
        debug!(
    ...
    async fn mouse_move(&self, x: i32, y: i32) -> Ai00XResult<()> {
        self.mouse_move_global_f64(x as f64, y as f64).await
    }

    async fn pointer_move_relative(&self, dx: i32, dy: i32) -> Ai00XResult<()> {
        if dx == 0 && dy == 0 {
            return Ok(());
        }
            ...
            tokio::task::spawn_blocking(move || {
                Self::run_enigo_job(|e| {
                    let (cx, cy) = macos::quartz_mouse_location().map_err(|err| {
                        Ai00XError::tool(format!("quartz pointer (relative move): {}", err))
                    })?;
                    ...
                    e.move_mouse(nx, ny, Coordinate::Abs)
                        .map_err(|err| Ai00XError::tool(format!("pointer_move_relative: {}", err)))
                })
            })
            .await
            .map_err(|e| Ai00XError::tool(e.to_string()))??;
    ...
    async fn mouse_click(&self, button: &str) -> Ai00XResult<()> {
    ...
    async fn mouse_click_authoritative(&self, button: &str) -> Ai00XResult<()> {
        debug!("computer_use: mouse_click_authoritative button={}", button);
        self.mouse_click_at_current_pointer(button).await
    }

    async fn mouse_down(&self, button: &str) -> Ai00XResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                ...
                e.button(b, Direction::Press)
                    .map_err(|err| Ai00XError::tool(format!("mouse_down: {}", err)))
            })
        })
        .await
        .map_err(|e| Ai00XError::tool(e.to_string()))??;
    ...
    async fn mouse_up(&self, button: &str) -> Ai00XResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                ...
                e.button(b, Direction::Release)
                    .map_err(|err| Ai00XError::tool(format!("mouse_up: {}", err)))
            })
        })
        .await
        .map_err(|e| Ai00XError::tool(e.to_string()))??;
    ...
    async fn scroll(&self, delta_x: i32, delta_y: i32) -> Ai00XResult<()> {
            ...
            Self::run_enigo_job(|e| {
                if delta_x != 0 {
                    e.scroll(delta_x, Axis::Horizontal)
                        .map_err(|err| Ai00XError::tool(format!("scroll horizontal: {}", err)))?;
                }
                if delta_y != 0 {
                    e.scroll(delta_y, Axis::Vertical)
                        .map_err(|err| Ai00XError::tool(format!("scroll vertical: {}", err)))?;
                }
                Ok(())
            })
        })
    ...
    async fn key_chord(&self, keys: Vec<String>) -> Ai00XResult<()> {
        if keys.is_empty() {
            return Ok(());
        }
            ...
            Self::run_enigo_job(|e| {
                ...
                if mapped.len() == 1 {
                    e.key(mapped[0], Direction::Click)
                        .map_err(|err| Ai00XError::tool(format!("key: {}", err)))?;
                } else {
                    let mods = &mapped[..mapped.len() - 1];
                    let last = *mapped.last().unwrap();
                    for k in mods {
                        e.key(*k, Direction::Press)
                            .map_err(|err| Ai00XError::tool(format!("key press: {}", err)))?;
                    }
                    ...
                    e.key(last, Direction::Click)
                        .map_err(|err| Ai00XError::tool(format!("key click: {}", err)))?;
                    for k in mods.iter().rev() {
                        e.key(*k, Direction::Release)
                            .map_err(|err| Ai00XError::tool(format!("key release: {}", err)))?;
                    }
                    if chord_has_modifier {
    ...
    async fn type_text(&self, text: &str) -> Ai00XResult<()> {
        if text.is_empty() {
            return Ok(());
        }
        ...
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                e.text(&owned)
                    .map_err(|err| Ai00XError::tool(format!("type_text: {}", err)))
            })
        })
        .await
        .map_err(|e| Ai00XError::tool(e.to_string()))??;
    ...
    async fn wait_ms(&self, ms: u64) -> Ai00XResult<()> {
    ...
    fn computer_use_guard_click_allowed(&self) -> Ai00XResult<()> {
        let s = self
            .state
            .lock()
            .map_err(|e| Ai00XError::tool(format!("lock: {}", e)))?;
    ...
    fn computer_use_guard_click_allowed_relaxed(&self) -> Ai00XResult<()> {
