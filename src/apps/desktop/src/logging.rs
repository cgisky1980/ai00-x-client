...
fn is_embedded_webdriver_mode() -> bool {
    cfg!(debug_assertions) && std::env::var_os("AI00_X_WEBDRIVER_PORT").is_some()
}

fn resolve_logs_root() -> PathBuf {
    if let Some(path) = std::env::var_os("AI00_X_LOG_DIR").map(PathBuf::from) {
        return path;
    }

    if let Some(path) = std::env::var_os("AI00_X_E2E_LOG_DIR").map(PathBuf::from) {
        return path;
    }

    if is_embedded_webdriver_mode() {
        return std::env::temp_dir().join("ai00-x-e2e-logs");
    }
