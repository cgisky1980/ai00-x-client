/**
 * Underlay plugin runtime (injected / ESM form).
 *
 * Loads enabled plugins that declare `module` hooks for the underlay layer
 * (`underlay:mount`), dynamically imports their ESM entry (versioned URL to
 * bust module cache after reinstall) and calls `activate(ctx)`. Reacts to the
 * `plugins-changed` event for hot-plug (install/uninstall/enable/disable take
 * effect live). Emits `app:startup` on the plugin bus after the initial load.
 *
 * The underlay window receives raw mouse input injected by the backend, so
 * plugin DOM works with normal pointer events (no no-penetrate marking
 * needed).
 */
...
const HOOK_CONTAINERS: Record<string, string> = {
  "underlay:mount": "#ai00-underlay-plugin-layer",
};
...
function pluginHookIds(plugin: PluginInfo): string[] {
  return Object.keys(HOOK_CONTAINERS).filter(
    (hookId) => isModuleHook(plugin.manifest.hooks?.[hookId]),
  );
}
...
async function refreshAll(): Promise<void> {
  ...
  // Desired set: `${id}::${hook}` -> { plugin, hookId, modulePath }
...
/**
 * Initialize the underlay plugin runtime. Discovers hook containers from the
...
export function initPluginRuntime(): () => void {
  ...
  refreshChain = refreshChain
    .then(() => refreshAll())
    .then(() => {
      // Lifecycle hook: notify plugins that startup finished
