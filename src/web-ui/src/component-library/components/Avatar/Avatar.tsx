...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const avatarClass = [
    'ai00-x-avatar',
    `ai00-x-avatar--${shape}`,
    typeof size === 'string' && `ai00-x-avatar--${size}`,
    className
  ].filter(Boolean).join(' ');
  ...
  const renderContent = () => {
    ...
    if (icon) {
      return <span className="ai00-x-avatar__icon">{icon}</span>;
    }
    if (children) {
      return <span className="ai00-x-avatar__text">{children}</span>;
    }
    return null;
  };
...
export const AvatarGroup: React.FC<AvatarGroupProps> = ({
  ...
  return (
    <div className={`ai00-x-avatar-group ${className}`} style={style}>
      {displayChildren}
      {restCount > 0 && (
        <Avatar className="ai00-x-avatar-group__rest">+{restCount}</Avatar>
      )}
    </div>
  );
};
