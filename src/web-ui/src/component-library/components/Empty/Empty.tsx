...
export const Empty: React.FC<EmptyProps> = ({
  ...
  return (
    <div className={`ai00-x-empty ${className}`} style={style}>
      <div className="ai00-x-empty__image">
        {image || <DefaultImage size={size} />}
      </div>
      {description && (
        <div className="ai00-x-empty__description">{description}</div>
      )}
      {children && <div className="ai00-x-empty__footer">{children}</div>}
    </div>
  );
};
