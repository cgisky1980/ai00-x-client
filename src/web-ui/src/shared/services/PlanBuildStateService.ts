...
class PlanBuildStateService {
  ...
  private setupGlobalListeners(): void {
    window.addEventListener('ai00-x:todowrite-update', this.handleTodoWriteUpdate);
    window.addEventListener('ai00-x:dialog-cancelled', this.handleDialogCancelled);
  }
  ...
  private handleTodoWriteUpdate = async (event: Event): Promise<void> => {
    ...
    log.debug('TodoWrite update received', {
      incomingCount: incomingTodos.length,
      incomingIds: incomingTodos.map(t => t.id),
      activeBuilds: Array.from(this.buildingPlans.entries()).map(([k, v]) => ({
        key: k,
        todoIds: Array.from(v.todoIds),
      })),
    });
    ...
    for (const [key, entry] of this.buildingPlans.entries()) {
      ...
      if (matchedTodos.length === 0) {
        log.debug('No matching todos for build', {
          key,
          planTodoIds: Array.from(entry.todoIds),
          incomingIds: incomingTodos.map(t => t.id),
        });
        continue;
      }

      log.debug('Matched todos for build', {
        key,
        matchedCount: matchedTodos.length,
        matchedIds: matchedTodos.map(t => t.id),
      });
