...
export class AgentService {
  ...
  private async setupEventListeners() {
    ...
    const unlisten2 = await listen<StreamStartEvent>('agent_stream_start', () => {
    ...
    const unlisten6 = await listen<StreamChunkEvent>('agent_stream_chunk', (event) => {
    ...
    const unlisten7 = await listen<StreamToolUseEvent>('agent_stream_tool_use', (event) => {
    ...
    const unlisten8 = await listen<StreamToolResultEvent>('agent_stream_tool_result', (event) => {
    ...
    const unlisten9 = await listen<StreamProgressEvent>('agent_stream_progress', (event) => {
    ...
    const unlisten10 = await listen<StreamCompleteEvent>('agent_stream_complete', (event) => {
    ...
    const unlisten11 = await listen<StreamErrorEvent>('agent_stream_error', (event) => {
  ...
  async executeAgentTaskStream(
    ...
    try {
      ...
      if (!workspacePath) {
        throw new Error('Workspace path is required to create an agent task');
      }
