...
export type VirtualItem =
  ...
  | { type: 'turn-token-summary'; data: { rounds: ModelRound[] }; turnId: string };
...
export function sessionToVirtualItems(session: Session | null): VirtualItem[] {
  ...
  session.dialogTurns.forEach(turn => {
    ...
    let nonEmptyRounds = turn.modelRounds.filter(round => round.items && round.items.length > 0);

    const seenPlanPaths = new Set<string>();
    const seenTodoWriteKeys = new Set<string>();
    ...
    for (const round of nonEmptyRounds) {
      for (const item of round.items) {
        ...
        if (ti.toolName === 'CreatePlan') {
          const raw = ti.toolResult?.result?.plan_file_path
            || (ti.toolCall?.input as Record<string, unknown>)?.plan_file_path
            || '';
          const key = String(raw).replace(/\\/g, '/').toLowerCase();
          if (!key) continue;
          if (seenPlanPaths.has(key)) {
            skipItemIds.add(item.id);
          } else {
            seenPlanPaths.add(key);
          }
        }

        if (ti.toolName === 'TodoWrite') {
          const todos = ti.toolResult?.result?.todos
            || (ti.toolCall?.input as Record<string, unknown>)?.todos;
          if (Array.isArray(todos) && todos.length > 0) {
            const key = todos.map((t: Record<string, unknown>) =>
              `${t.id || ''}:${t.content || ''}:${t.status || ''}`
            ).join('|');
            if (seenTodoWriteKeys.has(key)) {
              skipItemIds.add(item.id);
            } else {
              seenTodoWriteKeys.add(key);
            }
          }
        }
      }
    ...
    if (skipItemIds.size > 0) {
      nonEmptyRounds = nonEmptyRounds.map(round => {
        const filtered = round.items.filter(item => !skipItemIds.has(item.id));
        if (filtered.length === round.items.length) return round;
        return { ...round, items: filtered };
    ...
    // Token summary disabled — not useful in the chat view
    // if (turn.status === 'completed' && hasTokenData) {
    //   items.push({
    //     type: 'turn-token-summary',
    //     data: { rounds: turn.modelRounds },
    //     turnId: turn.id,
    //   });
    // }
  });
