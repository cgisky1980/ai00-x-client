...
export interface Session {
  ...
  /** SSH config host for `~/.ai00-x/remote_ssh/{host}/...` session paths when disconnected. */
  remoteSshHost?: string;

  /** Sandbox branch bound to this session. */
  sandboxBranch?: string;

  /** Whether task is fully completed and awaiting user action */
  taskCompleted?: boolean;
  /** Which completion phase: 'taskCompletion' | 'rating' | 'archived' */
  completionPhase?: 'taskCompletion' | 'rating' | 'archived';

  /** Plan file path when plan mode is active */
  planFilePath?: string | null;

  /** Current workflow phase */
  workflowPhase?: 'planning' | 'awaiting_plan_confirmation' | 'executing' | 'reviewing';

  /** Model usage statistics: maps resolved model_id to call count */
  modelUsageStats?: Record<string, number>;

  /** Plan confirmation state */
  planConfirmation?: {
    isAwaiting?: boolean;
    autoReviewState?: 'idle' | 'reviewing' | 'completed' | 'error';
    reviewedAt?: number;
    summary?: string;
    issuesFound?: number;
...
export interface SessionConfig {
  ...
  /** Sandbox branch bound to this session. */
  sandboxBranch?: string;
  /** Override session display mode ('code' | 'cowork' | 'task'). */
  sessionDisplayMode?: string;
}
