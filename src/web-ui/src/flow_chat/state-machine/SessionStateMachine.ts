...
function mapWorkflowPhaseToProcessingPhase(phase: string): ProcessingPhase {
  switch (phase) {
    case 'planning': return ProcessingPhase.WORKFLOW_PLANNING;
    case 'awaiting_plan_confirmation': return ProcessingPhase.AWAITING_PLAN_CONFIRMATION;
    case 'executing': return ProcessingPhase.WORKFLOW_EXECUTING;
    case 'reviewing': return ProcessingPhase.WORKFLOW_REVIEWING;
    default: return ProcessingPhase.THINKING;
  }
}
...
export class SessionStateMachineImpl {
  ...
  private updateContext(event: SessionExecutionEvent, payload?: any) {
    ...
    switch (event) {
      ...
      case SessionExecutionEvent.WORKFLOW_PHASE_CHANGED:
        if (payload?.toPhase) {
          this.context.workflowPhase = payload.toPhase as WorkflowPhase;
          this.context.processingPhase = mapWorkflowPhaseToProcessingPhase(payload.toPhase);
        }
        break;
