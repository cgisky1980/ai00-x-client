...
export class FlowChatManager {
  ...
  async initialize(
    ...
    try {
      ...
      const activeSession = state.activeSessionId
        ? this.context.flowChatStore.getState().sessions.get(state.activeSessionId) ?? null
        : null;
      const activeSessionBelongsToWorkspace =
        !!activeSession && sessionMatchesWorkspace(activeSession);

      // If the active session does not belong to this workspace, clear it
      // so the UI does not show a stale session from a different workspace.
      if (!activeSessionBelongsToWorkspace && state.activeSessionId) {
        this.context.flowChatStore.clearActiveSession();
      }
  ...
  private handleTodoWriteResult(sessionId: string, turnId: string, result: any): void {
    try {
      ...
      window.dispatchEvent(new CustomEvent('ai00-x:todowrite-update', {
        detail: {
          sessionId,
          turnId,
          todos: incomingTodos,
