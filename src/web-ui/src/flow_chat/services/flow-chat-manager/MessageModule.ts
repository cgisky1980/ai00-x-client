...
export async function sendMessage(
  ...
  try {
    ...
    const currentAgentType = (agentType?.trim() || session.mode || 'Code').trim();
    ...
    // Wallpaper session: auto-compact context before sending new message
    if (currentAgentType === 'Wallpaper' && workspacePath) {
      try {
        const result = await wallpaperAPI.compactWallpaperContext(sessionId, workspacePath);
        if (result.compacted && result.removedTurns > 0) {
          log.info('Wallpaper session context compacted', {
            sessionId,
            removedTurns: result.removedTurns,
          });
          // Remove compacted turns from frontend store
          const currentTurns = context.flowChatStore.getState().sessions.get(sessionId)?.dialogTurns ?? [];
          if (currentTurns.length > result.removedTurns) {
            context.flowChatStore.removeLeadingDialogTurns(sessionId, result.removedTurns);
          }
        }
      } catch (compactError) {
        log.warn('Wallpaper context compaction failed, continuing without compaction', {
          sessionId,
          error: compactError,
        });
      }
