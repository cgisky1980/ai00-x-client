...
type SessionDisplayMode = 'code' | 'cowork' | 'task' | 'wall';
...
const normalizeSessionDisplayMode = (
  ...
  if (normalizedMode === 'task') return 'task';
  if (normalizedMode === 'wall' || normalizedMode === 'wallpaper') return 'wall';
  return 'code';
};
...
const DISPLAY_MODE_TO_AGENT_TYPE: Record<SessionDisplayMode, string> = {
  code: 'Code',
  cowork: 'Code',
  task: 'Task',
  wall: 'Wallpaper',
...
const resolveAgentType = (
  ...
  if (!requestedMode) return 'Code';
  const displayMode = normalizeSessionDisplayMode(requestedMode);
  return DISPLAY_MODE_TO_AGENT_TYPE[displayMode] || 'Code';
};
...
export async function createChatSession(
  ...
  try {
    ...
    const sessionMode = config.sessionDisplayMode
      ? normalizeSessionDisplayMode(config.sessionDisplayMode, workspace)
      : normalizeSessionDisplayMode(agentType, workspace);
    const creationKey =
    ...
    const sessionName = i18nService.t('flow-chat:session.newWithIndex', { count: sameModeCount });
    ...
    const creationId = `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
    const sandboxStore = useSandboxCreationStore.getState();

    sandboxStore.show(sessionName, () => {
      agentAPI.cancelSessionCreation(creationId).catch(() => {});
    });
    ...
    const createPromise = (async () => {
      ...
      context.flowChatStore.createSession(
        ...
        remoteConnectionId,
        remoteSshHost,
        response.sandboxBranch
      );

      if (response.sandboxError) {
        notificationService.warning(response.sandboxError, { duration: 5000 });
      }
    ...
    useSandboxCreationStore.getState().hide();
    log.error('Failed to create chat session', { config, error });
    
    const errorMessage = error instanceof Error ? error.message : String(error);
    if (errorMessage.includes('cancelled')) {
      notificationService.info(i18nService.t('flow-chat:sandboxCreation.cancelled'), {
        duration: 2000
      });
    } else {
    ...
    if (!allowRecreateOnCoordinatorFailure) {
      ...
      const hint =
        raw.includes('Session metadata not found') || raw.includes('Not found')
          ? i18nService.t('flow-chat:session.backendSessionNotFound')
          : raw;
