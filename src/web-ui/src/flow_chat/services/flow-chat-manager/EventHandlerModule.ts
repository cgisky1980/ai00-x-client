 ...
 * Initializes event listeners and handles various Agent events
 */

import { FlowChatStore } from '../../store/FlowChatStore';
import { stateMachineManager } from '../../state-machine';
import { SessionExecutionEvent, SessionExecutionState } from '../../state-machine/types';
import { agentEventListener, type AgentEventCallbacks } from '../AgentEventListener';
import { 
...
function logDroppedDataEvent(
  ...
  log.debug('Dropped agent data event', {
    eventName,
    sessionId,
    turnId,
    ...details,
...
export async function initializeEventListeners(
  ...
  const callbacks: AgentEventCallbacks = {
    ...
    onModelRoundCompleted: (event) => {
      handleModelRoundCompleted(context, event);
    },
    ...
    onWorkflowPhaseChanged: (event) => {
      handleWorkflowPhaseChanged(context, event);
    },
    onPlanConfirmationNeeded: (event) => {
      handlePlanConfirmationNeeded(context, event);
    },
    onPlanConfirmationResponded: (event) => {
      handlePlanConfirmationResponded(context, event);
    },
    onPlanAutoReviewStarted: (event) => {
      handlePlanAutoReviewStarted(context, event);
    },
  ...
  await agentEventListener.startListening(callbacks);

  return () => {
    unlistenProgress();
    unlistenTerminalReady();
    unlistenMcpInteractionRequest();
    agentEventListener.stopListening();
  };
}
...
function handleWorkflowPhaseChanged(_context: FlowChatContext, event: WorkflowPhaseChangedEvent): void {
  const { sessionId, fromPhase, toPhase } = event;
  log.info('Workflow phase changed', { sessionId, fromPhase, toPhase });

  const store = FlowChatStore.getInstance();
  store.setWorkflowPhase(sessionId, toPhase as 'planning' | 'awaiting_plan_confirmation' | 'executing' | 'reviewing');
  ...
  void stateMachineManager
    .transition(sessionId, SessionExecutionEvent.WORKFLOW_PHASE_CHANGED, {
      fromPhase,
      toPhase,
    })
    .catch(error => {
      log.error('State machine transition failed on workflow phase change', { sessionId, error });
    });
}
...
function handlePlanConfirmationNeeded(_context: FlowChatContext, event: PlanConfirmationNeededEvent): void {
  const { sessionId, planFilePath } = event;
  log.info('Plan confirmation needed', { sessionId, planFilePath });

  const store = FlowChatStore.getInstance();
  store.setPlanConfirmationNeeded(sessionId, planFilePath || null);
  syncSessionToModernStore(sessionId);

  if (planFilePath) {
    ideControl.navigation.goToFile(planFilePath).catch(err => {
      log.error('Failed to auto-open plan viewer on PlanConfirmationNeeded', err);
    });
  }
}

function handlePlanConfirmationResponded(_context: FlowChatContext, event: PlanConfirmationRespondedEvent): void {
  const { sessionId, confirmed } = event;
  log.info('Plan confirmation responded', { sessionId, confirmed });

  const store = FlowChatStore.getInstance();
  store.setPlanConfirmationResponded(sessionId);
  syncSessionToModernStore(sessionId);

  if (confirmed) {
    const session = store.getState().sessions.get(sessionId);
    const planFilePath = session?.planFilePath;
    if (planFilePath) {
      loadPlanTodos(planFilePath).then(todos => {
        store.setActivePlan(sessionId, {
          planFilePath,
          name: 'Plan',
          todos,
        });
      ...
      ideControl.navigation.goToFile(planFilePath).catch(err => {
        log.error('Failed to auto-open plan viewer after confirmation', err);
      });
    }
  }
}

function handlePlanAutoReviewStarted(_context: FlowChatContext, event: PlanAutoReviewStartedEvent): void {
  const { sessionId } = event;
  log.info('Plan auto review started', { sessionId });

  const store = FlowChatStore.getInstance();
  store.setAutoReviewState(sessionId, 'reviewing');
  syncSessionToModernStore(sessionId);
}

function handlePlanAutoReviewCompleted(_context: FlowChatContext, event: PlanAutoReviewCompletedEvent): void {
  const { sessionId, summary } = event;
  log.info('Plan auto review completed', { sessionId });

  const store = FlowChatStore.getInstance();
  store.setAutoReviewState(sessionId, 'completed', summary);
  syncSessionToModernStore(sessionId);
}
 ...
 */
function handleSessionStateChanged(context: FlowChatContext, event: any): void {
  ...
  (machineContext as any).backendSyncedAt = Date.now();
  ...
  if (currentFrontendState !== frontendState && !isExpectedFinishingDrift) {
    ...
    // Force-sync: if backend is IDLE but frontend is stuck in PROCESSING/FINISHING,
    // the DialogTurnFailed/Cancelled event was likely lost in transit.
    // Force the frontend state machine back to IDLE so the user can continue.
    if (frontendState === SessionExecutionState.IDLE &&
        (currentFrontendState === SessionExecutionState.PROCESSING ||
         currentFrontendState === SessionExecutionState.FINISHING)) {
      log.info('Force-syncing frontend state to IDLE (backend is already idle)', { sessionId });
      machine.transition(SessionExecutionEvent.RESET);
      // Also clean up any stale UI state
      const session = context.flowChatStore.getState().sessions.get(sessionId);
      if (session) {
        context.flowChatStore.updateSessionMode(sessionId, 'idle');
      }
    }
  }
}
...
function handleModelRoundCompleted(_context: FlowChatContext, event: any): void {
  ...
  if (subagentParentInfo) {
    const parentSessionId = subagentParentInfo.sessionId;
    const parentTurnId = subagentParentInfo.dialogTurnId;
    if (parentSessionId && parentTurnId && modelId && (inputTokens !== undefined || outputTokens !== undefined)) {
      store.recordModelUsage(parentSessionId, modelId);
      store.recordSubagentModelRound(parentSessionId, parentTurnId, {
        modelId,
        inputTokens,
        outputTokens,
        totalTokens,
 ...
 */
// Disabled: see appendPlanDisplayItemsIfNeeded above.
...
//         if (planPath) createPlanFiles.add(normalizePathKey(planPath));
//       }
//       
...
//         if (filePath.endsWith('/PLAN.md') || filePath.endsWith('\\PLAN.md') || filePath === 'PLAN.md') {
...
//   }
//   
//   return [...new Set(planFiles)].filter(f => !createPlanFiles.has(normalizePathKey(f)));
// }

/**
 * Append PlanDisplay tool items if plan files were modified
 */
// Disabled: duplicate CreatePlan items are now deduped at the rendering layer
// (sessionToVirtualItems in modernFlowChatStore.ts).
