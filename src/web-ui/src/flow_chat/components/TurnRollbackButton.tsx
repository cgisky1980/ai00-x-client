...
export const TurnRollbackButton: React.FC<TurnRollbackButtonProps> = ({
  ...
  if (isCurrent) {
    return <span className="turn-rollback-button-current">{t('toolCards.turnRollback.current')}</span>;
  }
  ...
  return (
    <button
      ...
      disabled={loading}
      title={t('toolCards.turnRollback.rollbackToTurn', { index: turnIndex + 1 })}
    >
      {loading ? t('toolCards.turnRollback.rollingBack') : t('toolCards.turnRollback.rollback')}
    </button>
  );
};
