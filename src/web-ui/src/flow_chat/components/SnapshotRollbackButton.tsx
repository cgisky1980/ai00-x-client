...
export const SnapshotRollbackButton: React.FC<SnapshotRollbackButtonProps> = ({
  ...
  if (isCurrentTurn) {
    return (
      <div className="snapshot-rollback-button snapshot-rollback-button--current">
        <Check size={14} />
        <span>{t('snapshotRollback.currentCodeState')}</span>
      </div>
    );
  }
  ...
  if (rollbackStatus === 'success') {
    return (
      <div className="snapshot-rollback-button snapshot-rollback-button--success">
        <Check size={14} />
        <span>{t('snapshotRollback.rolledBackToTurn')}</span>
      </div>
    );
  }
  ...
  if (rollbackStatus === 'error') {
    return (
      <div className="snapshot-rollback-button snapshot-rollback-button--error" title={errorMessage}>
        <AlertCircle size={14} />
        <span>{t('snapshotRollback.rollbackFailed')}</span>
      </div>
    );
  }
  ...
  return (
    <button
      ...
      title={t('snapshotRollback.rollbackToCodeState', { index: turnIndex + 1 })}
    >
      {isRollingBack ? (
        <>
          <Loader2 size={14} className="snapshot-rollback-button__spinner" />
          <span>{t('snapshotRollback.rollingBack')}</span>
        </>
      ) : (
        <>
          <RotateCcw size={14} />
          <span>{t('snapshotRollback.rollbackToThisTurn')}</span>
        </>
      )}
    </button>
  );
