...
import { useSnapshotState } from '../../../tools/snapshot-system/hooks/useSnapshotState';
...
import { openMainSession } from '../../services/sessionNavigation';
...
export const SessionFilesBadge: React.FC<SessionFilesBadgeProps> = ({
  ...
  const handleReviewClick = useCallback(async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const reviewSessionId = await flowChatManager.createChatSession({
        workspacePath: currentWorkspace?.rootPath,
        agentType: 'CodeReview',
      }, 'CodeReview');

      await flowChatManager.renameChatSessionTitle(
        reviewSessionId,
        t('sessionFilesBadge.review.threadTitle', { defaultValue: 'Code review' })
      );

      await openMainSession(reviewSessionId);
