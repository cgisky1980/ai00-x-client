...
const RenameInput: React.FC<RenameInputProps> = ({ node, onRename, onCancel }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const input = document.querySelector('.ai00-x-file-explorer__rename-input-wrapper input') as HTMLInputElement | null;
      if (!input) {
        return;
      }
  ...
  return (
    <div className="ai00-x-file-explorer__rename-input-wrapper" onClick={(event) => event.stopPropagation()}>
      <Input
        type="text"
        variant="filled"
        inputSize="small"
...
export const FileTreeItem: React.FC<FileTreeItemProps> = ({
  ...
  return (
    <div 
      className={`ai00-x-file-explorer__node-content ${isSelected ? 'ai00-x-file-explorer__node-content--selected' : ''} ${node.isDirectory ? 'ai00-x-file-explorer__node-content--directory' : ''} ${isCompressed ? 'ai00-x-file-explorer__node-content--compressed' : ''} ${className}`}
    ...
    >
      {node.isDirectory ? (
        <span className={`ai00-x-file-explorer__expand-icon ${isExpanded ? 'ai00-x-file-explorer__expand-icon--expanded' : ''}`} onClick={handleExpandClick}>
          {isLoading ? (
            <Loader2 size={16} className="ai00-x-file-explorer__loading-icon" />
          ) : isExpanded ? (
            <ChevronDown size={16} />
          ) : (
      ...
      {isRenaming ? (
        ...
        <span className={`ai00-x-file-explorer__node-name ${isCompressed ? 'ai00-x-file-explorer__compressed-path' : ''}`}>
          {node.name}
        </span>
      )}

      {renderActions ? (
        <div className="ai00-x-file-explorer__node-actions" onClick={(event) => event.stopPropagation()}>
          {renderActions(node)}
        </div>
      ) : null}
    </div>
