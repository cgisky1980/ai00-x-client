...
const HighlightedText = memo<HighlightedTextProps>(({ text, query }) => {
  ...
  while (matchIndex !== -1) {
    ...
    parts.push(
      <mark key={keyIndex++} className="ai00-x-search-results__highlight">
...
const MatchItem = memo<MatchItemProps>(({ match, target, searchQuery, onLineClick }) => {
  ...
  return (
    <button
      type="button"
      className="ai00-x-search-results__match"
      onClick={() => onLineClick(target, match.lineNumber)}
    >
      <span 
        className="ai00-x-search-results__match-content"
        title={match.matchedContent || ''}
      >
        <code>
          {preview.before && (
            <span className="ai00-x-search-results__match-before">{preview.before}</span>
          )}
          {preview.inside ? (
            <mark className="ai00-x-search-results__highlight ai00-x-search-results__match-highlight">
              {preview.inside}
            </mark>
          ) : null}
          {preview.after && (
            <span className="ai00-x-search-results__match-after">{preview.after}</span>
          )}
        </code>
      </span>
    </button>
...
const FileGroup = memo<FileGroupProps>(({ 
  ...
  return (
    <div className="ai00-x-search-results__group">
      <div className="ai00-x-search-results__file">
        <button
          type="button"
          className="ai00-x-search-results__file-main"
          onClick={() => onFileClick(target)}
          onContextMenu={(event) => onFileContextMenu(event, target)}
        >
          <span
            className={`ai00-x-search-results__file-icon${
              group.isDirectory ? ' ai00-x-search-results__file-icon--directory' : ''
            }`}
          >
            {group.isDirectory ? (
              <Folder size={16} />
          ...
          <span className="ai00-x-search-results__file-info">
            <span className="ai00-x-search-results__file-name">
              <HighlightedText text={group.name} query={searchQuery} />
            </span>
            <span className="ai00-x-search-results__file-path">
              {group.path}
            </span>
          </span>
        </button>
        ...
        {hasContentMatches && (
          <button
            type="button"
            className="ai00-x-search-results__file-toggle"
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            ...
            <span className="ai00-x-search-results__file-toggle-count">
      ...
      {hasContentMatches && isExpanded && (
        <div className="ai00-x-search-results__matches">
...
export const FileSearchResults: React.FC<FileSearchResultsProps> = ({
  ...
  if (results.length === 0) {
    return (
      <div className={`ai00-x-search-results ai00-x-search-results--empty ${className}`}>
        <div className="ai00-x-search-results__empty">
          <div className="ai00-x-search-results__empty-icon">
            <FileText size={48} />
          </div>
          <p>{t('search.noResults')}</p>
          <p className="ai00-x-search-results__empty-hint">
  ...
  return (
    <div className={`ai00-x-search-results ${className}`}>
      <div className="ai00-x-search-results__header">
        <span className="ai00-x-search-results__count">
          {t('search.resultsSummary', { files: results.length, matches: totalMatches })}
          {hasMore && <span className="ai00-x-search-results__showing">{t('search.resultsShowing', { count: displayCount })}</span>}
        </span>
      </div>

      <div
        ref={listRef}
        className="ai00-x-search-results__list"
