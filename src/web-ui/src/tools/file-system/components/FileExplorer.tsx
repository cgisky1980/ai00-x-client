...
const ScrollBreadcrumb: React.FC<ScrollBreadcrumbProps> = ({ containerRef, workspacePath, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const detectCurrentDirectory = () => {
      const treeContainer = container.querySelector('.ai00-x-file-explorer__tree');
      ...
      expandedDirNodes.forEach((node) => {
        ...
        const nodeElement = node.closest('.ai00-x-file-explorer__node');
        const childrenContainer = nodeElement?.querySelector(':scope > .ai00-x-file-explorer__node-children');
  ...
  return (
    <div className="ai00-x-file-explorer__breadcrumb">
      {displaySegments.map((segment, index) => (
        <React.Fragment key={segment.fullPath || index}>
          {index > 0 && (
            <ChevronRight size={10} className="ai00-x-file-explorer__breadcrumb-separator" />
          )}
          <span 
            className={`ai00-x-file-explorer__breadcrumb-item ${segment.fullPath ? 'ai00-x-file-explorer__breadcrumb-item--clickable' : ''}`}
...
export const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const renderNodeContent = useCallback((node: FileSystemNode, _level: number) => {
    return (
      <div className="ai00-x-file-explorer__node-wrapper">
        <span className={`ai00-x-file-explorer__node-name ${node.isCompressed ? 'ai00-x-file-explorer__compressed-path' : ''}`}>
          {node.name}
        </span>
        
        {showFileSize && !node.isDirectory && node.size && (
          <span className="ai00-x-file-explorer__node-size">
            {formatFileSize(node.size)}
          </span>
        )}
        
        {showLastModified && node.lastModified && (
          <span className="ai00-x-file-explorer__node-modified">
  ...
  const handleBlur = useCallback((e: React.FocusEvent) => {
    const toolbar = e.currentTarget.querySelector('.ai00-x-file-explorer__toolbar');
  ...
  const handleContainerClick = useCallback((e: React.MouseEvent) => {
    ...
    if (target.closest('.ai00-x-file-explorer__toolbar')) {
      return;
    }
    setIsFocused(true);
    setIsToolbarVisible(true);
  ...
  if (filteredFileTree.length === 0) {
    return (
      <div 
        className={`ai00-x-file-explorer ai00-x-file-explorer--empty ${className}`}
        data-area="file-explorer"
        data-workspace-root={workspacePath}
        tabIndex={0}
      >
        <div className="ai00-x-file-explorer__empty">
          <Folder size={48} className="ai00-x-file-explorer__empty-icon" />
  ...
  return (
    <div 
      ref={containerRef}
      className={`ai00-x-file-explorer ${className}`}
      data-area="file-explorer"
      data-workspace-root={workspacePath}
      tabIndex={0}
    ...
    >
      {(onNewFile || onNewFolder || onRefresh) && !hideToolbar && (
        <div 
          className={`ai00-x-file-explorer__toolbar ${isToolbarVisible ? 'ai00-x-file-explorer__toolbar--visible' : ''}`}
      ...
      {useVirtualScroll ? (
        <VirtualFileTree
          ...
          className="ai00-x-file-explorer__tree"
          workspacePath={workspacePath}
