...
export function getFileIconClass(node: FileSystemNode, isExpanded?: boolean): string {
  if (node.isDirectory) {
    return `ai00-x-file-explorer__icon ai00-x-file-explorer__icon--folder${isExpanded ? ' ai00-x-file-explorer__icon--folder-open' : ''}`;
  }
  
  const iconType = getFileIconType(node);
  return `ai00-x-file-explorer__icon ai00-x-file-explorer__icon--${iconType}`;
}
