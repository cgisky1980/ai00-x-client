...
<body>
  ...
  <script>
    (function () {
          ...
          window.morphdom(root, target, {
            ...
            onNodeAdded: function (node) {
              if (
                ...
                node.style.animation = 'ai00xWidgetFadeIn 0.18s ease both';
              }
              return node;
            }
          });
      ...
      var bridge = {
        send: function (data) {
          send('ai00-x-widget:event', data);
        }
      };

      window.ai00xWidget = bridge;
      ...
      document.addEventListener('click', function (event) {
        ...
        var fileTarget = target && target.closest ? target.closest('[data-file-path], [data-ai00-x-open-file]') : null;
        if (fileTarget) {
          var filePath = fileTarget.getAttribute('data-file-path') || fileTarget.getAttribute('data-ai00-x-open-file') || '';
          if (filePath) {
      ...
      window.addEventListener('message', function (event) {
        var data = event.data;
        if (!data || data.type !== 'ai00-x-widget:update') return;
...
export const GenerativeWidgetFrame: React.FC<GenerativeWidgetFrameProps> = ({
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent<WidgetMessage>) => {
      ...
      if (!data || data.source !== 'ai00-x-widget') return;
      if (data.widgetId && data.widgetId !== widgetId) return;

      if (data.type === 'ai00-x-widget:resize') {
  ...
  return (
    <div
      className={`ai00-x-generative-widget-frame ${className}`.trim()}
      style={{ height: `${frameHeight}px` }}
    >
      <iframe
        ref={iframeRef}
        title={title || 'Generative widget'}
        className="ai00-x-generative-widget-frame__iframe"
        style={{
