...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  const [_currentThemeId, setCurrentThemeId] = useState<string>(Ai00XDarkThemeMetadata.id);
  ...
  const saveFileContentRef = useRef<(() => Promise<void>) | null>(null);
  ...
  useEffect(() => {
    ...
    const initEditor = async () => {
      try {
        ...
        let themeId = Ai00XDarkThemeMetadata.id;
        try {
          const { themeService } = await import('@/infrastructure/theme');
          const currentTheme = themeService.getCurrentTheme();
          if (currentTheme) {
            themeId = currentTheme.monaco ? currentTheme.id : (currentTheme.type === 'dark' ? Ai00XDarkThemeMetadata.id : 'vs');
  ...
  useEffect(() => {
    ...
    const unsubRevertFile = globalEventBus.on('editor:revert-file', (data: { filePath: string }) => {
      if (isSamePath(data.filePath || '', filePath || '')) {
        const original = originalContentRef.current;
        if (original !== undefined && modelRef.current) {
          modelRef.current.setValue(original);
          monacoModelManager.markAsSaved(filePath || '');
          setHasChanges(false);
          hasChangesRef.current = false;
          savedVersionIdRef.current = modelRef.current.getAlternativeVersionId();
    ...
    unsubscribers.push(unsubRevertFile);
    ...
    (async () => {
      try {
        ...
        unsubscribeThemeService = themeService.on('theme:after-change', (event) => {
          if (event.theme) {
            const newThemeId = event.theme.monaco ? event.theme.id : (event.theme.type === 'dark' ? Ai00XDarkThemeMetadata.id : 'vs');
