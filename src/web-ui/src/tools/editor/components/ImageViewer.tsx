...
export const ImageViewer: React.FC<ImageViewerProps> = ({
  ...
  return (
    <div className={`ai00-x-image-viewer ${className} ${isFullscreen ? 'fullscreen' : ''}`}>
      <div className="ai00-x-image-viewer__toolbar">
        <div className="ai00-x-image-viewer__info">
          <span className="ai00-x-image-viewer__filename">{fileName || filePath.split(/[/\\]/).pop()}</span>
          {imageDimensions && (
            <span className="ai00-x-image-viewer__dimensions">
              {imageDimensions.width} × {imageDimensions.height}
            </span>
          )}
          {fileSize > 0 && (
            <span className="ai00-x-image-viewer__filesize">
              {formatFileSize(fileSize)}
            </span>
          )}
        </div>
        <div className="ai00-x-image-viewer__controls">
          <Tooltip content={t('editor.imageViewer.zoomOut')} placement="top">
            <button
              className="ai00-x-image-viewer__btn"
          ...
          <Tooltip content={t('editor.imageViewer.zoomReset')} placement="top">
            <button
              className="ai00-x-image-viewer__btn ai00-x-image-viewer__btn--zoom-display"
          ...
          <div className="ai00-x-image-viewer__divider" />
      ...
      <div className="ai00-x-image-viewer__container">
        {loading && (
          <div className="ai00-x-image-viewer__loading">
            <div className="ai00-x-image-viewer__spinner" />
            <p>{t('editor.common.loading')}</p>
          </div>
        )}

        {error && (
          <div className="ai00-x-image-viewer__error">
            <p>{error}</p>
            <p className="ai00-x-image-viewer__error-path">{filePath}</p>
          </div>
        )}

        {!loading && !error && imageUrl && (
          <div className="ai00-x-image-viewer__image-wrapper">
            <img
              src={imageUrl}
              alt={fileName || filePath}
              className="ai00-x-image-viewer__image"
              style={{
