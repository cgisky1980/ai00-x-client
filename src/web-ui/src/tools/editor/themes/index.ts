...
import { Ai00XDarkTheme, Ai00XDarkThemeMetadata } from './ai00-x-dark.theme';
...
export function initializeBuiltinThemes(): boolean {
  return registerTheme(Ai00XDarkThemeMetadata.id, Ai00XDarkTheme);
}

export function ensureDefaultTheme(): void {
  if (!registeredThemes.has(Ai00XDarkThemeMetadata.id)) {
    registerTheme(Ai00XDarkThemeMetadata.id, Ai00XDarkTheme);
  }
}

export function isThemesInitialized(): boolean {
  return registeredThemes.has(Ai00XDarkThemeMetadata.id);
}
...
export function getDefaultThemeId(): string {
  return Ai00XDarkThemeMetadata.id;
}

export { Ai00XDarkTheme, Ai00XDarkThemeMetadata };
