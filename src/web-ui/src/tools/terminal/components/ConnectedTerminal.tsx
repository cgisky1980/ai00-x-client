...
const ConnectedTerminal: React.FC<ConnectedTerminalProps> = memo(({
  ...
  const handlePaste = useCallback(async (text: string): Promise<boolean> => {
    ...
    if (lineCount > MULTILINE_PASTE_THRESHOLD) {
      ...
      const confirmed = await confirmWarning(
        t('pasteMultipleLines'),
        t('pasteMultipleLinesWarning', { lineCount }),
        {
          confirmText: t('paste'),
          cancelText: t('cancel'),
          preview,
  ...
  if (isLoading) {
    return (
      <div className={`ai00-x-terminal ${className}`}>
        <div className="ai00-x-terminal__loading">
          <div className="ai00-x-terminal__loading-spinner" />
          <span className="ai00-x-terminal__loading-text">{t('connecting')}</span>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className={`ai00-x-terminal ${className}`}>
        <div className="ai00-x-terminal__error">
          <AlertCircle className="ai00-x-terminal__error-icon" size={32} />
          <span className="ai00-x-terminal__error-message">{error}</span>
          <button 
            className="ai00-x-terminal__error-retry"
            onClick={handleRetry}
          >
            <RefreshCw size={14} />
            <span>{t('retry')}</span>
  ...
  return (
    <div className={`ai00-x-terminal ${className}`}>
      {showToolbar && (
        <div className="ai00-x-terminal__toolbar">
          <div className="ai00-x-terminal__toolbar-left">
            <TerminalIcon size={14} />
            <span className="ai00-x-terminal__toolbar-title">
              {title}
              {session && (
          ...
          <div className="ai00-x-terminal__toolbar-right">
            <button
              className="ai00-x-terminal__toolbar-btn"
            ...
            <button
              className="ai00-x-terminal__toolbar-btn ai00-x-terminal__toolbar-btn--danger"
              onClick={handleClose}
      ...
      {showStatusBar && session && (
        <div className={`ai00-x-terminal__statusbar ${
          isExited ? 'ai00-x-terminal__statusbar--exited' : ''
        } ${
          error ? 'ai00-x-terminal__statusbar--error' : ''
        }`}>
          <div className="ai00-x-terminal__statusbar-left">
            <span className="ai00-x-terminal__statusbar-item">
              {session.shellType}
            </span>
            <span className="ai00-x-terminal__statusbar-item">
              PID: {session.pid || '-'}
          ...
          <div className="ai00-x-terminal__statusbar-right">
            <span className="ai00-x-terminal__statusbar-item">
