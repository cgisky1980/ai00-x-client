...
const GitDiffView: React.FC<GitDiffViewProps> = ({
  ...
  const renderDiffContent = useCallback((diff: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (!line.startsWith('diff ') && !line.startsWith('index ') && 
        ...
        lineNumber++;
        diffLines.push(
          <div key={index} className={`ai00-x-git-diff-view__diff-line ai00-x-git-diff-view__diff-line--${lineType}`}>
            <span className="ai00-x-git-diff-view__line-number">{lineNumber}</span>
            <span className="ai00-x-git-diff-view__line-content">{content}</span>
          </div>
        );
      }
    });
  ...
  if (loading) {
    return (
      <div className={`ai00-x-git-diff-view ${className}`}>
        <div className="ai00-x-git-diff-view__loading-state">
          <div className="ai00-x-git-diff-view__loading-spinner" />
  ...
  if (error) {
    return (
      <div className={`ai00-x-git-diff-view ${className}`}>
        <div className="ai00-x-git-diff-view__error-state">
          <FileText size={48} />
  ...
  return (
    <div className={`ai00-x-git-diff-view ${className}`}>
      <div className="ai00-x-git-diff-view__header">
        <div className="ai00-x-git-diff-view__header-left">
          {sourceCommit && targetCommit && (
            <span className="ai00-x-git-diff-view__commit-range">
              {sourceCommit.substring(0, 7)}...{targetCommit.substring(0, 7)}
            </span>
          )}
          {!sourceCommit && !targetCommit && (
            <div className="ai00-x-git-diff-view__diff-type-switcher">
              <button 
                className={`ai00-x-git-diff-view__type-btn ${!currentShowStaged ? 'ai00-x-git-diff-view__type-btn--active' : ''}`}
          ...
          {loading && (
            <span className="ai00-x-git-diff-view__loading-indicator">
        ...
        <div className="ai00-x-git-diff-view__header-right">
          <div className="ai00-x-git-diff-view__view-options">
            <IconButton
      ...
      <div className="ai00-x-git-diff-view__content">
        {error ? (
          <div className="ai00-x-git-diff-view__error-state">
          ...
          <div className="ai00-x-git-diff-view__file-list">
            {diffFiles.map((file, index) => (
              <div key={file.path} className="ai00-x-git-diff-view__file-item">
                <div 
                  className="ai00-x-git-diff-view__file-header"
                  onClick={() => toggleFileExpansion(index)}
                >
                  <div className="ai00-x-git-diff-view__file-info">
                    <span className={`ai00-x-git-diff-view__expand-icon ${file.expanded ? 'ai00-x-git-diff-view__expand-icon--expanded' : ''}`}>
                      {file.expanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    </span>
                    
                    <span className="ai00-x-git-diff-view__file-status-icon">
                      {getFileStatusIcon(file.status)}
                    </span>
                    
                    <span className="ai00-x-git-diff-view__file-path">{file.path}</span>
                    
                    {file.oldPath && file.oldPath !== file.path && (
                      <span className={`ai00-x-git-diff-view__file-status ai00-x-git-diff-view__file-status--${file.status}`}>
                        ← {file.oldPath}
                      </span>
                    )}
                  </div>
                  
                  <div className="ai00-x-git-diff-view__file-stats">
                    {file.additions > 0 && (
                      <span className="ai00-x-git-diff-view__additions">
                        <Plus size={12} />
                        {file.additions}
                      </span>
                    )}
                    {file.deletions > 0 && (
                      <span className="ai00-x-git-diff-view__deletions">
                        <Minus size={12} />
                        {file.deletions}
                      </span>
                    )}
                ...
                {file.expanded && (
                  <div className="ai00-x-git-diff-view__diff-content">
          ...
          <div className="ai00-x-git-diff-view__empty-state">
            <FileText size={48} />
