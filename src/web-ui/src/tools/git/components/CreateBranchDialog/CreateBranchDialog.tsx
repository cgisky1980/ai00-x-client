...
export const CreateBranchDialog: React.FC<CreateBranchDialogProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleCancel} title={t('dialog.createNewBranch.title')} size="small">
      <div className="ai00-x-create-branch-dialog" onKeyDown={handleKeyDown}>
        <div className="ai00-x-create-branch-dialog__base-info">
          <div className="ai00-x-create-branch-dialog__base-label">
            <GitBranch size={14} />
            <span>{t('dialog.createNewBranch.baseBranch')}</span>
          </div>
          <div className="ai00-x-create-branch-dialog__base-value">
            {baseBranch}
          </div>
        </div>

        <div className="ai00-x-create-branch-dialog__form">
          <Input
            ...
            errorMessage={error}
          />
          <div className="ai00-x-create-branch-dialog__hint">
        ...
        <div className="ai00-x-create-branch-dialog__actions">
          <Button 
            variant="secondary"
            size="small"
        ...
        <div className="ai00-x-create-branch-dialog__shortcuts">
          <span>{t('dialog.createNewBranch.escKey')}</span> {t('dialog.createNewBranch.cancel')} · <span>{t('dialog.createNewBranch.enterKey')}</span> {t('dialog.createNewBranch.confirm')}
        </div>
      </div>
    </Modal>
  );
