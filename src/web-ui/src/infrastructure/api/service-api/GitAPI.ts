...
export class GitAPI {
  ...
  async mergeBranch(repositoryPath: string, branch: string, targetBranch?: string): Promise<GitOperationResult> {
    try {
      return await api.invoke('git_merge_branch', { 
        request: { repositoryPath, branch, targetBranch } 
      });
    } catch (error) {
      throw createTauriCommandError('git_merge_branch', error, { repositoryPath, branch, targetBranch });
    }
  }

  async hasConflicts(repositoryPath: string): Promise<boolean> {
    try {
      return await api.invoke('git_has_conflicts', { 
        request: { repositoryPath } 
      });
    } catch (error) {
      throw createTauriCommandError('git_has_conflicts', error, { repositoryPath });
    }
  }

  async abortMerge(repositoryPath: string): Promise<GitOperationResult> {
    try {
      return await api.invoke('git_abort_merge', { 
        request: { repositoryPath } 
      });
    } catch (error) {
      throw createTauriCommandError('git_abort_merge', error, { repositoryPath });
    }
  }
}
