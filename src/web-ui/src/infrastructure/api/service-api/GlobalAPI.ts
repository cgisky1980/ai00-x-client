...
export interface OpenRemoteWorkspaceRequest {
  ...
  connectionId: string;
  connectionName: string;
  /** Passed through to Rust so session files map to ~/.ai00-x/remote_ssh/{host}/... before/during connect. */
  sshHost?: string;
}
...
export class GlobalAPI {
  ...
  async isTaskWorkspace(path: string): Promise<boolean> {
    try {
      const result = await api.invoke('is_task_workspace', {
        request: { path },
      });
      return (result as { isTask: boolean }).isTask;
    } catch (error) {
      throw createTauriCommandError('is_task_workspace', error, { path });
    }
  }

  async getTaskWorkspacePath(): Promise<string> {
    try {
      const result = await api.invoke('get_task_workspace_path', {});
      return (result as { path: string }).path;
    } catch (error) {
      throw createTauriCommandError('get_task_workspace_path', error);
    }
  }

  async getCodeWorkspacePath(): Promise<string> {
    try {
      const result = await api.invoke('get_code_workspace_path', {});
      return (result as { path: string }).path;
    } catch (error) {
      throw createTauriCommandError('get_code_workspace_path', error);
    }
  }
