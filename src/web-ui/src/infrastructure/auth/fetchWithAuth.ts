/**
 * web-ui fetchWithAuth:统一注入 Bearer token + 401 自动刷新 + 重试一次
 *
 * 与 loader-ui 版差异:
 * - baseUrl 通过 tokenManager.getBaseUrl() 异步获取(从 ConfigManager 读)
 ...
 * 5. 重试仍 401 → 抛 AuthError
 *
 * @param url 相对路径(如 '/ai00-s/api/ai/me' 或 '/api/v1/auth/member/login')
 */
...
async function doFetch<T>(url: string, init: RequestInit): Promise<FetchResult<T>> {
  ...
  if (bodyIsString && !hasContentType && headers) {
