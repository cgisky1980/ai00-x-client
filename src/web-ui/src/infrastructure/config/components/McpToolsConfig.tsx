...
const McpToolsConfig: React.FC = () => {
  ...
  const renderServerBadge = (server: MCPServerInfo) => (
    <span className={`ai00-x-mcp-tools__status-badge ${getStatusClass(server.status)}`}>
  ...
  const renderServerDetails = (server: MCPServerInfo) => {
    const isConnected = server.status === 'Connected' || server.status === 'Healthy';
    return (
      <div className="ai00-x-mcp-tools__server-details">
        <div className="ai00-x-mcp-tools__server-detail-item">
          <span className="ai00-x-mcp-tools__server-detail-label">
            {tMcp('server.transport')}:
          </span>
          <code className="ai00-x-mcp-tools__server-detail-value">{server.transport}</code>
        </div>
        {isConnected && (
          <div className="ai00-x-mcp-tools__server-detail-item ai00-x-mcp-tools__skill-hint">
            <span className="ai00-x-mcp-tools__server-detail-label">
              Skill:
            </span>
            <span className="ai00-x-mcp-tools__server-detail-value">
              mcp--{server.id}
            </span>
          </div>
        )}
        {isConnected && (
          <div className="ai00-x-mcp-tools__server-detail-item ai00-x-mcp-tools__activation-hint">
            <span className="ai00-x-mcp-tools__activation-hint-text">
              {tMcp('skillInfo.activationHint')}
            </span>
          </div>
        )}
        ...
        {isRemoteServer(server) && (
          <>
            <div className="ai00-x-mcp-tools__server-detail-item">
              ...
              <code className="ai00-x-mcp-tools__server-detail-value">
                {server.url || '-'}
              </code>
            </div>
            <div className="ai00-x-mcp-tools__server-detail-item">
  ...
  return (
    <ConfigPageLayout className="ai00-x-mcp-tools">
      ...
      <ConfigPageContent>
        ...
        >
          {showJsonEditor && (
            <div className="ai00-x-mcp-tools__json-editor">
              <div className="ai00-x-mcp-tools__json-editor-header">
                <h3>{tMcp('jsonEditor.title')}</h3>
                <p className="ai00-x-mcp-tools__json-hint">{tMcp('jsonEditor.hint1')}</p>
                <p className="ai00-x-mcp-tools__json-hint">{tMcp('jsonEditor.hint2')}</p>
              </div>
              <Textarea
                ref={jsonEditorRef}
                value={jsonConfig}
                ...
                placeholder={`{\n  "mcpServers": {\n    "server-name": {\n      "command": "npx",\n      "args": ["-y", "@package/name"],\n      "env": {}\n    }\n  }\n}`}
                variant="outlined"
                className="ai00-x-mcp-tools__json-textarea"
                spellCheck={false}
                error={!!jsonLintError}
                errorMessage={
                  jsonLintError
              ...
              />
              <div className="ai00-x-mcp-tools__json-actions">
              ...
              <div className="ai00-x-mcp-tools__json-examples">
                <h4>{tMcp('jsonEditor.exampleTitle')}</h4>
                <div className="ai00-x-mcp-tools__example">
        ...
        {authDialogServer && (
          <div className="ai00-x-mcp-tools__json-editor">
            {authDialogServer.oauthEnabled && (
              <>
                <p className="ai00-x-mcp-tools__json-hint">
