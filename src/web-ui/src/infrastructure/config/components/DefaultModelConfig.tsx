...
export const DefaultModelConfig: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      void invoke<{ plan_tier?: string | null } | null>('get_auth_info').then(async (authInfo) => {
    ...
    return (
      <div className="default-model-config__model-option">
        <div className="default-model-config__model-option-title">
          ...
          {isAi00sModel(String(modelOption.value)) && (
            <span className={`default-model-config__tier-badge default-model-config__tier-badge--${getAi00sTier(String(modelOption.value), userTier)}`}>
              {getAi00sTier(String(modelOption.value), userTier)}
            </span>
          )}
