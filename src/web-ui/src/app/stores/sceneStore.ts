  ...
  openScene: (id) => {
    ...
    if (id !== WELCOME_SCENE_ID) {
      ...
      if (state.openTabs.some(t => t.id === WELCOME_SCENE_ID)) {
        ...
        set({
          openTabs:   ensureAgentFirst(tabsWithoutWelcome),
...
// ── Sync right-side scene → left-side nav ─────────────────────────────────
{
  ...
  useSceneStore.subscribe((state) => {
    if (state.activeTabId !== prev) {
      ...
      // In wallpaper mode, always show WallpaperNav
      if (activeMode === 'wallpaper') {
        navStore.openNavScene('wallpaper');
        return;
      }

      // In task mode, use MainNav with filtered workspace list (not a separate TaskNav)
      if (activeMode === 'task') {
        navStore.closeNavScene();
        return;
      }

      // In code mode, use the normal resolve logic
