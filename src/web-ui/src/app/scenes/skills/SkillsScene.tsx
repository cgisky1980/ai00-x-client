...
const SkillsScene: React.FC = () => {
  ...
  return (
    <div className="ai00-x-skills-scene">
                ...
                {market.marketSkills.map((skill, index) => {
                  ...
                  return (
                    <SkillCard
                      ...
                      meta={(
                        <span className="ai00-x-skills-scene__market-meta">
                          <TrendingUp size={12} />
                          {skill.installs ?? 0}
                        </span>
                      )}
      ...
      >
        {selectedInstalledSkill ? (
          <div className="ai00-x-skills-scene__detail-row">
            <span className="ai00-x-skills-scene__detail-label">{t('list.item.pathLabel')}</span>
            {canRevealSkillPath ? (
              <button
                type="button"
                className="ai00-x-skills-scene__detail-path-btn"
              ...
              <code className="ai00-x-skills-scene__detail-value">{selectedInstalledSkill.path}</code>
            )}
          </div>
        ) : null}
        ...
        {selectedMarketSkill?.source ? (
          <div className="ai00-x-skills-scene__detail-row">
            <span className="ai00-x-skills-scene__detail-label">{t('market.item.sourceLabel')}</span>
            <span className="ai00-x-skills-scene__detail-value">{selectedMarketSkill.source}</span>
          </div>
        ) : null}

        {selectedMarketSkill ? (
          <div className="ai00-x-skills-scene__detail-row">
            <span className="ai00-x-skills-scene__detail-label">{t('market.detail.installsLabel')}</span>
            <span className="ai00-x-skills-scene__detail-value">{selectedMarketSkill.installs ?? 0}</span>
          </div>
        ) : null}

        {selectedMarketSkill?.url ? (
          <div className="ai00-x-skills-scene__detail-row">
            <span className="ai00-x-skills-scene__detail-label">{t('market.detail.linkLabel')}</span>
            <a
              href={selectedMarketSkill.url}
              target="_blank"
              rel="noreferrer"
              className="ai00-x-skills-scene__detail-link"
            >
              {selectedMarketSkill.url}
            </a>
          </div>
      ...
      <Modal
        ...
        size="small"
      >
        <div className="ai00-x-skills-scene__modal-form">
          <Select
          ...
          {installed.formLevel === 'project' && installed.hasWorkspace ? (
            <div className="ai00-x-skills-scene__form-hint">
              {t('form.level.currentWorkspace', { path: installed.workspacePath })}
            </div>
          ) : null}

          <div className="ai00-x-skills-scene__path-input">
            <Input
          ...
          <div className="ai00-x-skills-scene__path-hint">
            {t('form.path.hint')}
          </div>

          {installed.isValidating ? (
            <div className="ai00-x-skills-scene__validating">{t('form.validating')}</div>
          ) : null}

          {installed.validationResult ? (
            <div
              className={[
                'ai00-x-skills-scene__validation',
                installed.validationResult.valid ? 'is-valid' : 'is-invalid',
              ].filter(Boolean).join(' ')}
            >
              {installed.validationResult.valid ? (
                <>
                  <div className="ai00-x-skills-scene__validation-name">
                    {installed.validationResult.name}
                  </div>
                  <div className="ai00-x-skills-scene__validation-desc">
                    {installed.validationResult.description}
                  </div>
                </>
              ) : (
                <div className="ai00-x-skills-scene__validation-error">
          ...
          <div className="ai00-x-skills-scene__modal-form-actions">
            <Button
              variant="secondary"
              size="small"
              onClick={() => {
