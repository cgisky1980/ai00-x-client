 ...
 *  6. Exposes `window.__ai00-x_inspector_cancel` so a second eval call can
 *     programmatically cancel an active session.
 */

const INSPECTOR_SCRIPT_BODY = /* js */ `
(function () {
  if (window.__ai00-x_inspector_active) {
    window.__ai00-x_inspector_cancel && window.__ai00-x_inspector_cancel();
    return;
  }
  window.__ai00-x_inspector_active = true;

  var LABEL = window.__ai00-x_inspector_label || '';
  ...
  function cleanup() {
    ...
    delete window.__ai00-x_inspector_active;
    delete window.__ai00-x_inspector_cancel;
    delete window.__ai00-x_inspector_label;
  }
  ...
  window.__ai00-x_inspector_cancel = function () {
    emitTauri(EVENT_CANCELLED, null);
    cleanup();
  };
...
export function createInspectorScript(webviewLabel: string): string {
  ...
  return `window.__ai00-x_inspector_label = ${JSON.stringify(webviewLabel)};\n${INSPECTOR_SCRIPT_BODY}`;
}

/** Script to cancel an active inspector session without triggering a selection. */
export const CANCEL_INSPECTOR_SCRIPT =
  `if (window.__ai00-x_inspector_cancel) { window.__ai00-x_inspector_cancel(); }`;
...
export const BLANK_TARGET_INTERCEPT_SCRIPT = `(function(){
  if(window.__ai00-x_blank_intercept){return;}
  window.__ai00-x_blank_intercept=true;
