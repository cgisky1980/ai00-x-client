...
function highlightFirstMatch(text: string, query: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="ai00-x-settings-nav__search-highlight">
...
const SettingsNav: React.FC = () => {
  ...
  return (
    <div className="ai00-x-settings-nav">
      <div className="ai00-x-settings-nav__search">
        <Search
          ref={searchInputRef}
          className="ai00-x-settings-nav__search-field"
          size="small"
          value={draftQuery}
      ...
      <div
        ...
        className="ai00-x-settings-nav__sections"
      ...
      >
        {isSearchMode ? (
          <>
            {results.length === 0 ? (
              <div className="ai00-x-settings-nav__search-empty" role="status">
                {t('configCenter.searchNoResults')}
              </div>
            ) : (
              <div className="ai00-x-settings-nav__search-results">
                {results.map((row, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        'ai00-x-settings-nav__search-result-item',
                        active && 'is-active',
                    ...
                    >
                      <span className="ai00-x-settings-nav__search-result-line">
                        {highlightFirstMatch(line, displayQuery)}
                      </span>
                      {row.description ? (
                        <span className="ai00-x-settings-nav__search-result-desc">
          ...
          SETTINGS_CATEGORIES.map((category) => (
            <div key={category.id} className="ai00-x-settings-nav__category">
              <div className="ai00-x-settings-nav__category-header">
                <span className="ai00-x-settings-nav__category-label">
                  {t(category.nameKey, { defaultValue: category.id })}
                </span>
              </div>

              <div className="ai00-x-settings-nav__items">
                {category.tabs.map((tabDef) => {
                  ...
                  return (
                    ...
                    >
                      {Icon && <Icon size={15} className="ai00-x-settings-nav__item-icon" />}
                      <span className="ai00-x-settings-nav__item-label">
                        {t(tabDef.labelKey, { defaultValue: tabDef.id })}
                      </span>
                      {tabDef.beta ? (
                        <Badge variant="warning" className="ai00-x-settings-nav__item-beta">
                          {t('configCenter.beta')}
                        </Badge>
                      ) : null}
                    </button>
