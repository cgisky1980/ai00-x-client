...
export function useShellEntries(overrideWorkspacePath?: string): UseShellEntriesReturn {
  const { workspacePath: contextWorkspacePath, workspace } = useCurrentWorkspace();
  const workspacePath = overrideWorkspacePath ?? contextWorkspacePath;
