...
import { Button } from '@ai00-x/design-system/web';
...
const ShellNav: React.FC<ShellNavProps> = ({ workspacePath }) => {
  ...
  return (
    <div className="ai00-x-shell-nav">
      <div className="ai00-x-shell-nav__header">
        <div className="ai00-x-shell-nav__title-group">
          <span className="ai00-x-shell-nav__title">{t('nav.shell.title')}</span>
        ...
        <div className="ai00-x-shell-nav__header-actions" ref={menuRef}>
          <div className={`ai00-x-shell-nav__split-button${menuOpen ? ' is-active' : ''}`}>
            <Tooltip content={t('nav.shell.actions.newTerminal')} placement="bottom">
              <button
                type="button"
                className="ai00-x-shell-nav__split-button-main"
            ...
            <Tooltip content={t('actions.more')} placement="bottom">
              <button
                type="button"
                className="ai00-x-shell-nav__split-button-toggle"
          ...
          {menuOpen ? (
            <div className="ai00-x-shell-nav__dropdown-menu" role="menu">
              {shellMenuItems.map((shell) => (
                <button
                  key={shell.key}
                  type="button"
                  className="ai00-x-shell-nav__dropdown-item"
                  role="menuitem"
              ...
              {shellMenuItems.length > 0 ? <div className="ai00-x-shell-nav__dropdown-separator" /> : null}
              <button type="button" className="ai00-x-shell-nav__dropdown-item" role="menuitem" onClick={() => { setMenuOpen(false); void handleRefresh(); }}>
                <RefreshCw size={14} />
      ...
      <div className="ai00-x-shell-nav__view-toggle" role="tablist" aria-label={t('nav.shell.title')} style={{ display: 'none' }}>
        <button
          type="button"
          role="tab"
          className={`ai00-x-shell-nav__view-toggle-btn${navView === 'manual' ? ' is-active' : ''}`}
        ...
        <button
          type="button"
          role="tab"
          className={`ai00-x-shell-nav__view-toggle-btn${navView === 'agent' ? ' is-active' : ''}`}
      ...
      <div
        className={`ai00-x-shell-nav__sections${!hasVisibleContent ? ' ai00-x-shell-nav__sections--empty' : ''}`}
      >
        {hasVisibleContent ? (
          <div className="ai00-x-shell-nav__terminal-list">
          ...
          <div className="ai00-x-shell-nav__empty">
            <p className="ai00-x-shell-nav__empty-message">
