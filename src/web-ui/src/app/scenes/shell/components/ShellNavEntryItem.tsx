...
const ShellNavEntryItem: React.FC<ShellNavEntryItemProps> = ({
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={[
        'ai00-x-shell-nav__terminal-item',
        isActive && 'is-active',
      ].filter(Boolean).join(' ')}
    ...
    >
      <Tooltip content={entry.name} placement="right">
        <span className="ai00-x-shell-nav__terminal-item-main">
          {showSavedBadge ? (
            <Bookmark size={14} className="ai00-x-shell-nav__terminal-icon ai00-x-shell-nav__terminal-icon--saved" />
          ) : (
            <SquareTerminal size={14} className="ai00-x-shell-nav__terminal-icon" />
          )}

          <span className="ai00-x-shell-nav__terminal-label">{entry.name}</span>

          {showSavedBadge ? (
            <span className="ai00-x-shell-nav__saved-indicator">{savedBadgeLabel}</span>
          ) : null}

          {entry.startupCommand ? (
            <span className="ai00-x-shell-nav__cmd-indicator">{startupCommandBadgeLabel}</span>
          ) : null}

          <span className={`ai00-x-shell-nav__terminal-dot${entry.isRunning ? ' is-running' : ' is-stopped'}`} />
        </span>
      </Tooltip>
      ...
      <Tooltip content={quickAction.title} placement="right">
        <button
          type="button"
          className="ai00-x-shell-nav__terminal-close"
          onClick={(event) => {
