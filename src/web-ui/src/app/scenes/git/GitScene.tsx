...
const GitScene: React.FC<GitSceneProps> = ({
  ...
  if (!isActive) {
    return <div className="ai00-x-git-scene" aria-hidden="true" />;
  }

  if (!repoLoading && !isRepository) {
    return (
      <div className="ai00-x-git-scene ai00-x-git-scene--not-repository">
        <div className="ai00-x-git-scene__content">
          <div className="ai00-x-git-scene__init-container">
            <div className="ai00-x-git-scene__init-decoration">
              <div className="ai00-x-git-scene__init-line ai00-x-git-scene__init-line--dashed" />
              <div className="ai00-x-git-scene__init-dot" />
              <div className="ai00-x-git-scene__init-line ai00-x-git-scene__init-line--solid" />
            </div>
            <div className="ai00-x-git-scene__init-card">
              <div className="ai00-x-git-scene__init-icon">
                <GitBranch size={24} />
              </div>
              <div className="ai00-x-git-scene__init-text">
                <h3>{t('init.title')}</h3>
                <p>{t('init.notRepository')}</p>
              </div>
              <button type="button" className="ai00-x-git-scene__init-button" onClick={handleInitGitRepository}>
                <Plus size={14} />
            ...
            <div className="ai00-x-git-scene__init-decoration">
              <div className="ai00-x-git-scene__init-line ai00-x-git-scene__init-line--solid" />
              <div className="ai00-x-git-scene__init-dot ai00-x-git-scene__init-dot--muted" />
              <div className="ai00-x-git-scene__init-line ai00-x-git-scene__init-line--dashed" />
            </div>
            <div className="ai00-x-git-scene__init-hint">
  ...
  if ((repoLoading || statusLoading) && !forceReset) {
    return (
      <div className="ai00-x-git-scene ai00-x-git-scene--loading">
        <div className="ai00-x-git-scene__content">
          <div className="ai00-x-git-scene__loading-actions">
          ...
          <div className="ai00-x-git-scene__loading-state">
            <CubeLoading size="medium" text={t('loading.text')} />
            <p className="ai00-x-git-scene__loading-hint">{t('loading.hint')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return <div className="ai00-x-git-scene">{renderView()}</div>;
};
