...
export function useAgentsList({
  ...
  const getModeSkills = useCallback((_agentId: string): any[] => {
    return [];
  }, []);

  const handleSetSkills = useCallback(async (_agentId: string, _enabledSkillKeys: string[]) => {
    // Skills are now globally available, no per-mode override needed
  }, []);
  ...
  const handleSetTools = useCallback(async (agentId: string, toolNames: string[]) => {
    try {
      ...
      notification.error(t('agentsOverview.toolToggleFailed', 'Tool toggle failed'));
    }
  ...
  const handleResetTools = useCallback(async (agentId: string) => {
    try {
      ...
      notification.success(t('agentsOverview.toolsResetSuccess', 'Reset to default tools'));
      ...
      notification.error(t('agentsOverview.toolToggleFailed', 'Reset failed'));
    }
  }, [notification, t]);
