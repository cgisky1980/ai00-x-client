...
import { TOP_LEVEL_AGENT_IDS, isAgentInOverviewZone } from './agentVisibility';
...
const AgentsHomeView: React.FC = () => {
  ...
  const coreAgents = useMemo(() => allAgents.filter((agent) => TOP_LEVEL_AGENT_IDS.has(agent.id)), [allAgents]);
  ...
  return (
    <GalleryLayout className="ai00-x-agents-scene">
      <GalleryPageHeader
        title={t('page.title')}
      ...
      <div className="gallery-zones">
        ...
        <GalleryZone
          ...
          tools={(
            <>
              <div className="ai00-x-agents-scene__agent-filters">
                <div className="ai00-x-agents-scene__agent-filter-group">
                  <span className="ai00-x-agents-scene__agent-filter-label">
                    {t('filters.source')}
                  </span>
            ...
            <div className="agent-card__cap-grid">
              {selectedAgent.capabilities.map((cap) => (
                <div key={cap.category} className="agent-card__cap-row">
                  ...
                  >
                    {t(`capabilityCategories.${cap.category}`, cap.category)}
                  </span>
...
const AgentsScene: React.FC = () => {
  ...
  if (page === 'createAgent') {
    return (
      <div className="ai00-x-agents-scene">
        <CreateAgentPage />
      </div>
    );
  }
