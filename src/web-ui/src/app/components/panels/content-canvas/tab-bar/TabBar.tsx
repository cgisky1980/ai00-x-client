...
export const TabBar: React.FC<TabBarProps> = ({
  ...
  const activeFilePath = activeTab?.content?.data?.filePath as string | undefined;

  const handleSaveActiveTab = useCallback(() => {
    if (activeTab?.isDirty && activeFilePath) {
      globalEventBus.emit('editor:save-file', { filePath: activeFilePath });
    }
  }, [activeTab, activeFilePath]);

  const handleRevertActiveTab = useCallback(() => {
    if (activeTab?.isDirty && activeFilePath) {
      globalEventBus.emit('editor:revert-file', { filePath: activeFilePath });
    }
  }, [activeTab, activeFilePath]);
  ...
  return (
    ...
    >
      ...
      <div ref={actionsRef} className="canvas-tab-bar__actions">
        {/* Save button — visible when active tab is dirty */}
        {activeTab?.isDirty && activeFilePath && (
          <Tooltip content={t('tabs.save', { defaultValue: 'Save' })} placement="bottom">
            <button
              className="canvas-tab-bar__action-btn canvas-tab-bar__action-btn--save"
              onClick={(e) => {
                e.stopPropagation();
                handleSaveActiveTab();
              }}
        ...
        {/* Revert button — visible when active tab is dirty */}
        {activeTab?.isDirty && activeFilePath && (
          <Tooltip content={t('tabs.revert', { defaultValue: 'Revert' })} placement="bottom">
            <button
              className="canvas-tab-bar__action-btn canvas-tab-bar__action-btn--revert"
              onClick={(e) => {
                e.stopPropagation();
                handleRevertActiveTab();
              }}
            >
              <RotateCcw size={14} />
            </button>
