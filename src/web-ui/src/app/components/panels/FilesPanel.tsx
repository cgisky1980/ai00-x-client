    ...
    if (data.scrollIntoView && finalExpandPath) {
      setTimeout(() => {
        ...
        if (targetElement) {
          ...
          targetElement.classList.add('ai00-x-file-explorer__node-content--highlighted');
          setTimeout(() => {
            targetElement.classList.remove('ai00-x-file-explorer__node-content--highlighted');
          }, 2000);
        }
      }, 300);
    }
  ...
  return (
    ...
    >
      {!hideHeader && (
        <PanelHeader
          title={t('title')}
          className="ai00-x-files-panel__header"
          actions={
            workspacePath && (
              <IconButton
                size="xs"
      ...
      <div className="ai00-x-files-panel__content">
        {workspacePath && viewMode === 'search' && (
          <div className="ai00-x-files-panel__search">
            <Search
              ...
              loading={isSearching}
            />
            <div className="ai00-x-files-panel__search-toolbar">
              <div className="ai00-x-files-panel__search-modes">
                <button
                  type="button"
                  className={`ai00-x-files-panel__search-mode ${searchMode === 'content' ? 'active' : ''}`}
                ...
                <button
                  type="button"
                  className={`ai00-x-files-panel__search-mode ${searchMode === 'filenames' ? 'active' : ''}`}
                  onClick={() => setSearchMode('filenames')}
                >
                  {t('search.modeFiles')}
                </button>
              </div>
              <div className="ai00-x-files-panel__search-options">
                <Tooltip content={t('options.caseSensitive')}>
                  <button
                    type="button"
                    className={`ai00-x-files-panel__search-option ${searchOptions.caseSensitive ? 'active' : ''}`}
                ...
                <Tooltip content={t('options.wholeWord')}>
                  <button
                    type="button"
                    className={`ai00-x-files-panel__search-option ${searchOptions.wholeWord ? 'active' : ''}`}
                ...
                <Tooltip content={t('options.useRegex')}>
                  <button
                    type="button"
                    className={`ai00-x-files-panel__search-option ${searchOptions.useRegex ? 'active' : ''}`}
        ...
        <div
          className={`ai00-x-files-panel__main-content${
            fileDropHighlight ? ' ai00-x-files-panel__main-content--drop-target' : ''
          }`}
        >
        {!workspacePath ? (
          <div className="ai00-x-files-panel__placeholder">
            <div className="ai00-x-files-panel__placeholder-icon">
          ...
          searchQuery ? (
            <div className="ai00-x-files-panel__search-content">
              {searchLimitNotice && (
                <div className="ai00-x-files-panel__search-limit-notice">
                  <span>{searchLimitNotice}</span>
                </div>
              )}
              
              {searchError && (
                <div className="ai00-x-files-panel__error">
                  <p>❌ {searchError}</p>
                  <button 
                    className="ai00-x-files-panel__retry-button"
              ...
              {searchResults.length > 0 ? (
                <FileSearchResults
                  ...
                  className="ai00-x-files-panel__search-results"
                />
              ) : (
          ...
          loading && fileTree.length === 0 ? (
            <div className="ai00-x-files-panel__loading">
            ...
            <FileExplorer
              ...
              className="ai00-x-files-panel__explorer"
              enablePathCompression={true}
      ...
      {transferProgress && (
        <div className="ai00-x-files-panel__transfer" role="status">
          <div className="ai00-x-files-panel__transfer-label">
          ...
          <div
            className={`ai00-x-files-panel__transfer-track${
              transferProgress.indeterminate ? ' ai00-x-files-panel__transfer-track--indeterminate' : ''
            }`}
          >
            <div
              className="ai00-x-files-panel__transfer-fill"
              style={
