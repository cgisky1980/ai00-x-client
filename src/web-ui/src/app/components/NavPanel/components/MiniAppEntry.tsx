...
const MiniAppEntry: React.FC<MiniAppEntryProps> = ({
  ...
  return (
    <div className="ai00-x-nav-panel__miniapp-entry-wrap">
      <div
        className={[
          'ai00-x-nav-panel__miniapp-entry',
          isActive && 'is-active',
      ...
      >
        <span className="ai00-x-nav-panel__miniapp-entry-main">
          <span className="ai00-x-nav-panel__miniapp-entry-copy">
            <span className="ai00-x-nav-panel__miniapp-entry-title">{t('scenes.miniApps')}</span>
            <Badge variant="neutral" className="ai00-x-nav-panel__miniapp-badge">Beta</Badge>
          </span>
        </span>

        <span className="ai00-x-nav-panel__miniapp-entry-apps">
          {visibleApps.length > 0 ? (
            <>
              {visibleApps.map((app) => {
                ...
                return (
                  <Tooltip key={app.id} content={app.name} placement="right">
                    <span
                      className={[
                        'ai00-x-nav-panel__miniapp-bubble',
              ...
              {overflowCount > 0 ? (
                <span className="ai00-x-nav-panel__miniapp-bubble ai00-x-nav-panel__miniapp-bubble--more">
                  +{overflowCount}
                </span>
              ) : null}
            </>
