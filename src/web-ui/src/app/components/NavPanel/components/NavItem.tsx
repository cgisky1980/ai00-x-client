...
const NavItem: React.FC<NavItemProps> = ({
  ...
  return (
    ...
    >
      <span className="ai00-x-nav-panel__item-icon" aria-hidden="true">
        <Icon size={15} />
      </span>
      <span className="ai00-x-nav-panel__item-label">{displayLabel}</span>

      {badge && (
        <span
          ref={badgeRef}
          className={`ai00-x-nav-panel__item-badge ${onBadgeClick ? 'ai00-x-nav-panel__item-badge--clickable' : ''}`}
      ...
      {renderActions ? (
        <span className="ai00-x-nav-panel__item-actions-custom" onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()}>
          {renderActions()}
        </span>
        ...
        actionTitle ? (
          <Tooltip content={actionTitle} placement="right" followCursor>
            <span
              className="ai00-x-nav-panel__item-action"
