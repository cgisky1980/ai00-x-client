...
const SceneTab: React.FC<SceneTabProps> = ({
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target.closest('.ai00-x-scene-tab__action')) return;
    e.preventDefault();
  }, [pinned]);
  ...
  return (
    <div
      ...
      className={[
        'ai00-x-scene-tab',
        isActive && 'ai00-x-scene-tab--active',
        pinned && 'ai00-x-scene-tab--pinned',
      ].filter(Boolean).join(' ')}
      onClick={handleClick}
    ...
    >
      ...
      <div className="ai00-x-scene-tab__content">
        {Icon && <Icon size={13} className="ai00-x-scene-tab__icon" aria-hidden="true" />}
        <span className="ai00-x-scene-tab__label">{label}</span>
        {subtitle && (
          <>
            <span className="ai00-x-scene-tab__sep" aria-hidden="true">/</span>
            <span className="ai00-x-scene-tab__subtitle">{subtitle}</span>
          </>
        )}
        {onActionClick && (
          <Tooltip content={actionTitle} placement="bottom" followCursor>
            <span
              className="ai00-x-scene-tab__action"
      ...
      {!pinned && (
        <button
          type="button"
          className="ai00-x-scene-tab__close"
