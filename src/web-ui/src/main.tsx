...
// Only Fira Code is bundled; UI text falls back to system fonts.
...
const WHITE_SCREEN_LOGGED_FLAG = '__ai00-x_white_screen_crash_logged__';
...
function registerGlobalErrorHandlers() {
  const flag = '__ai00-x_global_error_handlers_registered__';
...
async function initializeAfterRender(): Promise<void> {
  ...
  log.info('Ai00-X core systems initialized successfully');
}
...
async function startApplication(): Promise<void> {
  try {
    ...
    log.error('Failed to initialize Ai00-X (pre-render)', error);
  }
