...
/* 独立版：全部组件直出 @ai00-x/design-system 双出口；
   Markdown/CodeEditor 为 web-ui 应用组件（依赖 API client），不在演示站范围 */
import {
...
import { WindowControls } from '@ai00-x/design-system/web';
      ...
      },
      {
        ...
        component: () => (
          <div style={{ border: '1px dashed var(--border-subtle)', borderRadius: 8, width: '100%' }}>
            <Empty description="还没有会话——落笔即开始" action={<DsButton variant="seal">新建会话</DsButton>} />
          </div>
        ),
      },
      {
        ...
        component: () => (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <DsButton variant="destructive">删除工作区…</DsButton>
            </AlertDialogTrigger>
            <AlertDialogSimple
              title="删除工作区？"
...
name: 'Search - Demo',
  ...
  /* 内容组件分类（Markdown/CodeEditor 为 web-ui 应用组件，独立版不含）已移除 */
  {
    id: 'navigation',
    name: '导航组件',
    description: 'Demo',
