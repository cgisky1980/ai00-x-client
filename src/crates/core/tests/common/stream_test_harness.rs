...
use ai00_x_ai_adapters::stream::{
    handle_anthropic_stream, handle_gemini_stream, handle_openai_stream, handle_responses_stream,
    UnifiedResponse,
};
use ai00_x_core::agent::events::{AgentEvent, EventQueue, EventQueueConfig};
use ai00_x_core::agent::execution::{StreamProcessError, StreamResult};
use ai00_x_core::StreamProcessor;
...
async fn drain_all_events(event_queue: &Arc<EventQueue>) -> Vec<AgentEvent> {
