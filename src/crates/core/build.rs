...
fn embed_agents_prompt_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-changed=src/agent/agents/prompts");
    ...
    // Read agent agents prompts
    let agent_prompt_path = Path::new(&manifest_dir)
        .join("src")
        .join("agent")
        .join("agents")
        .join("prompts");
    ...
    if agent_prompt_path.exists() {
        println!("Embedding agent prompts from: {:?}", agent_prompt_path);
        read_prompts_recursive(&agent_prompt_path, &agent_prompt_path, &mut prompts)?;
    } else {
        eprintln!(
            "Warning: agent prompts directory not found at {:?}",
            agent_prompt_path
        );
    }
