...
pub struct SessionMetadata {
    ...
    /// Overall completion rating (0-5)
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        alias = "complete_rating"
    )]
    pub complete_rating: Option<u8>,

    /// Overall completion feedback text
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        alias = "complete_feedback"
    )]
    pub complete_feedback: Option<String>,

    /// When the rating was submitted (Unix timestamp seconds)
...
impl DialogTurnKind {
    pub fn is_model_visible(self) -> bool {
        matches!(self, Self::UserDialog | Self::SubagentTask)
    }
}
