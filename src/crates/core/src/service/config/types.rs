...
/// VRM persona configuration (legacy, kept for backward-compatible deserialization).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct VrmPersonaConfig {
    pub name: String,
    pub gender: String,
    pub identity: String,
    ...
    pub selected_personality: String,
    #[serde(default)]
    pub optional_tics: Vec<String>,
    pub language: String,
}

impl Default for VrmPersonaConfig {
    fn default() -> Self {
        Self {
            name: "叶良辰".to_string(),
            gender: "女".to_string(),
            identity: "猫族".to_string(),
            personality: "傲娇".to_string(),
            user_title: "主人".to_string(),
            selected_identity: "猫族".to_string(),
            selected_personality: "傲娇".to_string(),
            optional_tics: Vec::new(),
            language: "zh".to_string(),
        }
    }
}

/// VRM display configuration (legacy, kept for backward-compatible deserialization).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct VrmDisplayConfig {
    pub visible: bool,
    pub scale: f64,
    pub head_scale: f64,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for VrmDisplayConfig {
    fn default() -> Self {
        Self {
            visible: true,
            scale: 1.0,
...
pub struct VoiceInputConfig {
    pub enabled: bool,
    pub trigger_duration_secs: f32,
    pub charge_show_delay_secs: f32,
...
/// Persona profile for the personas list (legacy, kept for backward-compatible deserialization).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct PersonaProfile {
    ...
    pub optional_tics: Vec<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tts_speaker_id: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for PersonaProfile {
    fn default() -> Self {
        Self {
            ...
            gender: String::new(),
            language: "zh".to_string(),
            selected_identity: String::new(),
            selected_personality: String::new(),
...
impl From<PersonaProfile> for Assistant {
    fn from(p: PersonaProfile) -> Self {
        Assistant {
            ...
            persona: AssistantPersona {
                gender: p.gender,
                identity: p.selected_identity.clone(),
                personality: p.selected_personality.clone(),
                user_title: p.user_title,
                optional_tics: p.optional_tics,
                language: p.language,
                custom_prompt: None,
            },
            appearance: AssistantAppearance {
                vrm_model: p.vrm_model,
                scale: 1.0,
                head_scale: 1.0,
            },
            voice: AssistantVoice {
                tts_speaker_id: p.tts_speaker_id,
                voice_style: None,
            },
            session: AssistantSessionInfo { session_id: None },
            metadata: p.metadata,
            created_at: None,
            updated_at: None,
        }
...
/// Assistant persona configuration (name, personality, identity, etc.).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
...
/// Assistant appearance configuration (VRM model, scale, etc.).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct AssistantAppearance {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub vrm_model: Option<String>,
    pub scale: f64,
    pub head_scale: f64,
...
impl Default for AssistantAppearance {
    fn default() -> Self {
        Self {
            vrm_model: None,
            scale: 1.0,
...
/// Assistant voice configuration (TTS speaker, style, etc.).
...
/// Assistant session information (RWKV session binding).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
...
/// Assistant - a complete VRM assistant with persona, appearance, voice, and session.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct Assistant {
    ...
    pub appearance: AssistantAppearance,
    pub voice: AssistantVoice,
    pub session: AssistantSessionInfo,
    #[serde(default)]
    pub metadata: serde_json::Value,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for Assistant {
    fn default() -> Self {
        Self {
            ...
            appearance: AssistantAppearance::default(),
            voice: AssistantVoice::default(),
            session: AssistantSessionInfo::default(),
            metadata: serde_json::Value::Object(serde_json::Map::new()),
            created_at: None,
            updated_at: None,
        }
...
impl Assistant {
    pub fn ensure_session_id(&mut self) -> &str {
        if self.session.session_id.is_none() {
            self.session.session_id = Some(format!("vrm-{}", self.id));
        }
        self.session.session_id.as_deref().unwrap_or("")
    }
}

/// Global display configuration (VRM visibility, position - not bound to any assistant).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
...
impl Default for GlobalDisplayConfig {
    fn default() -> Self {
        Self {
            visible: true,
            position_x: 0.85,
...
/// Gesture configuration types (grid pattern mode).
...
pub struct GestureConfig {
    ...
    pub dot_radius: f64,
    #[serde(default)]
    pub templates: Vec<GestureTemplateConfig>,
    #[serde(default)]
    pub bindings: Vec<GestureBinding>,
    #[serde(default)]
    pub saved_actions: Vec<SavedAction>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GestureTemplateConfig {
    pub name: String,
...
/// VRM settings configuration (assistants, display, voice input).
/// Underlay desktop replacement configuration.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct UnderlayConfig {
    /// Whether desktop replacement mode is enabled.
    pub enabled: bool,
    /// Desktop mode: "default" (restore original desktop) or "replace" (use underlay).
    pub mode: String,
    /// Grid columns for desktop icon layout.
    pub grid_columns: u32,
    /// Grid rows for desktop icon layout.
    pub grid_rows: u32,
}
...
pub struct P2pConfig {
    /// P2P 功能主开关（默认 true，仅当明确置 false 时禁用）。
    pub enabled: bool,
...
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct VrmSettingsConfig {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub active_assistant_id: Option<String>,
    #[serde(default)]
    pub assistants: Vec<Assistant>,
    pub display: GlobalDisplayConfig,
    pub voice_input: VoiceInputConfig,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub persona: Option<VrmPersonaConfig>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub vrm: Option<VrmDisplayConfig>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tts_voice_id: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub active_persona_id: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub personas: Option<Vec<PersonaProfile>>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gesture: Option<GestureConfig>,
}

impl VrmSettingsConfig {
    pub fn migrate_from_legacy(&mut self) -> bool {
        ...
        let legacy_personas = self.personas.take().unwrap_or_default();
        let legacy_persona = self.persona.take();
        let legacy_vrm = self.vrm.take();
        let legacy_tts_voice_id = self.tts_voice_id.take();
        let legacy_active_id = self.active_persona_id.take();

        if legacy_personas.is_empty() && legacy_persona.is_none() {
            let default_assistant = Assistant::default();
            self.assistants.push(default_assistant);
            self.active_assistant_id = Some(self.assistants[0].id.clone());
            return true;
        }

        for p in legacy_personas {
            let mut assistant: Assistant = p.into();
            assistant.ensure_session_id().to_string();
            self.assistants.push(assistant);
            migrated = true;
        }

        if let Some(ref active_id) = legacy_active_id {
            self.active_assistant_id = Some(active_id.clone());
        } else if !self.assistants.is_empty() {
            self.active_assistant_id = Some(self.assistants[0].id.clone());
        }

        if let Some(vrm_cfg) = legacy_vrm {
            self.display.visible = vrm_cfg.visible;
            if let Some(ref active_id) = self.active_assistant_id {
                if let Some(assistant) = self.assistants.iter_mut().find(|a| a.id == *active_id) {
                    assistant.appearance.scale = vrm_cfg.scale;
                    assistant.appearance.head_scale = vrm_cfg.head_scale;
                    if vrm_cfg.model_file.is_some() && assistant.appearance.vrm_model.is_none() {
                        assistant.appearance.vrm_model = vrm_cfg.model_file;
                    }
                }
            }
            migrated = true;
        ...
        if let Some(tts_id) = legacy_tts_voice_id {
            if let Some(ref active_id) = self.active_assistant_id {
                if let Some(assistant) = self.assistants.iter_mut().find(|a| a.id == *active_id) {
                    if assistant.voice.tts_speaker_id.is_none() {
                        assistant.voice.tts_speaker_id = Some(tts_id);
                    }
                }
            }
            migrated = true;
        ...
        if let Some(persona_cfg) = legacy_persona {
            if let Some(ref active_id) = self.active_assistant_id {
                if let Some(assistant) = self.assistants.iter_mut().find(|a| a.id == *active_id) {
                    if assistant.name.is_empty() || assistant.name == "Assistant" {
                        assistant.name = persona_cfg.name;
                    }
                }
            }
            migrated = true;
...
pub struct GlobalConfig {
    ...
    /// VRM persona/display/TTS settings.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub vrm: Option<VrmSettingsConfig>,
    /// Voice input settings (migrated from vrm.voice_input).
    #[serde(default)]
    pub voice_input: VoiceInputConfig,
    /// Gesture settings (migrated from vrm.gesture).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gesture: Option<GestureConfig>,
    /// Underlay desktop replacement settings.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct AppSessionConfig {
    /// Default new session agent type used by the frontend.
    /// Supported values: "Core", "Plan".
...
pub struct WorkspaceConfig {
    ...
    /// Scratch workspace path. Always exists, cleared on every startup. When None, defaults to `<default_workspace_parent_dir>/scratch`.
    pub default_workspace: Option<String>,
    /// Parent directory for workspace directories. When None, defaults to `<exe_dir>/workspaces`.
    pub default_workspace_parent_dir: Option<String>,
}
...
pub use ai00_x_ai_adapters::types::ReasoningMode;
...
pub struct AIConfig {
    ...
    /// Optional AnySearch API key. When set, sent as `Authorization: Bearer
    /// <key>` to raise rate limits from 10/min (anonymous) to 20/min. When
    /// `None` (default), anonymous access is used.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct ModeConfig {
    /// Mode ID (e.g. Core, debug, requirement, ui-design).
    pub mode_id: String,
    ...
    /// If true, all MCP tools are injected eagerly (old behavior).
    /// Default: false (progressive tool disclosure via Skill).
    #[serde(default)]
    pub mcp_eager_load: bool,
}
...
pub struct DebugModeConfig {
    /// Custom log path (relative to the workspace; default: `.ai00-x/debug.log`).
    pub log_path: String,
...
impl Default for DebugModeConfig {
    fn default() -> Self {
        Self {
            log_path: ".ai00-x/debug.log".to_string(),
            ingest_port: 7242,
...
pub use ai00_x_ai_adapters::types::ProxyConfig;
...
pub trait ConfigProvider: Send + Sync {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> Ai00XResult<Vec<String>>;
...
impl GlobalConfig {
    /// Migrates `voice_input` and `gesture` from the legacy `vrm` sub-object to
    /// top-level fields. Called after config deserialization. Returns `true` if
    /// migration occurred (caller should persist the updated config).
    pub fn migrate_vrm_to_standalone(&mut self) -> bool {
        let mut migrated = false;

        if let Some(vrm) = &mut self.vrm {
            // Migrate voice_input if top-level is still default.
            let default_voice = VoiceInputConfig::default();
            if self.voice_input.enabled == default_voice.enabled
                && self.voice_input.trigger_duration_secs == default_voice.trigger_duration_secs
                && self.voice_input.charge_show_delay_secs == default_voice.charge_show_delay_secs
                && self.voice_input.input_device_id == default_voice.input_device_id
                && self.voice_input.output_device_id == default_voice.output_device_id
            {
                self.voice_input = vrm.voice_input.clone();
                migrated = true;
            }

            // Migrate gesture if top-level is None.
            if self.gesture.is_none() {
                if vrm.gesture.is_some() {
                    self.gesture = vrm.gesture.take();
                    migrated = true;
                }
            } else if vrm.gesture.is_some() {
                // Top-level exists, prefer it; clear the legacy copy.
                vrm.gesture = None;
                migrated = true;
            }
        }
...
impl Default for WorkspaceConfig {
    fn default() -> Self {
        Self {
            ...
            default_workspace_parent_dir: None,
        }
    }
}
