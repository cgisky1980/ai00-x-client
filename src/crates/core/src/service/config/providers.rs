...
pub fn get_default_rwkv_model_config() -> AIModelConfig {
    AIModelConfig {
        id: "rwkv-local".to_string(),
        name: "RWKV Local".to_string(),
        provider: "rwkv".to_string(),
        model_name: "rwkv7-local".to_string(),
        base_url: String::new(),
        request_url: None,
        api_key: String::new(),
...
impl ConfigProvider for AIConfigProvider {
    ...
    fn get_default_config(&self) -> serde_json::Value {
        let mut default = AIConfig::default();
...
impl ConfigProviderRegistry {
    ...
    pub async fn validate_config(
        &self,
        config: &GlobalConfig,
    ) -> Ai00XResult<ConfigValidationResult> {
    ...
    fn get_config_section(
        ...
        match section {
            ...
            "underlay" => Ok(serde_json::to_value(&config.underlay)?),
            _ => Err(Ai00XError::validation(format!(
