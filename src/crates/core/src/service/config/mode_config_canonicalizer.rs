...
use crate::agent::tools::registry::get_all_registered_tools;
...
fn build_mode_view(
    ...
    ModeConfigView {
        ...
        mcp_eager_load: mode_config.map(|c| c.mcp_eager_load).unwrap_or(false),
    }
}
...
pub async fn get_mode_config_views() -> Ai00XResult<HashMap<String, ModeConfigView>> {
...
pub async fn get_mode_config_view(mode_id: &str) -> Ai00XResult<ModeConfigView> {
    let views = get_mode_config_views().await?;
    views
        .get(mode_id)
        .cloned()
        .ok_or_else(|| Ai00XError::config(format!("Mode does not exist: {}", mode_id)))
}

pub async fn persist_mode_config_from_value(mode_id: &str, config: Value) -> Ai00XResult<()> {
    let config_service = GlobalConfigManager::get_service()?;
...
pub async fn reset_mode_config_to_default(mode_id: &str) -> Ai00XResult<()> {
    let config_service = GlobalConfigManager::get_service()?;
    let mut stored_configs: HashMap<String, ModeConfig> = config_service
        .get_config(Some("ai.mode_configs"))
        .await
...
pub async fn canonicalize_mode_configs() -> Ai00XResult<ModeConfigCanonicalizationReport> {
    eprintln!("[TRACE] CANONICALIZE_ENTER");
    let config_service = GlobalConfigManager::get_service()?;
    let valid_tools = get_valid_tool_names().await;
    let mode_defaults = get_mode_defaults().await;
    ...
    let ai_object = ai_value
        .as_object_mut()
        .ok_or_else(|| Ai00XError::config("AI config must be a JSON object".to_string()))?;
    ...
    eprintln!("[TRACE] CANONICALIZE_EXIT");
...
#[cfg(test)]
mod tests {
    // Tests removed: normalize_skill_override_lists and skill override logic
    // are no longer part of the skill system.
}
