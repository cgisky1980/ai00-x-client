...
impl ProjectContextService {
    ...
    pub async fn get_document_statuses(
        &self,
        workspace: &Path,
    ) -> Ai00XResult<Vec<ContextDocumentStatus>> {
    ...
    pub async fn create_document(&self, workspace: &Path, doc_id: &str) -> Ai00XResult<PathBuf> {
        let doc = find_builtin_document(doc_id)
            .ok_or_else(|| Ai00XError::service(format!("Unknown document id: {}", doc_id)))?;

        let relative_path = doc.possible_paths.first().ok_or_else(|| {
            Ai00XError::service(format!("No possible path for document: {}", doc_id))
        })?;
        ...
        fs::write(&full_path, template)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to create document: {}", e)))?;
    ...
    /// # Returns
    /// Generated file path
    pub async fn generate_document(&self, workspace: &Path, doc_id: &str) -> Ai00XResult<PathBuf> {
        ...
        let result = match result {
            Ok(r) => r,
            Err(e) => {
                if matches!(e, Ai00XError::Cancelled(_)) {
                    debug!(
    ...
    pub async fn cancel_generate_document(&self, doc_id: &str) -> Ai00XResult<()> {
    ...
    /// Loads configuration.
    pub async fn load_config(&self, workspace: &Path) -> Ai00XResult<ProjectContextConfig> {
        ...
        let content = fs::read_to_string(&config_path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to read config: {}", e)))?;

        serde_json::from_str(&content)
            .map_err(|e| Ai00XError::service(format!("Failed to parse config: {}", e)))
    }
    ...
    async fn load_config_and_cleanup(&self, workspace: &Path) -> Ai00XResult<ProjectContextConfig> {
        let mut config = self.load_config(workspace).await.unwrap_or_default();
    ...
    pub async fn save_config(
        ...
        if let Some(parent) = config_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                Ai00XError::service(format!("Failed to create .ai00-x directory: {}", e))
            })?;
        }

        let content = serde_json::to_string_pretty(config)
            .map_err(|e| Ai00XError::service(format!("Failed to serialize config: {}", e)))?;

        fs::write(&config_path, content)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to write config: {}", e)))?;
    ...
    fn get_config_path(&self, workspace: &Path) -> PathBuf {
        workspace.join(".ai00-x").join(CONFIG_FILE_NAME)
    }
    ...
    pub async fn create_category(
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .map_err(|e| Ai00XError::service(format!("Failed to get timestamp: {}", e)))?
            .as_secs();
    ...
    pub async fn update_category(
        ...
        let category = config
            ...
            .find(|cat| cat.id == category_id)
            .ok_or_else(|| {
                Ai00XError::service(format!("Custom category not found: {}", category_id))
            })?;
    ...
    pub async fn delete_category(&self, workspace: &Path, category_id: &str) -> Ai00XResult<()> {
    ...
    pub async fn get_all_categories(&self, workspace: &Path) -> Ai00XResult<Vec<CategoryInfo>> {
        let config = self.load_config_and_cleanup(workspace).await?;
    ...
    pub async fn import_document(
        ...
        let target_dir = workspace.join(".ai00-x").join("docs").join(&category_id);
        ...
        fs::copy(source_path, &final_target_path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to copy file: {}", e)))?;
    ...
    pub async fn delete_imported_document(
        ...
        let doc_index = config
            ...
            .ok_or_else(|| {
                Ai00XError::service(format!("Imported document not found: {}", doc_id))
            })?;
        ...
        if doc_path.exists() {
            fs::remove_file(&doc_path).await.map_err(|e| {
                Ai00XError::service(format!("Failed to remove document file: {}", e))
            })?;
    ...
    /// - workspace: Workspace path
    /// - doc_id: Document ID
    pub async fn delete_context_document(&self, workspace: &Path, doc_id: &str) -> Ai00XResult<()> {
    ...
    pub async fn get_imported_documents(
        &self,
        workspace: &Path,
    ) -> Ai00XResult<Vec<ImportedDocument>> {
        let config = self.load_config(workspace).await.unwrap_or_default();
