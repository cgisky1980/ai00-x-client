...
impl FileSystemService {
    ...
    /// Builds a file tree.
    pub async fn build_file_tree(&self, root_path: &str) -> Ai00XResult<Vec<FileTreeNode>> {
    ...
    pub async fn scan_directory(&self, root_path: &str) -> Ai00XResult<DirectoryScanResult> {
    ...
    pub async fn get_directory_contents(&self, path: &str) -> Ai00XResult<Vec<FileTreeNode>> {
    ...
    ) -> Ai00XResult<Vec<FileSearchResult>> {
    ...
    /// Writes a file.
    pub async fn write_file(&self, file_path: &str, content: &str) -> Ai00XResult<FileWriteResult> {
    ...
    /// Gets the file size.
    pub async fn get_file_size(&self, file_path: &str) -> Ai00XResult<u64> {
        let info = self.get_file_info(file_path).await?;
        Ok(info.size)
    }

    /// Reads a text file quickly.
    pub async fn read_text_file(&self, file_path: &str) -> Ai00XResult<String> {
        let result = self.read_file(file_path).await?;
        if result.is_binary {
            Err(Ai00XError::service(
    ...
    pub async fn write_text_file(&self, file_path: &str, content: &str) -> Ai00XResult<()> {
        self.write_file(file_path, content).await.map(|_| ())
    }

    /// Lists all files in a directory (recursive).
    pub async fn list_all_files(&self, root_path: &str) -> Ai00XResult<Vec<String>> {
    ...
    pub async fn calculate_directory_size(&self, dir_path: &str) -> Ai00XResult<u64> {
    ...
    pub async fn get_directory_stats(&self, dir_path: &str) -> Ai00XResult<DirectoryStats> {
        let scan_result = self.scan_directory(dir_path).await?;
