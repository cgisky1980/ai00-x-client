...
impl WorkspaceRuntimeService {
    ...
    async fn persist_layout_state(
        ...
        let bytes = serde_json::to_vec_pretty(&state).map_err(|e| {
            Ai00XError::service(format!("Failed to serialize runtime state: {}", e))
        })?;
    ...
    async fn migrate_legacy_project_runtime_data(
        ...
        context: &WorkspaceRuntimeContext,
    ) -> Ai00XResult<Vec<RuntimeMigrationRecord>> {
    ...
    async fn migrate_first_existing_path(
        ...
        candidates: &[PathBuf],
        target: &Path,
    ) -> Ai00XResult<Option<RuntimeMigrationRecord>> {
        if target.exists() {
            return Ok(None);
        }
    ...
    async fn move_legacy_path(
        ...
        source: &Path,
        target: &Path,
    ) -> Ai00XResult<RuntimeMigrationRecord> {
...
fn copy_dir_recursive(source: &Path, target: &Path) -> Ai00XResult<()> {
...
pub fn try_get_workspace_runtime_service_arc() -> Ai00XResult<Arc<WorkspaceRuntimeService>> {
...
mod tests {
    ...
    async fn ensure_local_workspace_runtime_creates_complete_layout_without_project_dot_dir() {
        let test_root =
            std::env::temp_dir().join(format!("ai00-x-runtime-test-{}", Uuid::new_v4()));
    ...
    async fn ensure_local_workspace_runtime_migrates_legacy_runtime_entries() {
        let test_root =
            std::env::temp_dir().join(format!("ai00-x-runtime-test-{}", Uuid::new_v4()));
        let workspace_root = test_root.join("workspace");
        let legacy_root = workspace_root.join(".ai00-x");
