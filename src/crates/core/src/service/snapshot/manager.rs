...
impl Tool for WrappedTool {
    ...
    async fn description(&self) -> crate::util::errors::Ai00XResult<String> {
    ...
    async fn input_schema_for_model_with_context(
        &self,
        context: Option<&crate::agent::tools::framework::ToolUseContext>,
    ) -> Value {
        self.original_tool
    ...
    ) -> crate::agent::tools::framework::ValidationResult {
    ...
    fn render_tool_use_message(
        &self,
        input: &Value,
        options: &crate::agent::tools::framework::ToolRenderOptions,
    ) -> String {
    ...
    ) -> crate::util::errors::Ai00XResult<Vec<ToolResult>> {
...
impl WrappedTool {
    ...
    async fn handle_file_modification_internal(
        ...
        let raw_path = match self.extract_file_path_simple(input) {
            Ok(path) => path,
            Err(e) => return Err(crate::util::errors::Ai00XError::Tool(e.to_string())),
        };
        ...
        let snapshot_manager = get_or_create_snapshot_manager(snapshot_workspace.clone(), None)
            .await
            .map_err(|e| crate::util::errors::Ai00XError::Tool(e.to_string()))?;
