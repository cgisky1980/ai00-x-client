...
impl MCPServerRegistry {
    ...
    /// Registers a server.
    pub async fn register(&self, config: &MCPServerConfig) -> crate::util::errors::Ai00XResult<()> {
        config.validate()?;
    ...
    pub async fn unregister(&self, server_id: &str) -> crate::util::errors::Ai00XResult<()> {
        let mut servers = self.servers.write().await;
        ...
        if let Some(process) = servers.remove(server_id) {
            ...
            Err(crate::util::errors::Ai00XError::NotFound(format!(
    ...
    /// Clears the registry.
    pub async fn clear(&self) -> crate::util::errors::Ai00XResult<()> {
        let mut servers = self.servers.write().await;
