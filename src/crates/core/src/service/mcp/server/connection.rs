...
impl MCPConnection {
        ...
        match &self.transport {
            TransportType::Local(transport) => {
                ...
                match tokio::time::timeout(self.request_timeout, rx).await {
                    ...
                    Ok(Err(_)) => Err(Ai00XError::MCPError(format!(
                        "Request channel closed for method: {}",
                        method
                    ))),
                    Err(_) => Err(Ai00XError::Timeout(format!(
                        "Request timeout for method: {}",
                        method
                    ))),
                }
            }
            TransportType::Remote(_transport) => Err(Ai00XError::NotImplemented(
    ...
    pub async fn send_response(&self, request_id: Value, result: Value) -> Ai00XResult<()> {
        match &self.transport {
            TransportType::Local(transport) => transport.send_response(request_id, result).await,
            TransportType::Remote(_) => Err(Ai00XError::NotImplemented(
    ...
    pub async fn send_error(&self, request_id: Value, error: MCPError) -> Ai00XResult<()> {
        match &self.transport {
