...
impl MCPServerProcess {
    fn is_auth_error(error: &Ai00XError) -> bool {
    ...
    pub async fn start(
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| Ai00XError::ProcessError("Failed to capture stdin".to_string()))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| Ai00XError::ProcessError("Failed to capture stdout".to_string()))?;
    ...
    pub async fn start_remote(&mut self, config: &MCPServerConfig) -> Ai00XResult<()> {
        let url = config.url.as_deref().ok_or_else(|| {
            Ai00XError::Configuration(format!("Remote MCP server '{}' is missing a URL", self.id))
        })?;
        ...
        if !merged_headers.contains_key("Authorization")
            ...
            && !merged_headers.contains_key("AUTHORIZATION")
        {
            // Backward compatibility: older Ai00-X configs store `Authorization` under `env`.
    ...
    async fn handshake(&mut self) -> Ai00XResult<()> {
        let connection = self
            .connection
            .as_ref()
            .ok_or_else(|| Ai00XError::MCPError("Connection not established".to_string()))?;
        ...
        let result: InitializeResult = connection
            .initialize("Ai00-X", env!("CARGO_PKG_VERSION"))
            .await?;
    ...
    pub async fn stop(&mut self) -> Ai00XResult<()> {
    ...
    pub async fn restart(
        ...
        if self.restart_count >= self.max_restarts {
            ...
            .await;
            return Err(Ai00XError::MCPError(format!(
                "Max restart attempts ({}) reached",
                self.max_restarts
            )));
        }
...
mod tests {
    ...
    fn detect_auth_error_patterns() {
        let unauthorized = Ai00XError::MCPError("Handshake failed: Unauthorized (401)".to_string());
        assert!(MCPServerProcess::is_auth_error(&unauthorized));

        let oauth_refresh = Ai00XError::MCPError(
        ...
        let generic = Ai00XError::MCPError("Handshake failed: connection reset".to_string());
