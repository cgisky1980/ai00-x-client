...
pub fn map_auth_error(error: impl ToString) -> Ai00XError {
    Ai00XError::MCPError(format!("OAuth error: {}", error.to_string()))
}

pub async fn has_stored_oauth_credentials(server_id: &str) -> Ai00XResult<bool> {
...
pub async fn clear_stored_oauth_credentials(server_id: &str) -> Ai00XResult<()> {
...
pub async fn build_authorization_manager(
    server_id: &str,
    server_url: &str,
) -> Ai00XResult<(AuthorizationManager, bool)> {
...
fn effective_oauth_config(config: &MCPServerConfig) -> MCPServerOAuthConfig {
    ...
    if oauth.client_name.is_none() {
        oauth.client_name = Some(format!("Ai00-X MCP Client ({})", config.name));
    }
    oauth
}
