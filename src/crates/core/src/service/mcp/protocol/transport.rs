...
impl MCPTransport {
    ...
    /// Sends a request.
    pub async fn send_request(&self, method: String, params: Option<Value>) -> Ai00XResult<u64> {
    ...
    /// Sends a response.
    pub async fn send_response(&self, id: Value, result: Value) -> Ai00XResult<()> {
        let response = MCPResponse::success(id, result);
        self.send_message(MCPMessage::Response(response)).await
    }

    /// Sends an error response.
    pub async fn send_error(&self, id: Value, error: MCPError) -> Ai00XResult<()> {
        let response = MCPResponse::error(id, error);
        self.send_message(MCPMessage::Response(response)).await
    }

    /// Sends a message.
    async fn send_message(&self, message: MCPMessage) -> Ai00XResult<()> {
        let json = serde_json::to_string(&message).map_err(|e| {
            Ai00XError::serialization(format!("Failed to serialize MCP message: {}", e))
        })?;
        ...
        stdin
            ...
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to write to MCP server stdin: {}", e)))?;
        stdin.write_all(b"\n").await.map_err(|e| {
        ...
        stdin
            .flush()
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to flush MCP server stdin: {}", e)))?;
