...
use super::types::{
    InitializeResult as Ai00XInitializeResult, MCPAnnotations, MCPCapability, MCPPrompt,
...
#[derive(Clone)]
struct Ai00XRmcpClientHandler {
    info: ClientInfo,
}

impl ClientHandler for Ai00XRmcpClientHandler {
...
enum ClientState {
    Connecting {
        transport: Option<StreamableHttpClientTransport<Ai00XStreamableHttpClient>>,
    },
    Ready {
        service: Arc<RunningService<RoleClient, Ai00XRmcpClientHandler>>,
    },
}

#[derive(Clone)]
struct Ai00XStreamableHttpClient {
    client: reqwest::Client,
    oauth_manager: Option<Arc<Mutex<AuthorizationManager>>>,
}

impl Ai00XStreamableHttpClient {
...
impl StreamableHttpClient for Ai00XStreamableHttpClient {
...
impl RemoteMCPTransport {
    ...
    fn build_default_headers(headers: &HashMap<String, String>) -> HeaderMap {
        ...
        if !header_map.contains_key(USER_AGENT) {
            header_map.insert(
                USER_AGENT,
                HeaderValue::from_static("Ai00-X-MCP-Client/1.0"),
            );
        }
    ...
    async fn service(
        &self,
    ) -> Ai00XResult<Arc<RunningService<RoleClient, Ai00XRmcpClientHandler>>> {
        let guard = self.state.lock().await;
        match &*guard {
            ClientState::Ready { service } => Ok(Arc::clone(service)),
            ClientState::Connecting { .. } => Err(Ai00XError::MCPError(
        ...
        match &mut *guard {
            ...
            ClientState::Connecting { transport } => {
                ...
                let service = tokio::time::timeout(self.request_timeout, transport_fut)
                    .await
                    .map_err(|_| {
                        Ai00XError::Timeout(format!(
                            "Timed out handshaking with MCP server after {:?}: {}",
                            self.request_timeout, self.url
                        ))
                    })?
                    .map_err(|e| Ai00XError::MCPError(format!("Handshake failed: {}", e)))?;
    ...
    pub async fn ping(&self) -> Ai00XResult<()> {
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP ping timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP ping failed: {}", e)))?;

        match result {
            rmcp::model::ServerResult::EmptyResult(_) => Ok(()),
            other => Err(Ai00XError::MCPError(format!(
    ...
    pub async fn list_resources(&self, cursor: Option<String>) -> Ai00XResult<ResourcesListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP resources/list timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP resources/list failed: {}", e)))?;
    ...
    pub async fn read_resource(&self, uri: &str) -> Ai00XResult<ResourcesReadResult> {
        let service = self.service().await?;
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP resources/read timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP resources/read failed: {}", e)))?;
    ...
    pub async fn list_prompts(&self, cursor: Option<String>) -> Ai00XResult<PromptsListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP prompts/list timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP prompts/list failed: {}", e)))?;
    ...
    pub async fn get_prompt(
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP prompts/get timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP prompts/get failed: {}", e)))?;
    ...
    pub async fn list_tools(&self, cursor: Option<String>) -> Ai00XResult<ToolsListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP tools/list timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP tools/list failed: {}", e)))?;
    ...
    pub async fn call_tool(
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| Ai00XError::Timeout("MCP tools/call timeout".to_string()))?
            .map_err(|e| Ai00XError::MCPError(format!("MCP tools/call failed: {}", e)))?;

        Ok(map_tool_result(result))
    }
}

fn map_initialize_result(info: &rmcp::model::ServerInfo) -> Ai00XInitializeResult {
    Ai00XInitializeResult {
...
mod tests {
    ...
    fn build_client_info_declares_supported_client_capabilities() {
        let info = RemoteMCPTransport::build_client_info("Ai00-X", "1.0.0");
