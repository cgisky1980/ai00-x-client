...
impl MCPConfigService {
    ...
    pub async fn load_mcp_json_config(&self) -> Ai00XResult<String> {
        match self
            .config_service
    ...
    pub async fn save_mcp_json_config(&self, json_config: &str) -> Ai00XResult<()> {
        ...
        let config_value: serde_json::Value = serde_json::from_str(json_config).map_err(|e| {
            ...
            error!("{}", error_msg);
            Ai00XError::validation(error_msg)
        })?;

        if config_value.get("mcpServers").is_none() {
            let error_msg = "Config missing 'mcpServers' field";
            error!("{}", error_msg);
            return Err(Ai00XError::validation(error_msg.to_string()));
        }
