...
pub async fn initialize_global_ai_rules_service() -> Ai00XResult<()> {
    ...
    let path_manager = try_get_path_manager_arc()
        .map_err(|e| Ai00XError::service(format!("Failed to create PathManager: {}", e)))?;
    ...
    GLOBAL_AI_RULES_SERVICE.set(wrapper).map_err(|_| {
        Ai00XError::service("Failed to initialize global AIRulesService".to_string())
    })?;
...
pub async fn get_global_ai_rules_service() -> Ai00XResult<Arc<AIRulesService>> {
    let wrapper = GLOBAL_AI_RULES_SERVICE.get()
        .ok_or_else(|| Ai00XError::service(
    ...
    guard
        .as_ref()
        .ok_or_else(|| Ai00XError::service("Global AIRulesService is None".to_string()))
        .map(Arc::clone)
}
...
impl AIRulesService {
    ...
    pub async fn new(path_manager: Arc<PathManager>) -> Ai00XResult<Self> {
        let service = Self {
            path_manager,
    ...
    pub async fn set_workspace(&self, workspace_path: PathBuf) -> Ai00XResult<()> {
    ...
    pub async fn get_user_rules(&self) -> Ai00XResult<Vec<AIRule>> {
        Ok(self.user_rules.read().await.clone())
    }

    /// Returns a single user-level rule.
    pub async fn get_user_rule(&self, name: &str) -> Ai00XResult<Option<AIRule>> {
        let rules = self.user_rules.read().await;
        Ok(rules.iter().find(|r| r.name == name).cloned())
    }

    /// Creates a user-level rule.
    pub async fn create_user_rule(&self, request: CreateRuleRequest) -> Ai00XResult<AIRule> {
        ...
        self.get_user_rule(&rule_name)
            .await?
            .ok_or_else(|| Ai00XError::service("Failed to create rule".to_string()))
    }
    ...
    pub async fn update_user_rule(
        ...
        self.get_user_rule(new_name)
            .await?
            .ok_or_else(|| Ai00XError::service("Failed to update rule".to_string()))
    }

    /// Deletes a user-level rule.
    pub async fn delete_user_rule(&self, name: &str) -> Ai00XResult<bool> {
        let rules_dir = self.path_manager.user_rules_dir();
    ...
    pub async fn reload_user_rules(&self) -> Ai00XResult<()> {
        let rules_dir = self.path_manager.user_rules_dir();
        let rules = self
    ...
    pub async fn get_user_rules_stats(&self) -> Ai00XResult<RuleStats> {
        let rules = self.user_rules.read().await;
        Ok(Self::calculate_stats(&rules))
    }

    /// Returns all project-level rules.
    pub async fn get_project_rules(&self) -> Ai00XResult<Vec<AIRule>> {
    ...
    pub async fn get_project_rules_for_workspace(
        &self,
        workspace: &Path,
    ) -> Ai00XResult<Vec<AIRule>> {
        self.load_project_rules_for_workspace(workspace).await
    }

    /// Returns a single project-level rule.
    pub async fn get_project_rule(&self, name: &str) -> Ai00XResult<Option<AIRule>> {
    ...
    pub async fn create_project_rule(&self, request: CreateRuleRequest) -> Ai00XResult<AIRule> {
    ...
    /// Supports rules from both the Ai00-X and Cursor directories.
    ...
    pub async fn update_project_rule_for_workspace(
        ...
        let rule = self
            ...
            .await?
            .ok_or_else(|| Ai00XError::service(format!("Rule '{}' not found", name)))?;

        let rules_dir = rule
            .file_path
            .parent()
            .ok_or_else(|| Ai00XError::service("Invalid rule file path".to_string()))?;
    ...
    /// Supports rules from both the Ai00-X and Cursor directories.
    pub async fn delete_project_rule(&self, name: &str) -> Ai00XResult<bool> {
        let workspace_path = self.require_workspace_path().await?;
    ...
    /// Loads Ai00-X rules first, then Cursor rules; for duplicates, the first loaded wins.
    pub async fn reload_project_rules(&self) -> Ai00XResult<()> {
    ...
    pub async fn reload_project_rules_for_workspace(&self, workspace: &Path) -> Ai00XResult<()> {
        let rules = self.load_project_rules_for_workspace(workspace).await?;
        self.sync_project_rules_cache_if_current(workspace, &rules)
            .await;
        Ok(())
    ...
    pub async fn get_project_rules_stats(&self) -> Ai00XResult<RuleStats> {
        let rules = self.project_rules.read().await;
        Ok(Self::calculate_stats(&rules))
    }
    ...
    pub async fn get_project_rules_stats_for_workspace(
        &self,
        workspace: &Path,
    ) -> Ai00XResult<RuleStats> {
        let rules = self.load_project_rules_for_workspace(workspace).await?;
        Ok(Self::calculate_stats(&rules))
    }

    async fn load_project_rules_for_workspace(&self, workspace: &Path) -> Ai00XResult<Vec<AIRule>> {
        let mut all_rules = Vec::new();
        let mut loaded_names = std::collections::HashSet::new();

        let ai00x_rules_dir = self.path_manager.project_rules_dir(workspace);
        let ai00x_rules = self
            .load_rules_from_dir(&ai00x_rules_dir, RuleLevel::Project)
            .await?;

        for rule in ai00x_rules {
        ...
        if cursor_rules_dir.exists() {
            ...
            for rule in cursor_rules {
                if !loaded_names.contains(&rule.name) {
                    ...
                    debug!(
                        "Skipping Cursor rule '{}' (already loaded from Ai00-X)",
                        rule.name
                    );
                }
            }
...
"#);
    ...
    pub async fn build_system_prompt(&self) -> Ai00XResult<String> {
        let user_rules = self.user_rules.read().await.clone();
    ...
    async fn load_rules_from_dir(&self, dir: &Path, level: RuleLevel) -> Ai00XResult<Vec<AIRule>> {
    ...
    async fn load_rule_from_file(&self, path: &Path, level: RuleLevel) -> Ai00XResult<AIRule> {
        let content = tokio::fs::read_to_string(path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to read file {:?}: {}", path, e)))?;

        let name = path
            .file_stem()
            .and_then(|s| s.to_str())
            .map(|s| s.to_string())
            .ok_or_else(|| Ai00XError::service("Invalid file name".to_string()))?;

        AIRule::from_mdc(name, level, path.to_path_buf(), &content).map_err(Ai00XError::service)
    }
    ...
    async fn create_rule_internal(
        ...
        tokio::fs::create_dir_all(dir)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to create directory: {}", e)))?;
        ...
        tokio::fs::write(&file_path, mdc_content)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to write file: {}", e)))?;
    ...
    async fn update_rule_internal(
        ...
        if !old_file_path.exists() {
            return Err(Ai00XError::service(format!("Rule '{}' not found", name)));
        }

        let content = tokio::fs::read_to_string(&old_file_path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to read file: {}", e)))?;

        let (mut frontmatter, mut body) =
            parse_mdc_content(&content).map_err(Ai00XError::service)?;
        ...
        let new_file_path = if let Some(new_name) = &request.name {
            if new_name != name {
                ...
                tokio::fs::remove_file(&old_file_path).await.map_err(|e| {
                    Ai00XError::service(format!("Failed to delete old file: {}", e))
                })?;
                new_path
            } else {
                old_file_path
    ...
    /// Deletes a rule file.
    async fn delete_rule_internal(&self, dir: &Path, name: &str) -> Ai00XResult<bool> {
        ...
        tokio::fs::remove_file(&file_path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to delete file: {}", e)))?;
    ...
    pub async fn toggle_user_rule(&self, name: &str) -> Ai00XResult<AIRule> {
        let rule = self
            .get_user_rule(name)
            .await?
            .ok_or_else(|| Ai00XError::service(format!("Rule '{}' not found", name)))?;
    ...
    pub async fn toggle_project_rule(&self, name: &str) -> Ai00XResult<AIRule> {
        let workspace_path = self.require_workspace_path().await?;
    ...
    async fn require_workspace_path(&self) -> Ai00XResult<PathBuf> {
        self.workspace_path
            .read()
            .await
            .clone()
            .ok_or_else(|| Ai00XError::service("No workspace set".to_string()))
    }
...
mod tests {
    ...
    async fn loading_missing_project_rules_does_not_create_rules_dir() {
        let test_root =
            std::env::temp_dir().join(format!("ai00-x-ai-rules-test-{}", Uuid::new_v4()));
