...
impl WorkspaceProvider {
    ...
    pub async fn open(&self, path: &str) -> Ai00XResult<WorkspaceInfo> {
    ...
    pub async fn switch(&self, workspace_id: &str) -> Ai00XResult<()> {
    ...
    pub async fn close_current(&self) -> Ai00XResult<()> {
    ...
    /// Quick cleanup.
    pub async fn quick_cleanup(&self) -> Ai00XResult<WorkspaceCleanupResult> {
    ...
    pub async fn detect_project_type(&self, path: &str) -> Ai00XResult<WorkspaceType> {
        let path_buf = PathBuf::from(path);

        if !path_buf.exists() {
            return Err(Ai00XError::service("Path does not exist".to_string()));
        }
    ...
    pub async fn get_file_stats(
        &self,
        workspace_id: &str,
    ) -> Ai00XResult<Option<WorkspaceStatistics>> {
    ...
    /// Rescans a workspace.
    pub async fn rescan(&self, workspace_id: &str) -> Ai00XResult<WorkspaceInfo> {
