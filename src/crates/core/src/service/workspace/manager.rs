...
use ai00_x_git::GitOperationResult;
...
impl WorkspaceInfo {
    ...
    pub async fn new(root_path: PathBuf, options: WorkspaceOpenOptions) -> Ai00XResult<Self> {
        ...
        let (id, resolved_root_path) = if is_remote {
            ...
            let (canonical_pb, norm_str) =
                canonicalize_local_workspace_root(&root_path).map_err(Ai00XError::service)?;
    ...
    async fn load_worktree(&mut self) {
        let normalized_workspace_path = self.root_path.to_string_lossy().replace('\\', "/");
        let worktrees = match GitService::list_worktrees(&self.root_path).await {
            Ok(w) => w,
            Err(_) => return,
        };

        let main_worktree = worktrees.iter().find(|w| w.is_main);
        let main_repo_path = match main_worktree {
            Some(w) => PathBuf::from(&w.path),
            None => return,
        };

        self.repo_root_path = Some(main_repo_path.clone());

        let matching = worktrees
            .iter()
            .find(|w| w.path == normalized_workspace_path);
        let active_branch = matching.and_then(|w| w.branch.clone());

        self.active_branch = active_branch;

        self.available_branches = worktrees
            .iter()
            .map(|w| BranchInfo {
                name: w.branch.clone().unwrap_or_else(|| "main".to_string()),
                worktree_path: w.path.clone(),
                is_main: w.is_main,
                is_active: w.branch.as_ref() == self.active_branch.as_ref()
                    || (w.is_main && self.active_branch.is_none()),
                has_changes: None,
            })
            .collect();

        if let Some(matching_wt) = matching {
            let main_repo_name = main_repo_path
                .file_name()
                .and_then(|n| n.to_str())
                .map(|s| s.to_string());
            self.worktree = Some(WorkspaceWorktreeInfo {
                path: matching_wt.path.clone(),
                branch: matching_wt.branch.clone(),
                main_repo_path: main_repo_path.to_string_lossy().to_string(),
                is_main: matching_wt.is_main,
                main_repo_name,
            });
        }
    }

    pub async fn refresh_branches(&mut self) -> Vec<BranchInfo> {
        let repo_root = match &self.repo_root_path {
            Some(p) => p.clone(),
            None => return self.available_branches.clone(),
        };

        let worktrees = match GitService::list_worktrees(&repo_root).await {
            Ok(w) => w,
            Err(_) => return self.available_branches.clone(),
        };
    ...
    /// Scans the workspace.
    async fn scan_workspace(&mut self, options: ScanOptions) -> Ai00XResult<()> {
    ...
    fn scan_directory<'a>(
        ...
        depth: usize,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Ai00XResult<()>> + 'a + Send>> {
        Box::pin(async move {
            ...
            let mut read_dir = fs::read_dir(dir)
                .await
                .map_err(|e| Ai00XError::service(format!("Failed to read directory: {}", e)))?;

            while let Some(entry) = read_dir.next_entry().await.map_err(|e| {
                Ai00XError::service(format!("Failed to read directory entry: {}", e))
            })? {
                let path = entry.path();
...
impl WorkspaceManager {
    ...
    pub fn rekey_workspace_id(&mut self, old_id: &str, new_id: String) -> Ai00XResult<()> {
    ...
    /// Migrates persisted local workspaces from legacy UUID ids to `local_*` stable ids.
    ...
    /// Opens a workspace.
    pub async fn open_workspace(&mut self, path: PathBuf) -> Ai00XResult<WorkspaceInfo> {
    ...
    pub async fn open_workspace_with_options(
        ...
        let existing_workspace_id = if is_remote {
            ...
            let canon_norm = match normalize_local_workspace_root_for_stable_id(&path) {
                Ok(n) => n,
                Err(e) => return Err(Ai00XError::service(e)),
            };
    ...
    pub fn close_current_workspace(&mut self) -> Ai00XResult<()> {
    ...
    pub fn close_workspace(&mut self, workspace_id: &str) -> Ai00XResult<()> {
    ...
    pub fn set_active_workspace(&mut self, workspace_id: &str) -> Ai00XResult<()> {
        if !self
    ...
    pub fn set_current_workspace(&mut self, workspace_id: String) -> Ai00XResult<()> {
    ...
    /// Removes a workspace.
    pub fn remove_workspace(&mut self, workspace_id: &str) -> Ai00XResult<()> {
    ...
    pub async fn cleanup_invalid_workspaces(&mut self) -> Ai00XResult<usize> {
    ...
    pub async fn switch_branch(
        ...
        let target_path = {
            let workspace = self.workspaces.get(workspace_id).ok_or_else(|| {
                Ai00XError::service(format!("Workspace not found: {}", workspace_id))
            })?;
            workspace
                .available_branches
                .iter()
                ...
                .ok_or_else(|| Ai00XError::service(format!("Branch not found: {}", branch_name)))?
        };

        let workspace = self.workspaces.get_mut(workspace_id).unwrap();
        workspace.root_path = PathBuf::from(&target_path);
        workspace.active_branch = Some(branch_name.to_string());
        workspace.refresh_branches().await;

        Ok(workspace.clone())
    }

    /// Creates a new branch with a worktree and switches to it.
    pub async fn create_and_switch_branch(
        ...
        GitService::add_worktree(&repo_path_str, branch_name, true, None)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to create branch: {}", e)))?;

        self.switch_branch(workspace_id, branch_name).await
    }

    /// Merges a branch into main and cleans up the worktree.
    pub async fn merge_branch_to_main(
        &mut self,
        workspace_id: &str,
        branch_name: &str,
    ) -> Ai00XResult<GitOperationResult> {
        let (repo_root, worktree_path_to_remove) = {
            let workspace = self.workspaces.get(workspace_id).ok_or_else(|| {
                Ai00XError::service(format!("Workspace not found: {}", workspace_id))
            })?;
            ...
            (repo_root, branch_info.worktree_path.clone())
        };

        let repo_path_str = repo_root.to_string_lossy().to_string();

        let merge_result = GitService::merge_branch(&repo_path_str, branch_name, None)
            .await
            .map_err(|e| Ai00XError::service(format!("Merge failed: {}", e)))?;

        if let Err(e) =
            GitService::remove_worktree(&repo_path_str, &worktree_path_to_remove, true).await
        {
            log::warn!("Failed to remove worktree after merge: {}", e);
        }

        self.switch_branch(workspace_id, "main").await?;
