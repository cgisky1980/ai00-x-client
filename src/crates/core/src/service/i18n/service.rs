...
impl I18nService {
    ...
    pub async fn initialize(&self) -> Ai00XResult<()> {
    ...
    async fn load_all_bundles(&self) -> Ai00XResult<()> {
    ...
    pub async fn set_locale(&self, locale: LocaleId) -> Ai00XResult<()> {
        let old_locale = {
            let mut current = self.current_locale.write().await;
