...
#[cfg(windows)]
fn list_ngrok_pids() -> Vec<u32> {
    crate::util::process_manager::create_command("tasklist")
...
pub async fn start_ngrok_tunnel(local_port: u16) -> Result<NgrokTunnel> {
    let ngrok_path = find_ngrok().ok_or_else(|| {
        anyhow!(
            "ngrok is not installed.\n\
             ...
             No need to start ngrok manually — Ai00-X will start it automatically.\n\
    ...
    let mut child = crate::util::process_manager::create_tokio_command(&ngrok_path)
        .args([
            "http",
...
#[cfg(windows)]
fn kill_process(pid: u32) {
    let _ = crate::util::process_manager::create_command("taskkill")
