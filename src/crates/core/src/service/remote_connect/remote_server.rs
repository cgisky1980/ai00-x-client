...
async fn load_remote_model_catalog(
    ...
    let config_service =
        get_global_config_service().map_err(|e| format!("Config service not available: {e}"))?;
...
pub enum RemoteCommand {
    ...
    SendMessage {
        ...
        image_contexts: Option<Vec<crate::agent::image_analysis::ImageContextData>>,
    },
    CancelTask {
        session_id: String,
...
fn strip_user_input_tags(content: &str) -> String {
    let s = crate::agent::core::strip_prompt_markup(content);
...
fn resolve_agent_type(mobile_type: Option<&str>) -> &'static str {
    match mobile_type {
        Some("code") | Some("agentic") | Some("Agentic") | Some("Core") | Some("Cowork")
        | Some("cowork") => "Code",
        Some("task") | Some("Task") => "Task",
        Some("wall") | Some("Wallpaper") => "Wallpaper",
...
pub fn images_to_contexts(
    images: Option<&Vec<ImageAttachment>>,
) -> Vec<crate::agent::image_analysis::ImageContextData> {
    ...
    imgs.iter()
        .map(|img| {
            ...
            crate::agent::image_analysis::ImageContextData {
...
/// Subscribes to `AgentEvent` and updates an in-memory snapshot.
...
impl RemoteSessionStateTracker {
    ...
    /// is maintained directly from the source `AgentEvent` stream and is
    ...
    fn handle_event(&self, event: &crate::agent::events::AgentEvent) {
        use ai00_x_events::AgentEvent as AE;
...
impl crate::agent::events::EventSubscriber for Arc<RemoteSessionStateTracker> {
    async fn on_event(
        &self,
        event: &crate::agent::events::AgentEvent,
    ) -> crate::util::errors::Ai00XResult<()> {
...
impl RemoteExecutionDispatcher {
    ...
    pub fn ensure_tracker(&self, session_id: &str) -> Arc<RemoteSessionStateTracker> {
        ...
        if let Some(coordinator) = crate::agent::coordination::get_global_coordinator() {
            ...
            if let Some(session) = session_mgr.get_session(session_id) {
                if let crate::agent::core::SessionState::Processing {
                    current_turn_id, ..
                } = &session.state
                {
    ...
    /// [`DialogScheduler`](crate::agent::coordination::DialogScheduler) (same as desktop).
    ...
    pub async fn send_message(
        ...
        submission_policy: crate::agent::coordination::DialogSubmissionPolicy,
        turn_id: Option<String>,
    ) -> std::result::Result<crate::agent::coordination::DialogSubmitOutcome, String> {
        use crate::agent::coordination::{get_global_coordinator, get_global_scheduler};
                ...
                match binding
                    .get_or_create(
                        ...
                        TerminalBindingOptions {
                            ...
                            env: Some(
                                crate::agent::tools::implementations::bash_tool::BashTool::noninteractive_env(),
                            ),
    ...
    pub async fn cancel_task(
        ...
        let session = match session_mgr.get_session(session_id) {
            ...
            None => {
                let workspace_path = resolve_session_workspace_path(session_id)
                    .await
                    .ok_or_else(|| {
                        format!("Workspace path not available for task: {}", session_id)
                    })?;
                coordinator
                    .restore_session(&workspace_path, session_id)
                    .await
        ...
        let running_turn_id = match &session.state {
            crate::agent::core::SessionState::Processing {
                current_turn_id, ..
...
impl RemoteServer {
        ...
        let (ws_path, has_workspace, path_str, project_name, git_branch, workspace_kind) =
            if let Some(ws_service) = get_global_workspace_service() {
                if let Some(ws) = ws_service.get_current_workspace().await {
                    ...
                    let branch = ai00_x_git::get_current_branch_from_path(&p);
    ...
    async fn handle_poll_command(&self, cmd: &RemoteCommand) -> RemoteResponse {
        ...
        let Some(workspace_path) = resolve_session_workspace_path(session_id).await else {
            return RemoteResponse::Error {
                message: format!("Workspace path not available for task: {}", session_id),
            };
        };
        let (all_chat_msgs, _) =
    ...
    async fn handle_session_command(&self, cmd: &RemoteCommand) -> RemoteResponse {
        use crate::agent::{coordination::get_global_coordinator, core::SessionConfig};
        ...
        match cmd {
            RemoteCommand::ListSessions {
                ...
                let Some(workspace_path) = workspace_path
                    ...
                    return RemoteResponse::Error {
                        message: "workspace_path is required for ListTasks".to_string(),
                    };
                };
                ...
                if let Ok(pm) = PathManager::new() {
                    ...
                    match PersistenceManager::new(pm) {
                        ...
                        Err(e) => {
                            ...
                            RemoteResponse::Error {
                                message: format!("Failed to initialize task storage: {e}"),
                            }
                        }
                    }
                } else {
            ...
            RemoteCommand::CreateSession {
                ...
                let Some(binding_ws_str) = binding_ws_str else {
                    return RemoteResponse::Error {
                        message: "workspace_path is required for CreateTask".to_string(),
                    };
                };
            ...
            RemoteCommand::SetSessionModel {
                    ...
                    .is_none()
                {
                    let Some(workspace_path) = resolve_session_workspace_path(session_id).await
                        ...
                        return RemoteResponse::Error {
                            message: format!(
                                "Workspace path not available for task: {}",
                                session_id
                            ),
                        };
                    };
                    ...
                    {
                        return RemoteResponse::Error {
                            message: format!("Failed to restore task: {e}"),
                        };
                    }
                }
            ...
            _ => RemoteResponse::Error {
                message: "Unknown task command".into(),
            },
        }
    }
        ...
        match cmd {
            RemoteCommand::SendMessage {
                ...
                info!(
                    ...
                    requested_agent_type.as_deref().unwrap_or("Core"),
                    ...
                    .await
                {
                    Ok(outcome) => {
                        let (sid, turn_id) = match outcome {
                            crate::agent::coordination::DialogSubmitOutcome::Started {
                                session_id,
                                turn_id,
                            }
                            | crate::agent::coordination::DialogSubmitOutcome::Queued {
                                session_id,
                                turn_id,
                            } => (session_id, turn_id),
                        };
