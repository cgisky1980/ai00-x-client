...
impl FeishuBot {
    ...
    fn enter_pairing_code_message(language: BotLanguage) -> &'static str {
        if language.is_chinese() {
            "请输入 Ai00-X Desktop 中显示的 6 位配对码。"
        } else {
            "Please enter the 6-digit pairing code from Ai00-X Desktop."
        }
    }
    ...
    async fn get_access_token(&self) -> Result<String> {
        ...
        let body: serde_json::Value = serde_json::from_str(&token_resp_text).map_err(|e| {
            anyhow!(
                ...
                &token_resp_text[..token_resp_text
                    .char_indices()
                    .take(200)
                    .last()
                    .map(|(i, c)| i + c.len_utf8())
                    .unwrap_or(token_resp_text.len())]
            )
        })?;
                ...
                "elements": [
                    {
                        ...
                        "element_id": "ai00-x_remote_reply_markdown",
                    }
                ],
            },
        })
    ...
    async fn download_image_as_data_url(&self, message_id: &str, file_key: &str) -> Result<String> {
        ...
        match crate::agent::image_analysis::optimize_image_with_size_limit(
            raw_bytes.to_vec(),
            "openai",
            Some(&content_type),
            Some(MAX_BYTES),
    ...
    async fn get_ws_endpoint(&self) -> Result<(String, serde_json::Value)> {
        ...
        let body: serde_json::Value = serde_json::from_str(&ws_resp_text).map_err(|e| {
            anyhow!(
                ...
                &ws_resp_text[..ws_resp_text
                    .char_indices()
                    .take(300)
                    .last()
                    .map(|(i, c)| i + c.len_utf8())
                    .unwrap_or(ws_resp_text.len())]
            )
        })?;
