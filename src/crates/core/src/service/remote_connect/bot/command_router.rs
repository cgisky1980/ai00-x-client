...
/// Display mode for bot sessions - Professional
...
pub struct ForwardRequest {
    ...
    pub turn_id: String,
    pub image_contexts: Vec<crate::agent::image_analysis::ImageContextData>,
}
...
要连接你的 Ai00-X 桌面端，请发送 Ai00-X Remote Connect 面板里显示的 6 位配对码。

如果你还没有配对码，请打开 Ai00-X Desktop -> Remote Connect -> Telegram/飞书/微信机器人，复制 6 位配对码并发送到这里。"
    } else {
        "\
Welcome to Ai00-X!

To connect your Ai00-X desktop app, please enter the 6-digit pairing code shown in your Ai00-X Remote Connect panel.

Need a pairing code? Open Ai00-X Desktop -> Remote Connect -> Telegram/Feishu/WeChat bot -> copy the 6-digit code and send it here."
    }
}
...
/new_code_session - 创建新的编码任务
/new_cowork_session - 创建新的协作任务
/cancel_task - 取消当前任务
/help - 显示帮助信息"
    } else {
        "\
...
/new_code_session - Create a new coding task
/new_cowork_session - Create a new cowork task
...
pub fn paired_success_message(language: BotLanguage) -> String {
    if language.is_chinese() {
        format!("配对成功！Ai00-X 已连接。\n\n{}", help_message(language))
    } else {
        format!(
            "Pairing successful! Ai00-X is now connected.\n\n{}",
            help_message(language)
        )
    }
}

/// After IM pairing: set up default workspace and create a fresh session.
/// Always creates a new session so the user starts with a clean context in the IM channel.
/// Mutates `state` (`current_session_id`). Does not set `paired`.
pub async fn bootstrap_im_chat_after_pairing(state: &mut BotChatState) -> String {
    ...
    if state.current_workspace.is_none() {
        return if language.is_chinese() {
            "自动准备未能完成：没有可用工作区。请使用 /switch_workspace 选择工作区。".to_string()
        } else {
            "Auto-setup incomplete: no workspace available. Use /switch_workspace to select one."
                .to_string()
        };
    }
    ...
    let create_res = handle_new_session(state, "Core").await;
    ...
    if language.is_chinese() {
        "已为你新建任务。直接发送消息即可开始。".to_string()
    } else {
        "A new task was created. Send a message to start.".to_string()
    }
}
...
pub fn main_menu_actions(language: BotLanguage, _display_mode: BotDisplayMode) -> Vec<BotAction> {
    vec![
...
fn session_entry_actions(language: BotLanguage, _display_mode: BotDisplayMode) -> Vec<BotAction> {
    vec![
...
fn new_session_actions(language: BotLanguage, _display_mode: BotDisplayMode) -> Vec<BotAction> {
    vec![
        BotAction::primary(label_new_code_session(language), "/new_code_session"),
        BotAction::secondary(label_new_cowork_session(language), "/new_cowork_session"),
        BotAction::secondary(label_switch_workspace(language), "/switch_workspace"),
...
async fn dispatch_im_bot_command(
    ...
    cmd: BotCommand,
    image_contexts: Vec<crate::agent::image_analysis::ImageContextData>,
) -> HandleResult {
...
async fn dispatch_im_bot_command_inner(
    ...
    match cmd {
        ...
        BotCommand::SwitchMode(_new_mode) => HandleResult {
            reply: if language.is_chinese() {
                "当前仅支持专业模式。".to_string()
            } else {
                "Only Expert mode is supported.".to_string()
            },
        ...
        BotCommand::PairingCode(_) => HandleResult {
            reply: if language.is_chinese() {
                "配对码会自动处理。如果你需要重新配对，请在 Ai00-X Desktop 中重新启动连接。"
                    .to_string()
            } else {
                "Pairing codes are handled automatically. If you need to re-pair, please restart the connection from Ai00-X Desktop."
                    .to_string()
            },
            actions: vec![],
            forward_to_session: None,
        ...
        BotCommand::NewCodeSession => {
            ...
            handle_new_session(state, "Core").await
        }
...
pub async fn handle_command(
    ...
    let image_contexts: Vec<crate::agent::image_analysis::ImageContextData> =
...
fn not_paired(language: BotLanguage) -> HandleResult {
    HandleResult {
        reply: if language.is_chinese() {
            "尚未连接到 Ai00-X Desktop。请先发送 6 位配对码。".to_string()
        } else {
            "Not connected to Ai00-X Desktop. Please enter the 6-digit pairing code first."
                .to_string()
        },
        actions: vec![],
        forward_to_session: None,
...
async fn handle_switch_workspace(state: &mut BotChatState) -> HandleResult {
    ...
    if workspaces.is_empty() {
        return HandleResult {
            reply: if language.is_chinese() {
                "未找到工作区。请先在 Ai00-X Desktop 中打开一个项目。".to_string()
            } else {
                "No workspaces found. Please open a project in Ai00-X Desktop first.".to_string()
            },
            actions: vec![],
            forward_to_session: None,
        };
    ...
    if all_meta.is_empty() {
        let reply = if language.is_chinese() {
            if state.display_mode == BotDisplayMode::Pro {
                "当前工作区没有任务。请使用 /new_code_session 或 /new_cowork_session 创建一个。"
                    .to_string()
            } else {
                "当前工作区没有任务。请使用 /new_claw_session 创建一个。".to_string()
            }
        } else {
            if state.display_mode == BotDisplayMode::Pro {
                "No tasks found in this workspace. Use /new_code_session or /new_cowork_session to create one.".to_string()
            } else {
                "No tasks found in this workspace. Use /new_claw_session to create one.".to_string()
            }
        };
        return HandleResult {
            reply,
...
async fn handle_new_session(state: &mut BotChatState, agent_type: &str) -> HandleResult {
    use crate::agent::coordination::get_global_coordinator;
    use crate::agent::core::SessionConfig;
    ...
    let coordinator = match get_global_coordinator() {
        ...
        None => {
            return HandleResult {
                reply: if language.is_chinese() {
                    "Ai00-X 任务系统尚未就绪。".to_string()
                } else {
                    "Ai00-X task system not ready.".to_string()
                },
                actions: vec![],
                forward_to_session: None,
            };
...
fn build_workspace_switched_reply(
    ...
    if session_count > 0 {
        if language.is_chinese() {
            reply.push_str(&format!(
                "这个工作区已有 {session_count} 个任务。你想做什么？\n\n"
            ));
        } else {
            let s = if session_count == 1 { "" } else { "s" };
            reply.push_str(&format!(
                "This workspace has {session_count} existing task{s}. What would you like to do?\n\n"
            ));
        }
    } else {
        if language.is_chinese() {
            reply.push_str("这个工作区还没有任务。你想做什么？\n\n");
        } else {
            reply.push_str("No tasks found in this workspace. What would you like to do?\n\n");
        }
    }
    ...
    if is_pro {
        if language.is_chinese() {
            reply.push_str(
                "/resume_session - 恢复已有任务\n\
                 /new_code_session - 开始新的编码任务\n\
                 /new_cowork_session - 开始新的协作任务\n\
                 /assistant - 切换到助理模式",
            );
        } else {
            reply.push_str(
                "/resume_session - Resume an existing task\n\
                 /new_code_session - Start a new coding task\n\
                 /new_cowork_session - Start a new cowork task\n\
        ...
        if language.is_chinese() {
            reply.push_str(
                "/resume_session - 恢复已有任务\n\
                 /new_claw_session - 开始新的助理任务\n\
                 /pro - 切换到专业模式",
            );
        } else {
            reply.push_str(
                "/resume_session - Resume an existing task\n\
                 /new_claw_session - Start a new claw task\n\
...
async fn select_session(
    ...
    let mut reply = if language.is_chinese() {
        format!("已恢复任务：{session_name}\n\n")
    } else {
        format!("Resumed task: {session_name}\n\n")
    };
...
fn strip_user_message_tags(raw: &str) -> String {
    crate::agent::core::strip_prompt_markup(raw)
}
...
async fn handle_cancel_task(
    ...
    let session_id = match state.current_session_id.clone() {
        ...
        None => {
            return HandleResult {
                reply: if language.is_chinese() {
                    "当前没有可取消的活动任务。".to_string()
                } else {
                    "No active task to cancel.".to_string()
                },
...
async fn submit_question_answers(tool_id: &str, answers: &[Value]) -> HandleResult {
    use crate::agent::tools::user_input_manager::get_user_input_manager;
...
async fn handle_chat_message(
    ...
    if let Some(pending) = state.pending_action.clone() {
        return match pending {
            ...
            PendingAction::SelectSession { has_more, .. } => HandleResult {
                reply: if has_more {
                    if language.is_chinese() {
                        "请回复任务编号，或回复 `0` 查看下一页。".to_string()
                    } else {
                        "Please reply with the task number, or `0` for the next page.".to_string()
                    }
                } else {
                    if language.is_chinese() {
                        "请回复任务编号。".to_string()
                    } else {
                        "Please reply with the task number.".to_string()
                    }
                },
                actions: vec![],
                forward_to_session: None,
    ...
    if state.current_session_id.is_none() {
        let reply = if language.is_chinese() {
            if state.display_mode == BotDisplayMode::Pro {
                "当前没有活动任务。请使用 /resume_session 恢复已有任务，或使用 /new_code_session、/new_cowork_session 创建新任务。"
                    .to_string()
            } else {
                "当前没有活动任务。请使用 /resume_session 恢复已有任务，或使用 /new_claw_session 创建新任务。"
                    .to_string()
            }
        } else {
            if state.display_mode == BotDisplayMode::Pro {
                "No active task. Use /resume_session to resume one or /new_code_session, /new_cowork_session to create a new one."
                    .to_string()
            } else {
                "No active task. Use /resume_session to resume one or /new_claw_session to create a new one."
                    .to_string()
            }
        };
        return HandleResult {
...
pub async fn execute_forwarded_turn(
    ...
    use crate::agent::coordination::{DialogSubmissionPolicy, DialogTriggerSource};
