...
//! (LAN / ngrok / Ai00-X Server / Custom Server).  Calling `stop()` only
...
impl Default for RemoteConnectConfig {
    fn default() -> Self {
        Self {
            lan_port: 9700,
            // Ai00-S serves relay at /ai00-s/relay/* under its root
            ai00_x_server_url: crate::service::config::server_endpoints::ai00_s_relay_url(),
            web_app_url: crate::service::config::server_endpoints::ai00_s_web_app_url(),
...
impl RemoteConnectService {
    ...
    ///
    /// For relay methods (LAN / ngrok / Ai00-X Server / Custom Server) this
    ...
    pub async fn start(&self, method: ConnectionMethod) -> Result<ConnectionResult> {
        ...
        let relay_url = match &method {
            ...
            ConnectionMethod::Ai00XServer => self.config.ai00_x_server_url.clone(),
        ...
        let ws_url = match &method {
            ConnectionMethod::Lan | ConnectionMethod::Ngrok => {
                format!("ws://127.0.0.1:{}/ai00-s/relay/ws", self.config.lan_port)
            }
            _ => {
                format!(
                    "{}/ai00-s/relay/ws",
    ...
    /// Stop relay connections (LAN / ngrok / Ai00-X Server / Custom Server).
...
async fn upload_mobile_web(relay_url: &str, room_id: &str, web_dir: &str) -> Result<()> {
    ...
    let check_url = format!("{relay_base}/ai00-s/relay/rooms/{room_id}/check-web-files");
...
async fn upload_needed_files(
    ...
    let url = format!("{relay_base}/ai00-s/relay/rooms/{room_id}/upload-web-files");
...
async fn upload_all_files(
    ...
    let url = format!("{relay_base}/ai00-s/relay/rooms/{room_id}/upload-web");
