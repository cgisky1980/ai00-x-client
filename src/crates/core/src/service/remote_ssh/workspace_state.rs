...
/// This returns a dedicated stub under `~/.ai00-x/remote_ssh/_unresolved/.../sessions` that is
...
impl RemoteWorkspaceStateManager {
    ...
    /// Local mirror directory for persisted sessions (`~/.ai00-x/remote_ssh/.../sessions`).
...
mod tests {
    ...
    async fn local_assistant_path_not_remote_without_connection_id() {
        ...
        let assistant_path = pm.user_agents_dir().to_string_lossy().to_string();
        ...
        assert!(
            ...
            "local path must not bind to SSH when remote_connection_id is omitted"
        );
    }
    ...
    fn local_stable_id_is_deterministic_and_prefixed() {
        let id1 = super::local_workspace_stable_storage_id("/Users/foo/Ai00-X");
        let id2 = super::local_workspace_stable_storage_id("/Users/foo/Ai00-X");
        assert_eq!(id1, id2);
