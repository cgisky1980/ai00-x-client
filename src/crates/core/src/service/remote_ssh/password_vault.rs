...
//!
//! A random 32-byte key lives in `<secrets_dir>/.ssh_password_vault.key` (0600 on Unix).
//! Ciphertext map is stored in `<secrets_dir>/ssh_password_vault.json`.
//!
//! The `secrets_dir` (typically `<exe_dir>/data/ssh_secrets/`) is distinct from the
//! sync-able SSH connections dir (`<exe_dir>/data/profile/ssh/`) so that passwords
//! are never transported during profile sync.
