...
impl AIMemoryManager {
    ...
    pub async fn new(path_manager: Arc<PathManager>) -> Ai00XResult<Self> {
        ...
        if let Some(parent) = storage_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                Ai00XError::io(format!("Failed to create memory storage directory: {}", e))
            })?;
        }
    ...
    async fn load_storage(path: &PathBuf) -> Ai00XResult<MemoryStorage> {
        let content = fs::read_to_string(path)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to read memory storage file: {}", e)))?;

        let storage: MemoryStorage = serde_json::from_str(&content).map_err(|e| {
            Ai00XError::Deserialization(format!("Failed to deserialize memory storage: {}", e))
        })?;
    ...
    async fn save_storage(&self) -> Ai00XResult<()> {
        let storage = self.storage.read().await;
        let content = serde_json::to_string_pretty(&*storage).map_err(|e| {
            Ai00XError::serialization(format!("Failed to serialize memory storage: {}", e))
        })?;

        fs::write(&self.storage_path, content)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to write memory storage file: {}", e)))?;
    ...
    /// Adds a memory point.
    pub async fn add_memory(&self, memory: AIMemory) -> Ai00XResult<AIMemory> {
    ...
    pub async fn delete_memory(&self, id: &str) -> Ai00XResult<bool> {
        let mut storage = self.storage.write().await;
    ...
    pub async fn update_memory(&self, memory: AIMemory) -> Ai00XResult<bool> {
        let mut storage = self.storage.write().await;
    ...
    pub async fn get_all_memories(&self) -> Ai00XResult<Vec<AIMemory>> {
        let storage = self.storage.read().await;
        Ok(storage.memories.clone())
    }

    /// Returns enabled memory points.
    pub async fn get_enabled_memories(&self) -> Ai00XResult<Vec<AIMemory>> {
        let storage = self.storage.read().await;
        Ok(storage
    ...
    pub async fn get_memories_for_prompt(&self) -> Ai00XResult<Option<String>> {
        ...
        sorted_memories.sort_by_key(|b| std::cmp::Reverse(b.importance));
    ...
    pub async fn toggle_memory(&self, id: &str) -> Ai00XResult<bool> {
        let mut storage = self.storage.write().await;
