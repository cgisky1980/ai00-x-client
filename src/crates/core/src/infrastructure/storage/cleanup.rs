...
impl CleanupService {
    ...
    pub async fn cleanup_all(&self) -> Ai00XResult<CleanupResult> {
    ...
    async fn cleanup_temp_files(&self) -> Ai00XResult<CleanupResult> {
    ...
    async fn cleanup_old_logs(&self) -> Ai00XResult<CleanupResult> {
    ...
    async fn cleanup_oversized_cache(&self) -> Ai00XResult<CleanupResult> {
    ...
    async fn cleanup_by_size(&self, dir: &Path, max_size: u64) -> Ai00XResult<CleanupResult> {
        let mut result = CleanupResult::default();
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Ai00XResult<()>> + Send + 'a>>
    where
        F: Fn(&std::fs::Metadata) -> bool + Copy + Send + 'a,
    {
        Box::pin(async move {
            ...
            while let Some(entry) = read_dir
                .next_entry()
                .await
                .map_err(|e| Ai00XError::service(format!("Failed to read entry: {}", e)))?
            {
                let path = entry.path();
    ...
    fn calculate_dir_size(
        dir: &Path,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Ai00XResult<u64>> + Send + '_>> {
        Box::pin(async move {
            let mut total = 0u64;
