...
impl FileOperationService {
    ...
    pub async fn read_file(&self, file_path: &str) -> Ai00XResult<FileReadResult> {
        ...
        let metadata = fs::metadata(path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to read file metadata: {}", e)))?;
    ...
    pub async fn editor_sync_content_sha256_hex(&self, file_path: &str) -> Ai00XResult<String> {
        let path = Path::new(file_path);
    ...
    pub async fn write_file(
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                Ai00XError::service(format!("Failed to create parent directory: {}", e))
            })?;
        }
    ...
    pub async fn write_binary_file(
        ...
        fs::write(path, data)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to write binary file: {}", e)))?;
    ...
    pub async fn copy_file(&self, from: &str, to: &str) -> Ai00XResult<u64> {
        ...
        if let Some(parent) = to_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                Ai00XError::service(format!("Failed to create target directory: {}", e))
            })?;
        }
    ...
    pub async fn move_file(&self, from: &str, to: &str) -> Ai00XResult<()> {
        let from_trim = from.trim();
        let to_trim = to.trim();
        let from_path = Path::new(from_trim);
        let to_path = Path::new(to_trim);
        ...
        fs::rename(from_path, to_path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to move file: {}", e)))?;

        Ok(())
    }

    pub async fn delete_file(&self, file_path: &str) -> Ai00XResult<()> {
        let path = Path::new(file_path);
    ...
    pub async fn get_file_info(&self, file_path: &str) -> Ai00XResult<FileInfo> {
        let path = Path::new(file_path);
    ...
    pub async fn create_directory(&self, dir_path: &str) -> Ai00XResult<()> {
    ...
    pub async fn delete_directory(&self, dir_path: &str, recursive: bool) -> Ai00XResult<()> {
        let path = Path::new(dir_path);
        ...
        if !path.is_dir() {
            return Err(Ai00XError::service("Path is not a directory".to_string()));
        }

        if recursive {
            fs::remove_dir_all(path).await.map_err(|e| {
                Ai00XError::service(format!("Failed to delete directory recursively: {}", e))
            })?;
        } else {
            fs::remove_dir(path)
                .await
                .map_err(|e| Ai00XError::service(format!("Failed to delete directory: {}", e)))?;
        }

        Ok(())
    }

    async fn validate_file_access(&self, path: &Path, is_write: bool) -> Ai00XResult<()> {
    ...
    async fn create_backup(&self, path: &Path) -> Ai00XResult<String> {
        ...
        fs::copy(path, &backup_path)
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to create backup: {}", e)))?;
