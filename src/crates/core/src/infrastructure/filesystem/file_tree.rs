...
impl FileTreeService {
    ...
    pub async fn build_tree_with_stats(
        &self,
        root_path: &str,
    ) -> Ai00XResult<(Vec<FileTreeNode>, FileTreeStatistics)> {
        ...
        if !root_path_buf.exists() {
            return Err(Ai00XError::service("Directory does not exist".to_string()));
        }
    ...
    fn should_skip_file(&self, file_name: &str) -> bool {
        ...
        // But .gitignore and .ai00-x are always shown
    ...
    fn compile_search_regex(
        ...
        RegexBuilder::new(&search_pattern)
            ...
            .build()
            .map_err(|error| Ai00XError::service(format!("Invalid regex pattern: {}", error)))
    }
    ...
    fn search_file_content_lines(
        ...
        let file = File::open(path)
            .map_err(|error| Ai00XError::service(format!("Failed to open file: {}", error)))?;
        ...
        for (index, line_result) in reader.split(b'\n').enumerate() {
            ...
            let line_bytes = line_result
                .map_err(|error| Ai00XError::service(format!("Failed to read file: {}", error)))?;
