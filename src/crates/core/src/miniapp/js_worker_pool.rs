...
impl JsWorkerPool {
    pub fn new(
        ...
        let runtime = detect_runtime().ok_or_else(|| {
            Ai00XError::validation("No JS runtime found (install Bun or Node.js)".to_string())
        })?;
    ...
    ) -> Ai00XResult<Arc<Mutex<JsWorker>>> {
    ...
    pub async fn install_deps(&self, app_id: &str, _deps: &[NpmDep]) -> Ai00XResult<InstallResult> {
        let app_dir = self.path_manager.miniapp_dir(app_id);
        ...
        let output = process_manager::create_tokio_command(cmd)
            ...
            .output()
            .await
            .map_err(|e| Ai00XError::io(format!("install_deps failed: {}", e)))?;
