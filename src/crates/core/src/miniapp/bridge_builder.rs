//! Bridge script builder — generate window.app Runtime Adapter (Ai00-X Hosted) for iframe.
...
(function() {{
  ...
  window.parent.postMessage({{ method: 'ai00-x/request-theme' }}, '*');
}})();
"#,
...
pub fn scroll_boundary_script() -> &'static str {
    r#"<script>(()=>{const s=(e)=>{for(let n=e.target;n;n=n.parentNode){if(!(n instanceof Element))continue;if(n===document.documentElement||n===document.body)continue;const o=window.getComputedStyle(n).overflowY;if(o==='hidden'||o==='visible')continue;if(e.deltaY<0&&n.scrollTop>0)return false;if(e.deltaY>0&&n.scrollTop+n.clientHeight<n.scrollHeight)return false;}return true};window.addEventListener('wheel',e=>{if(!e.defaultPrevented&&s(e))window.parent.postMessage({jsonrpc:'2.0',method:'ai00-x/sandbox-wheel',params:{deltaX:e.deltaX,deltaY:e.deltaY,deltaZ:e.deltaZ,deltaMode:e.deltaMode}},'*')},{passive:true});})();</script>"#
}
...
pub fn build_miniapp_default_theme_css() -> &'static str {
    r#"<style id="ai00-x-theme-default">:root{--ai00-x-bg:#121214;--ai00-x-bg-secondary:#18181a;--ai00-x-bg-tertiary:#121214;--ai00-x-bg-elevated:#18181a;--ai00-x-text:#e8e8e8;--ai00-x-text-secondary:#b0b0b0;--ai00-x-text-muted:#858585;--ai00-x-accent:#60a5fa;--ai00-x-accent-hover:#3b82f6;--ai00-x-success:#34d399;--ai00-x-warning:#f59e0b;--ai00-x-error:#ef4444;--ai00-x-info:#E1AB80;--ai00-x-border:#2e2e32;--ai00-x-border-subtle:#27272a;--ai00-x-element-bg:#27272a;--ai00-x-element-hover:#3f3f46;--ai00-x-radius:6px;--ai00-x-radius-lg:10px;--ai00-x-font-sans:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;--ai00-x-font-mono:ui-monospace,SFMono-Regular,'SF Mono',Menlo,Consolas,monospace;--ai00-x-scrollbar-thumb:rgba(255,255,255,0.12);}</style>"#
}
