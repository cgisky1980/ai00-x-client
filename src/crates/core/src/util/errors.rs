...
/// Unified error type for the Ai00-X application
...
impl From<Ai00XError> for String {
    fn from(err: Ai00XError) -> String {
        err.to_string()
    }
}

impl From<String> for Ai00XError {
    fn from(error: String) -> Self {
        Ai00XError::Service(error)
    }
}

impl From<&str> for Ai00XError {
    fn from(error: &str) -> Self {
        Ai00XError::Service(error.to_string())
    }
}

impl From<tokio::sync::AcquireError> for Ai00XError {
    fn from(error: tokio::sync::AcquireError) -> Self {
        Ai00XError::Semaphore(error.to_string())
    }
}
