...
use super::types::{AgentEvent, EventEnvelope};
...
impl EventRouter {
    ...
    pub async fn route(&self, envelope: EventEnvelope) -> Ai00XResult<()> {
    ...
    pub async fn route_batch(&self, envelopes: Vec<EventEnvelope>) -> Ai00XResult<()> {
        // First collect subscribers list (avoid holding DashMap iterator across await points)
        let subscribers: Vec<(String, Arc<dyn EventSubscriber>)> = self
            .internal_subscribers
            .iter()
