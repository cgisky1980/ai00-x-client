...
use crate::agent::tools::permission::PermissionLevel;
use crate::agent::tools::workspace_paths::{
    build_ai00x_runtime_uri, is_ai00x_runtime_uri, normalize_runtime_relative_path,
    parse_ai00x_runtime_uri,
};
...
// Re-export types from ai00-x-tool-framework (shared across crates)
pub use ai00_x_tool_framework::{
    PermissionLevel as ToolFrameworkPermissionLevel, ToolError, ToolImageAttachment,
    ToolRenderOptions,
};
pub use ai00_x_tool_framework::{ToolResult, ValidationResult};
...
impl ToolPathResolution {
    ...
    pub fn logical_child_path(&self, absolute_child_path: &Path) -> Option<String> {
        ...
        build_ai00x_runtime_uri(scope, &relative_str).ok()
    }
}
...
pub struct ToolUseContext {
    ...
    /// Desktop automation (Computer use); only set in Ai00-X desktop.
    pub computer_use_host: Option<crate::agent::tools::computer_use_host::ComputerUseHostRef>,
...
impl ToolUseContext {
    ...
    pub fn ws_fs(&self) -> Option<&dyn crate::agent::workspace::WorkspaceFileSystem> {
        self.workspace_services.as_ref().map(|s| s.fs.as_ref())
    }

    pub fn ws_shell(&self) -> Option<&dyn crate::agent::workspace::WorkspaceShell> {
    ...
    pub fn resolve_workspace_tool_path(&self, path: &str) -> Ai00XResult<String> {
        let workspace_root_owned = self.workspace.as_ref().map(|w| w.root_path_string());
        crate::agent::tools::workspace_paths::resolve_workspace_tool_path(
            path,
    ...
    pub fn current_workspace_runtime_root(&self) -> Ai00XResult<PathBuf> {
        let workspace = self.workspace.as_ref().ok_or_else(|| {
            crate::util::errors::Ai00XError::tool(
    ...
    pub async fn ensure_current_workspace_runtime(&self) -> Ai00XResult<WorkspaceRuntimeContext> {
        let workspace = self.workspace.as_ref().ok_or_else(|| {
            crate::util::errors::Ai00XError::tool(
    ...
    pub fn build_runtime_uri(&self, relative_path: &str) -> Ai00XResult<String> {
        let scope = self
            .current_workspace_scope()
            .unwrap_or_else(|| "current".to_string());
        build_ai00x_runtime_uri(&scope, &normalize_runtime_relative_path(relative_path)?)
    }

    pub fn build_runtime_artifact_reference(&self, relative_path: &str) -> Ai00XResult<String> {
    ...
    pub fn current_workspace_session_dir(&self, session_id: &str) -> Ai00XResult<PathBuf> {
        Ok(self
    ...
    pub fn resolve_tool_path(&self, path: &str) -> Ai00XResult<ToolPathResolution> {
        if is_ai00x_runtime_uri(path) {
            let parsed = parse_ai00x_runtime_uri(path)?;
            ...
            let logical_path = build_ai00x_runtime_uri(&effective_scope, &parsed.relative_path)?;
    ...
    pub fn workspace_path_is_effectively_absolute(&self, path: &str) -> bool {
        ...
        if self.is_remote() {
            crate::agent::tools::workspace_paths::posix_style_path_is_absolute(path)
        } else {
...
// ValidationResult: re-exported from ai00-x-tool-framework (see top of file)

// ToolResult: re-exported from ai00-x-tool-framework (see top of file)
...
pub trait Tool: Send + Sync {
    ...
    async fn call(&self, input: &Value, context: &ToolUseContext) -> Ai00XResult<Vec<ToolResult>> {
        if let Some(cancellation_token) = context.cancellation_token.as_ref() {
            tokio::select! {
                ...
                _ = cancellation_token.cancelled() => {
                    Err(crate::util::errors::Ai00XError::Cancelled("Tool execution cancelled".to_string()))
                }
            }
        } else {
...
// ToolRenderOptions: re-exported from ai00-x-tool-framework (see top of file)
