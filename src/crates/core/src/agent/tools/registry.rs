...
use crate::agent::tools::framework::Tool;
use crate::agent::tools::implementations::*;
...
impl ToolRegistry {
    ...
    fn register_all_tools(&mut self) {
        ...
        // Wait tool — pause for rate-limit resets or background polling
        self.register_tool(Arc::new(WaitTool::new()));
        ...
        self.register_tool(Arc::new(WebAdapterManagerTool::new()));
        ...
        // SelfControl — operates Ai00-X's own GUI (kept for backward compatibility)
        self.register_tool(Arc::new(SelfControlTool::new()));

        // ComputerUse — desktop automation (kept for backward compatibility)
        // Currently disabled — no VLM/visual control planned.
        ...
        // MCPDeactivate — manually deactivate MCP server tools
        self.register_tool(Arc::new(MCPDeactivateTool::new()));

        // Wallpaper tools — AI-powered wallpaper creation
        self.register_tool(Arc::new(ListMyWallpapersTool::new()));
    }
...
/// Get readonly tools
pub async fn get_readonly_tools() -> Ai00XResult<Vec<Arc<dyn Tool>>> {
