//! Host abstraction for desktop automation (implemented in `ai00-x-desktop`).

// Re-export optimizer types so downstream crates can import from computer_use_host.
pub use crate::agent::tools::computer_use_optimizer::{ActionRecord, LoopDetectionResult};
...
pub trait ComputerUseHost: Send + Sync + std::fmt::Debug {
    ...
    async fn accessibility_hit_at_global_point(
        ...
        _gx: f64,
        _gy: f64,
    ) -> Ai00XResult<Option<OcrAccessibilityHit>> {
        Ok(None)
    }
    ...
    /// `button`: "left" | "right" | "middle"
    async fn mouse_down(&self, _button: &str) -> Ai00XResult<()> {
        Err(Ai00XError::tool(
    ...
    /// `button`: "left" | "right" | "middle"
    async fn mouse_up(&self, _button: &str) -> Ai00XResult<()> {
        Err(Ai00XError::tool(
    ...
    async fn open_app(&self, _app_name: &str) -> Ai00XResult<OpenAppResult> {
        Err(Ai00XError::tool(
