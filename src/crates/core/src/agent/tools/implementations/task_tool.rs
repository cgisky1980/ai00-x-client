use crate::agent::agents::{get_agent_registry, AgentInfo};
...
use crate::agent::tools::InputValidator;
...
When you MUST spawn subagents:
- **Parallelization**: when you have two or more independent items to work on, and each item may involve multiple steps
- **Context-hiding**: when you wish to accomplish a high-token-cost subtask without distraction from the main task
- **Specialization**: when a task requires expertise from a specific subagent (e.g., CodeReview for quality audit, debug for evidence-driven debugging, DeepResearch for extensive research)

Subagent calling conventions — always include in the prompt:
1. **Task description**: What the subagent should accomplish
2. **Constraints**: What the subagent must NOT do (e.g., "do not modify files", "do not run commands")
3. **Success criteria**: How to determine the task is complete
4. **Expected output format**: How the subagent should structure its response

CodeReview parameterization for multi-perspective review:
To run a review from a specific perspective, inject the perspective context into the prompt:
```
Task(
  subagent_type: "CodeReview",
  description: "Multi-perspective review: [perspective name]",
  prompt: "Review the following changes specifically from the perspective of [perspective name].
PERSPECTIVE CONTEXT: [Detailed description of what this perspective focuses on]
CHANGES TO REVIEW: [Description of the changes, or instruction to use GetFileDiff tool]
Focus your review on concerns relevant to this perspective."
)
```
You can run multiple perspective reviews in parallel by sending multiple Task calls in a single message.
...
impl Tool for TaskTool {
    ...
    async fn call_impl(
        ...
        let subagent_type = input
            .get("subagent_type")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("Required parameters: subagent_type, prompt, description. Missing subagent_type".to_string()))?
            .to_string();
