...
Usage:
- The path parameter must be either an absolute path or an exact `ai00-x://runtime/...` URI returned by another tool
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "The absolute path to the directory to list, or an exact ai00-x://runtime URI returned by another tool"
                },
                "limit": {
                    "type": "number",
    ...
    async fn call_impl(
        ...
        if resolved.uses_remote_workspace_backend() {
            let ws_shell = context.ws_shell().ok_or_else(|| {
                Ai00XError::tool("Workspace shell not available for remote LS".to_string())
            })?;
            ...
            let (stdout, _stderr, _exit_code) = ws_shell
                ...
                .await
                .map_err(|e| Ai00XError::tool(format!("Failed to list remote directory: {}", e)))?;
        ...
        let entries =
            list_directory_entries(&resolved.resolved_path, limit).map_err(Ai00XError::tool)?;
