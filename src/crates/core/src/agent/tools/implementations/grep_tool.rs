...
impl GrepTool {
    ...
    async fn call_remote(
        ...
        let ws_shell = context
            .ws_shell()
            .ok_or_else(|| Ai00XError::tool("Workspace shell not available".to_string()))?;

        let pattern = input
            .get("pattern")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("pattern is required".to_string()))?;
        ...
        let (stdout, _stderr, _exit_code) = ws_shell
            ...
            .await
            .map_err(|e| Ai00XError::tool(format!("Remote grep failed: {}", e)))?;

        let lines: Vec<&str> = stdout.lines().collect();
        let total_matches = lines.len();
        let file_count: usize = if lines.is_empty() {
            0
        } else {
        ...
        Ok(vec![ToolResult::Result {
            ...
            result_for_assistant: Some({
                let _sid = context.session_id.as_deref().unwrap_or("unknown");
                let _tid = context.dialog_turn_id.as_deref().unwrap_or("unknown");
                // TODO: RWKV grep organize temporarily disabled — re-enable and verify individually later
                // organize_grep_results_with_rwkv(
                //     pattern,
                //     &result_text,
                //     file_count,
                //     total_matches,
    ...
    fn build_grep_options(
        ...
        let output_mode_str = input
            ...
            .unwrap_or("files_with_matches");
        let output_mode =
            OutputMode::from_str(output_mode_str).map_err(|e| Ai00XError::tool(e.to_string()))?;
        let show_line_numbers = input
            .get("-n")
            .and_then(|v| v.as_bool())
...
#[allow(dead_code)]
async fn organize_grep_results_with_rwkv(
    pattern: &str,
    result_text: &str,
    file_count: usize,
    total_matches: usize,
    ...
    if total_matches <= 20 && file_count <= 5 {
        return None;
    }
    ...
    let factory =
        match crate::infrastructure::ai::client_factory::get_global_ai_client_factory().await {
            Ok(f) => f,
            Err(e) => {
                warn!("Grep organize: AIClientFactory not available: {}", e);
                return None;
            }
        };
    ...
    let prompt = format!(
        "You are a grep result organizer. Given a search pattern and grep results ({total} matches across {files} files), organize matches into a structured summary.\n\
         Search pattern: {pattern}\n\
         Input: {files} files, {total} total matches\n\
         Rules:\n\
         - Group matches by file, showing the file path and match count per file\n\
         - For files with > 10 matches: show the most representative 3-5 matches\n\
         - For files with <= 10 matches: keep all significant lines\n\
         - Identify semantic patterns (e.g., function definitions vs. function calls vs. imports)\n\
         - Skip duplicate/near-identical lines from test fixtures or generated code\n\
         - Do NOT add code that is not present in the grep results\n\
         - Output format: each file as a Markdown section with most relevant matches first\n\
         Raw grep results:\n{text}",
        total = total_matches,
        files = file_count,
        pattern = pattern,
    ...
    match client.send_message(vec![message], None).await {
        Ok(response) => {
            ...
            let summary = format!(
                "[Grep organized: {} matches in {} files]\n{}",
                total_matches, file_count, text
            );
            Some(summary)
        }
        Err(e) => {
            warn!("Grep organize: RWKV call failed: {}", e);
            None
        }
    }
}
...
- The path parameter may be an absolute path or an exact `ai00-x://runtime/...` URI returned by another tool
...
- Multiline matching: By default patterns match within single lines only. For cross-line patterns like `struct \{[\s\S]*?field`, use `multiline: true`"#.to_string())
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "path": {
                    "type": "string",
                    "description": "File or directory to search in (rg PATH). Defaults to current working directory. May be an absolute path or an exact ai00-x://runtime URI."
                },
                "glob": {
                    "type": "string",
    ...
    fn render_tool_use_message(
        &self,
        input: &Value,
        _options: &crate::agent::tools::framework::ToolRenderOptions,
    ) -> String {
    ...
    async fn call_impl(
        ...
        let GrepSearchResult {
            ...
            Ok(Ok(result)) => result,
            Ok(Err(e)) => return Err(Ai00XError::tool(e)),
            Err(e) => return Err(Ai00XError::tool(format!("grep search failed: {}", e))),
        };
