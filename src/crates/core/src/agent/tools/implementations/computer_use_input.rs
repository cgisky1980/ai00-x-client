...
pub fn ensure_pointer_move_uses_screen_coordinates_only(input: &Value) -> Ai00XResult<()> {
...
pub fn parse_screenshot_crop_center(input: &Value) -> Ai00XResult<Option<ScreenshotCropCenter>> {
    ...
    match (x_none, y_none) {
        ...
        (false, false) => {
            let x = xv
                .and_then(|v| v.as_u64())
                .ok_or_else(|| Ai00XError::tool("screenshot_crop_center_x must be a non-negative integer (full-display native pixels).".to_string()))?;
            let y = yv
                .and_then(|v| v.as_u64())
                .ok_or_else(|| Ai00XError::tool("screenshot_crop_center_y must be a non-negative integer (full-display native pixels).".to_string()))?;
            Ok(Some(ScreenshotCropCenter {
                x: u32::try_from(x)
                    .map_err(|_| Ai00XError::tool("screenshot_crop_center_x is too large.".to_string()))?,
                y: u32::try_from(y)
                    .map_err(|_| Ai00XError::tool("screenshot_crop_center_y is too large.".to_string()))?,
            }))
        }
...
pub fn parse_screenshot_crop_half_extent_native(input: &Value) -> Ai00XResult<Option<u32>> {
    match input.get("screenshot_crop_half_extent_native") {
        ...
        Some(v) => {
            ...
            Ok(Some(u32::try_from(n).map_err(|_| {
                Ai00XError::tool("screenshot_crop_half_extent_native is too large.".to_string())
            })?))
        }
    }
}
...
pub fn parse_screenshot_implicit_center(
    input: &Value,
) -> Ai00XResult<Option<ComputerUseImplicitScreenshotCenter>> {
    match input
        ...
        .map(str::trim)
    {
        ...
        Some(other) => Err(Ai00XError::tool(format!(
...
pub fn parse_screenshot_navigate_quadrant(
    input: &Value,
) -> Ai00XResult<Option<ComputerUseNavigateQuadrant>> {
    let value = input
...
pub fn parse_screenshot_params(input: &Value) -> Ai00XResult<(ComputerUseScreenshotParams, bool)> {
