...
- Use `replace_all` for replacing and renaming strings across the file. This parameter is useful if you want to rename a variable for instance."#
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "The absolute path to the file to modify, or an exact ai00-x://runtime URI returned by another tool"
                },
                "old_string": {
                    "type": "string",
                    "default": "",
    ...
    async fn call_impl(
        ...
        let new_string = input
            .get("new_string")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("new_string is required".to_string()))?;

        let old_string = input
            .get("old_string")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("old_string is required".to_string()))?;
        ...
        atomic_write(resolved_path, edit_result.new_content.as_bytes())
            .await
            .map_err(|e| {
                Ai00XError::tool(format!(
