...
impl GitTool {
        ...
        if let Some(dir) = working_directory {
            ...
            if trimmed.is_empty() {
                return context
                    ...
                    .ok_or_else(|| Ai00XError::tool("No workspace path available".to_string()));
            }
    ...
    async fn execute_remote_git_cli(
        ...
        let shell = context.ws_shell().ok_or_else(|| {
            Ai00XError::tool("Remote Git requires workspace shell (SSH)".to_string())
        })?;
        ...
        let (stdout, stderr, exit_code) = shell
            ...
            .await
            .map_err(|e| Ai00XError::tool(format!("Remote git failed: {}", e)))?;
    ...
    async fn execute_status(repo_path: &str) -> Ai00XResult<Value> {
        let status = GitService::get_status(repo_path)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git status failed: {}", e)))?;
    ...
    async fn execute_diff(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        ...
        let diff_output = GitService::get_diff(repo_path, &params)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git diff failed: {}", e)))?;
    ...
    async fn execute_log(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let commits = GitService::get_commits(repo_path, params)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git log failed: {}", e)))?;
    ...
    async fn execute_add(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or(".");
        ...
        let result = GitService::add_files(repo_path, params)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git add failed: {}", e)))?;
    ...
    async fn execute_commit(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let result = GitService::commit(repo_path, params)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git commit failed: {}", e)))?;
    ...
    async fn execute_push(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let result = GitService::push(repo_path, params)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git push failed: {}", e)))?;
    ...
    async fn execute_pull(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or("");
        let parts: Vec<&str> = args_str
            .split_whitespace()
            .filter(|s| !s.starts_with('-'))
        ...
        let result = GitService::pull(repo_path, params)
            .await
            .map_err(|e| Ai00XError::tool(format!("Git pull failed: {}", e)))?;
    ...
    async fn execute_checkout(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let branch_name = args_str
            ...
            .ok_or_else(|| Ai00XError::tool("Branch name is required".to_string()))?;
        ...
        .map_err(|e| Ai00XError::tool(format!("Git checkout failed: {}", e)))?;
    ...
    async fn execute_branch(repo_path: &str, args: Option<&str>) -> Ai00XResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        if is_list {
            ...
            let branches = GitService::get_branches(repo_path, include_remote)
                .await
                .map_err(|e| Ai00XError::tool(format!("Git branch failed: {}", e)))?;
            ...
            let branch_name = args_str
                ...
                .ok_or_else(|| {
                    Ai00XError::tool("Branch name is required for deletion".to_string())
                })?;

            let result = GitService::delete_branch(repo_path, branch_name, force)
                .await
                .map_err(|e| Ai00XError::tool(format!("Git branch delete failed: {}", e)))?;
    ...
    #[allow(dead_code)]
    async fn summarize_git_output_with_rwkv(
        operation: &str,
        raw_output: &str,
        session_id: &str,
        turn_id: &str,
    ) -> Option<String> {
        let output_len = raw_output.chars().count();
        let should_summarize = match operation {
            "diff" => output_len > 500,
            "log" => output_len > 500,
            _ => return None,
        };
        ...
        let factory =
            match crate::infrastructure::ai::client_factory::get_global_ai_client_factory().await {
                Ok(f) => f,
                Err(e) => {
                    warn!("Git summary: AIClientFactory not available: {}", e);
                    return None;
                }
            };
        ...
        let prompt = format!(
            "You are a git output summarizer. Below is the raw output of a `git {}` command. \
             Summarize it concisely for a developer to understand at a glance.\n\
             Rules:\n\
             - For diff: list which files changed, what functions/structs/modules were added/modified/removed, and the overall intent of the changes\n\
             - For log: extract the key commits with hash, author, date, and summary. Group related commits.\n\
             - Keep the summary under 300 words\n\
             - Do NOT add information not present in the output\n\
             - Output as clean Markdown\n\
             Raw git {} output:\n{}",
            operation, operation, raw_output
        );

        let message = crate::util::types::message::Message::user(prompt);
        match client.send_message(vec![message], None).await {
            Ok(response) => {
                ...
                let summary = format!("[Git {} summary]\n{}", operation, text);
                Some(summary)
            }
            Err(e) => {
                warn!("Git summary: RWKV call failed: {}", e);
                None
            }
        }
    }
...
- End with:
  ...
  Generated with Ai00-X

  Co-Authored-By: Ai00-X"#.to_string())
    }
    ...
    async fn call_impl(
        ...
        let operation = input
            .get("operation")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("operation is required".to_string()))?;
        ...
        // TODO: RWKV git summary temporarily disabled — re-enable and verify individually later
        // if operation == "diff" || operation == "log" {
        //     if let Some(stdout) = result_with_meta.get("stdout").and_then(|v| v.as_str()) {
        //         let sid = context.session_id.as_deref().unwrap_or("unknown");
        //         let tid = context.dialog_turn_id.as_deref().unwrap_or("unknown");
        //         if let Some(summary) =
        //             Self::summarize_git_output_with_rwkv(operation, stdout, sid, tid).await
        //         {
