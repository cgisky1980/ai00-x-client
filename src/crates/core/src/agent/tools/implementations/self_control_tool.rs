...
/// SelfControl tool — lets the Ai00-X agent operate its own GUI.
///
...
pub async fn submit_self_control_response(response: SelfControlResponse) -> Ai00XResult<()> {
...
impl Tool for SelfControlTool {
    ...
    async fn description(&self) -> Ai00XResult<String> {
        Ok(
            r#"Operate the Ai00-X application's own GUI.

Use this tool when the user asks you to change settings, open scenes/tabs,
click UI elements, set models, or perform any action inside the Ai00-X app itself.
...
4. After completing the user's request, return to the session scene with "return_to_session" task or open_scene "session"."#
    ...
    async fn call_impl(
        ...
        let validated: SelfControlInput = serde_json::from_value(input.clone())
            .map_err(|e| Ai00XError::tool(format!("Invalid SelfControl input: {}", e)))?;
        ...
        match decision {
            ...
            Ok(Err(_)) => Err(Ai00XError::tool(
                "Self-control channel closed before response".to_string(),
            )),
            Err(_) => Err(Ai00XError::tool(
