...
impl BashTool {
    ...
    pub fn noninteractive_env() -> std::collections::HashMap<String, String> {
        ...
        env.insert("AI00_X_NONINTERACTIVE".to_string(), "1".to_string());
...
Package management rules:
  - When installing packages, use the project's existing package manager (npm, yarn, pnpm, uv, cargo, etc.)
  - For Python: prefer `uv pip install` over `pip install`. If you must use pip, add `--break-system-packages` flag when installing outside a virtual environment
  - For Node.js: use `pnpm add` for pnpm projects, `npm install` for npm projects, `yarn add` for yarn projects
  - Never install packages globally unless explicitly requested by the user"#
        ))
    }
    ...
    async fn description_with_context(
        ...
        if context.map(|c| c.is_remote()).unwrap_or(false) {
            base = format!(
                r#"**Remote workspace:** Commands run on the **SSH server** in a shell whose initial working directory is the **remote workspace root** (same as running a terminal on that machine). The shell name shown below may reflect your **local** Ai00-X settings; the actual interpreter on the server is typically `sh`/`bash`. Use **Unix** syntax and POSIX paths — not PowerShell or Windows paths.
...
{base}"#,
    ...
    async fn call(&self, input: &Value, context: &ToolUseContext) -> Ai00XResult<Vec<ToolResult>> {
        ...
        let command_str = input
            .get("command")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("command is required".to_string()))?;
        ...
        if context.is_remote() {
            if let Some(ws_shell) = context.ws_shell() {
                ...
                let (stdout, stderr, exit_code) = ws_shell
                    ...
                    .await
                    .map_err(|e| {
                        Ai00XError::tool(format!("Remote command execution failed: {}", e))
                    })?;
        ...
        let chat_session_id = context
            .session_id
            .as_ref()
            .ok_or_else(|| Ai00XError::tool("session_id is required for Bash tool".to_string()))?;
        ...
        let terminal_api = TerminalApi::from_singleton()
            .map_err(|e| Ai00XError::tool(format!("Terminal not initialized: {}", e)))?;
        ...
        let workspace_path = context
            .workspace_root()
            .ok_or_else(|| {
                Ai00XError::tool("workspace_path is required for Bash tool".to_string())
            })?
            .to_string_lossy()
            .to_string();
        ...
        let primary_session_id = binding
            ...
            .await
            .map_err(|e| Ai00XError::tool(format!("Failed to create Terminal session: {}", e)))?;
...
impl BashTool {
    ...
    async fn call_background(
        ...
        terminal_api
            ...
            .await
            .map_err(|e| Ai00XError::tool(format!("Failed to send background command: {}", e)))?;
        ...
        // local:  ~/.ai00-x/projects/<project-slug>/sessions/<chat-session-id>/tool-results/<tool-use-id>.txt
        // remote: ~/.ai00-x/remote_ssh/<host>/<remote-path>/sessions/<chat-session-id>/tool-results/<tool-use-id>.txt
