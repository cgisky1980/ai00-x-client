...
use crate::agent::tools::workspace_paths::is_ai00x_runtime_uri;
...
impl FileReadTool {
    ...
    async fn read_remote_window(
    ...
    ) -> Ai00XResult<tool_runtime::fs::read_file::ReadFileResult> {
        const TOTAL_LINES_MARKER: &str = "__AI00_X_TOTAL_LINES__=";
        const HIT_TOTAL_CHAR_LIMIT_MARKER: &str = "__AI00_X_HIT_TOTAL_CHAR_LIMIT__=";

        let end_line = start_line
            .checked_add(limit.saturating_sub(1))
            .ok_or_else(|| Ai00XError::tool("Requested line range is too large".to_string()))?;

        let ws_shell = context
            .ws_shell()
            .ok_or_else(|| Ai00XError::tool("Remote workspace shell is unavailable".to_string()))?;
        ...
        let (stdout, stderr, status) = ws_shell
            .exec(&command, None)
            .await
            .map_err(|e| Ai00XError::tool(format!("Failed to read file: {}", e)))?;
        ...
        if status != 0 {
            ...
            return Err(Ai00XError::tool(message));
        }

        let total_lines = total_lines.ok_or_else(|| {
            Ai00XError::tool("Failed to read file: remote line count was unavailable".to_string())
        })?;
...
"#,
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "The absolute path to the file to read, or an exact ai00-x://runtime URI returned by another tool"
                },
                "start_line": {
                    "type": "number",
    ...
    async fn validate_input(
        ...
        let resolved = match context.map(|ctx| ctx.resolve_tool_path(file_path)) {
            ...
            None => {
                if is_ai00x_runtime_uri(file_path) {
                    return ValidationResult {
                        result: false,
                        message: Some(
                            "Tool context is required to resolve ai00-x runtime URIs".to_string(),
                        ),
                        error_code: Some(400),
                        meta: None,
                    };
