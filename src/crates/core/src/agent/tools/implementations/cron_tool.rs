...
impl CronTool {
    ...
    fn resolve_workspace(
        ...
        Self::validate_workspace_format(workspace, context).map_err(Ai00XError::tool)?;
    ...
    fn resolve_workspace_from_context(&self, context: &ToolUseContext) -> Ai00XResult<String> {
    ...
    fn resolve_effective_session_id(
        ...
        Self::validate_session_id(&resolved).map_err(Ai00XError::tool)?;
        Ok(resolved)
    }

    async fn ensure_session_exists(&self, workspace: &str, session_id: &str) -> Ai00XResult<()> {
    ...
    fn normalize_optional_name(name: Option<String>) -> Ai00XResult<Option<String>> {
        match name {
            Some(name) if name.trim().is_empty() => Err(Ai00XError::tool(
    ...
    fn validate_payload(payload: &str, field_name: &str) -> Ai00XResult<()> {
    ...
    fn parse_iso_timestamp_ms(value: &str, field_name: &str) -> Ai00XResult<i64> {
    ...
    fn format_iso_timestamp_local(timestamp_ms: i64, field_name: &str) -> Ai00XResult<String> {
        let datetime = Local
    ...
    fn seconds_to_every_ms(seconds: u64, field_name: &str) -> Ai00XResult<u64> {
        if seconds == 0 {
            return Err(Ai00XError::tool(format!(
        ...
        seconds
            .checked_mul(1_000)
            .ok_or_else(|| Ai00XError::tool(format!("{}.every is too large", field_name)))
    }

    fn serialize_job(job: &CronJob) -> Ai00XResult<Value> {
        serde_json::to_value(CronToolJobOutput::try_from(job)?)
            .map_err(|err| Ai00XError::serialization(err.to_string()))
    }

    fn serialize_jobs(jobs: &[CronJob]) -> Ai00XResult<Vec<Value>> {
...
impl CronToolScheduleInput {
    fn to_service_schedule(&self, field_name: &str) -> Ai00XResult<CronSchedule> {
        match self {
            Self::At { at } => {
                let at = at.trim();
                if at.is_empty() {
...
impl TryFrom<&CronSchedule> for CronToolScheduleOutput {
    type Error = Ai00XError;

    fn try_from(schedule: &CronSchedule) -> Ai00XResult<Self> {
        match schedule {
...
impl TryFrom<&CronJob> for CronToolJobOutput {
    ...
    fn try_from(job: &CronJob) -> Ai00XResult<Self> {
        Ok(Self {
            id: job.id.clone(),
            name: job.name.clone(),
...
- Same fields as "job", but every field is optional."#
    ...
    async fn call_impl(
        ...
        let params: CronToolInput = serde_json::from_value(input.clone())
            .map_err(|err| Ai00XError::tool(format!("Invalid input: {}", err)))?;
        ...
        match params.action {
            ...
            CronAction::List => {
                let cron_service = get_global_cron_service()
                    .ok_or_else(|| Ai00XError::tool("cron service not initialized".to_string()))?;
                let workspace =
            ...
            CronAction::Add => {
                ...
                let job = params
                    .job
                    .ok_or_else(|| Ai00XError::tool("job is required for add".to_string()))?;
            ...
            CronAction::Update => {
                ...
                let job_id = params
                    .job_id
                    .ok_or_else(|| Ai00XError::tool("job_id is required for update".to_string()))?;
                Self::validate_job_id(&job_id).map_err(Ai00XError::tool)?;
                let patch = params
                    .patch
                    .ok_or_else(|| Ai00XError::tool("patch is required for update".to_string()))?;
                if patch.is_empty() {
            ...
            CronAction::Remove => {
                ...
                let job_id = params
                    .job_id
                    .ok_or_else(|| Ai00XError::tool("job_id is required for remove".to_string()))?;
                Self::validate_job_id(&job_id).map_err(Ai00XError::tool)?;
            ...
            CronAction::Run => {
                ...
                let job_id = params
                    .job_id
                    .ok_or_else(|| Ai00XError::tool("job_id is required for run".to_string()))?;
                Self::validate_job_id(&job_id).map_err(Ai00XError::tool)?;
