...
use crate::agent::tools::browser_control::actions::BrowserActions;
use crate::agent::tools::browser_control::browser_launcher::{
    BrowserLauncher, LaunchResult, DEFAULT_CDP_PORT,
};
use crate::agent::tools::browser_control::cdp_client::CdpClient;
use crate::agent::tools::browser_control::daemon::DEFAULT_DAEMON_PORT;
use crate::agent::tools::browser_control::daemon_client::DaemonClient;
...
impl ControlHubTool {
    ...
    async fn dispatch(
        ...
        match domain {
            ...
            other => Err(Ai00XError::tool(format!(
    ...
    async fn handle_desktop(
        ...
        if context.computer_use_host.is_none() {
            return Err(Ai00XError::tool(
                "Desktop control is only available in the Ai00-X desktop app".to_string(),
            ));
        }
    ...
    async fn handle_browser(&self, action: &str, params: &Value) -> Ai00XResult<Vec<ToolResult>> {
        let port = params
            .get("port")
            .and_then(|v| v.as_u64())
            .map(|p| p as u16)
        ...
        match action {
                ...
                match mode {
                    "extension" | "daemon" => {
                        ...
                        if running && ext_connected {
                            Ok(vec![ToolResult::ok(
                                ...
                                Some(format!(
                                    "Connected to browser extension via daemon on port {}",
                                    daemon_port
                                )),
                            )])
                        } else {
                            Ok(vec![ToolResult::ok(
                                json!({
                                    ...
                                    "status": if running { "extension_not_connected" } else { "daemon_not_running" },
                                    "hint": "Install the Ai00-X Browser Helper extension and ensure the daemon is running"
                                }),
                                Some(if running {
                                    "Daemon is running but browser extension is not connected. Install the Ai00-X Browser Helper extension.".to_string()
                                } else {
                                    "Browser daemon is not running. Start the daemon first."
                                        .to_string()
                                }),
                            )])
                        }
                    ...
                    "cdp" => {
                        ...
                        match &launch_result {
                            ...
                            LaunchResult::LaunchedButCdpNotReady { message, .. } => {
                                Ok(vec![ToolResult::ok(
                                    json!({ "success": false, "mode": "cdp", "status": "cdp_not_ready", "message": message }),
                                    Some(message.clone()),
                                )])
                            }
                            LaunchResult::BrowserRunningWithoutCdp { instructions, .. } => {
                                Ok(vec![ToolResult::ok(
                                    json!({ "success": false, "mode": "cdp", "status": "needs_restart", "instructions": instructions }),
                    ...
                    _ => {
                        ...
                        if running && ext_connected {
                            ...
                            log::info!("Extension mode not available (daemon_running={}, ext_connected={}), falling back to CDP", running, ext_connected);
                            let kind = BrowserLauncher::detect_default_browser()?;
                            let launch_result =
                                BrowserLauncher::launch_with_cdp(&kind, port).await?;
            ...
            "daemon_status" => {
                ...
                Ok(vec![ToolResult::ok(
                    json!({
                        ...
                        "daemon_port": daemon_port,
                        "extension_connected": ext_connected,
                        "mode": if running && ext_connected { "extension" } else if running { "daemon_only" } else { "unavailable" }
                    }),
                    Some(if running && ext_connected {
                        format!(
                            "Daemon running on port {} with extension connected",
                            daemon_port
                        )
                    } else if running {
                        format!(
                            "Daemon running on port {} but no extension connected",
                            daemon_port
                        )
                    } else {
                        format!("Daemon not running on port {}", daemon_port)
                    }),
                )])
            }
            ...
            "switch_page" => {
                let page_id = params
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        Ai00XError::tool("switch_page requires 'page_id'".to_string())
                    })?;
                let pages = CdpClient::list_pages(port).await?;
                let page = pages
                    .iter()
                    .find(|p| p.id == page_id)
                    .ok_or_else(|| Ai00XError::tool(format!("Page '{}' not found", page_id)))?;
                let ws_url = page
                    .web_socket_debugger_url
                    .as_ref()
                    .ok_or_else(|| Ai00XError::tool("Page has no WebSocket URL".to_string()))?;
                ...
                match action {
                    "navigate" => {
                        let url = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("navigate requires 'url'".to_string())
                            })?;
                    ...
                    "click" => {
                        let selector = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("click requires 'selector'".to_string())
                            })?;
                    ...
                    "fill" => {
                        let selector = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("fill requires 'selector'".to_string())
                            })?;
                        let value = params
                            .get("value")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("fill requires 'value'".to_string())
                            })?;
                    ...
                    "type" => {
                        let text = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("type requires 'text'".to_string())
                            })?;
                    ...
                    "select" => {
                        let selector = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("select requires 'selector'".to_string())
                            })?;
                        let option_text = params
                            .get("option_text")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("select requires 'option_text'".to_string())
                            })?;
                    ...
                    "press_key" => {
                        let key = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("press_key requires 'key'".to_string())
                            })?;
                    ...
                    "get_text" => {
                        let selector = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("get_text requires 'selector'".to_string())
                            })?;
                    ...
                    "evaluate" => {
                        let expression = params
                            ...
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| {
                                Ai00XError::tool("evaluate requires 'expression'".to_string())
                            })?;
                    ...
                    other => Err(Ai00XError::tool(format!(
                        "Unknown browser action: '{}'. Valid: connect, daemon_status, navigate, snapshot, click, fill, type, select, press_key, scroll, wait, get_text, get_url, get_title, screenshot, evaluate, close, list_pages, switch_page",
                        other
                    ))),
                }
            }
    ...
    async fn handle_terminal(
        ...
        let terminal_session_id = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                Ai00XError::tool("Terminal actions require 'terminal_session_id'".to_string())
            })?;
        ...
        match action {
            "open_app" => {
                let app_name = params
                    .get("app_name")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| Ai00XError::tool("open_app requires 'app_name'".to_string()))?;
                ...
                let output = process_manager::create_command(&cmd)
                    .args(&args)
                    .output()
                    .map_err(|e| Ai00XError::tool(format!("Failed to open app: {}", e)))?;
            ...
            "run_script" => {
                let script = params
                    .get("script")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| Ai00XError::tool("run_script requires 'script'".to_string()))?;
                ...
                match script_type {
                    "applescript" => {
                        #[cfg(target_os = "macos")]
                        {
                            let output = std::process::Command::new("/usr/bin/osascript")
                                ...
                                .output()
                                .map_err(|e| {
                                    Ai00XError::tool(format!("Failed to run AppleScript: {}", e))
                                })?;
                            ...
                            if output.status.success() {
                                ...
                                Err(Ai00XError::tool(format!("AppleScript error: {}", stderr)))
                            }
                        }
                    ...
                    "shell" => {
                        ...
                        let output = process_manager::create_command("cmd")
                            ...
                            .output()
                            .map_err(|e| {
                                Ai00XError::tool(format!("Failed to run script: {}", e))
                            })?;
                        #[cfg(not(target_os = "windows"))]
                        let output = process_manager::create_command("sh")
                            .args(["-c", script])
                            .output()
                            .map_err(|e| {
                                Ai00XError::tool(format!("Failed to run script: {}", e))
                        ...
                        if output.status.success() {
                            ...
                            Err(Ai00XError::tool(format!("Script error: {}", stderr)))
                        }
                    }
                    other => Err(Ai00XError::tool(format!(
...
impl Tool for ControlHubTool {
    ...
    async fn description(&self) -> Ai00XResult<String> {
        Ok(r#"Unified control hub for the Agent OS. All control operations go through this single tool.

## Domains and Actions

### domain: "browser" — Control the user's default browser (Chrome/Edge/etc.) via CDP or Extension
Preserves user login sessions, cookies, and extensions.
- **connect**: Connect to or launch the browser. Modes: `auto` (try Extension first, fall back to CDP), `extension` (Extension+Daemon only), `cdp` (CDP only). Optional: `mode`, `port` (CDP, default 9222), `daemon_port` (Extension, default 19926).
- **daemon_status**: Check if the browser daemon and extension are running. Optional: `daemon_port`.
...
Only available in Ai00-X desktop app.
...
### domain: "app" — Control Ai00-X's own GUI
...
4. For cross-domain workflows (e.g., browser data → desktop paste), call actions sequentially."#
    ...
    async fn call_impl(
        ...
        let domain = input
            .get("domain")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("Missing 'domain'".to_string()))?;
        let action = input
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("Missing 'action'".to_string()))?;
