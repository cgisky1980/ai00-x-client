...
use crate::infrastructure::filesystem::acquire_file_write_lock;
...
3. **Path Requirements**:
   - You can use either relative paths (e.g., "temp/data.txt"), absolute paths (e.g., "/workspace/temp/data.txt"), or exact `ai00-x://runtime/...` URIs returned by another tool
 ...
 - The tool will fail gracefully if permissions are insufficient"#.to_string())
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "The absolute path to the file or directory to delete, or an exact ai00-x://runtime URI returned by another tool"
                },
                "recursive": {
                    "type": "boolean",
    ...
    async fn validate_input(
        ...
        let resolved = match context.map(|ctx| ctx.resolve_tool_path(path_str)) {
            ...
            None => {
                if is_ai00x_runtime_uri(path_str) {
                    return ValidationResult {
                        result: false,
                        message: Some(
    ...
    async fn call_impl(
        ...
        let path_str = input
            .get("path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("path is required".to_string()))?;
        ...
        if resolved.uses_remote_workspace_backend() {
            let ws_shell = context.ws_shell().ok_or_else(|| {
                Ai00XError::tool("Workspace shell not available for remote Delete".to_string())
            })?;
            ...
            let (_stdout, stderr, exit_code) = ws_shell
                ...
                .await
                .map_err(|e| Ai00XError::tool(format!("Failed to delete on remote: {}", e)))?;
        ...
        let lock = acquire_file_write_lock(path).await;
        ...
        if is_directory {
            if recursive {
                fs::remove_dir_all(path)
                    .await
                    .map_err(|e| Ai00XError::tool(format!("Failed to delete directory: {}", e)))?;
            } else {
                fs::remove_dir(path)
                    .await
                    .map_err(|e| Ai00XError::tool(format!("Failed to delete directory: {}", e)))?;
            ...
            fs::remove_file(path)
                .await
                .map_err(|e| Ai00XError::tool(format!("Failed to delete file: {}", e)))?;
        }
