...
use crate::agent::tools::framework::{ToolResult, ToolUseContext};
use crate::agent::tools::implementations::computer_use_tool::computer_use_augment_result_json;
...
pub(crate) async fn execute_computer_use_locate(
    ...
    if !ai.computer_use_enabled {
        return Err(Ai00XError::tool(
            "Computer use is disabled in Ai00-X settings.".to_string(),
        ));
    }

    let host = context.computer_use_host.as_ref().ok_or_else(|| {
        Ai00XError::tool("Computer use is only available in the Ai00-X desktop app.".to_string())
    })?;
