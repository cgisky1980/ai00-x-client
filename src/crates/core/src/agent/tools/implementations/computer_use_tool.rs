...
use crate::agent::tools::computer_use_capability::computer_use_desktop_available;
use crate::agent::tools::computer_use_host::{
...
use crate::agent::tools::computer_use_optimizer::hash_screenshot_bytes;
...
pub(crate) async fn computer_use_augment_result_json(
    host: &dyn crate::agent::tools::computer_use_host::ComputerUseHost,
    mut body: Value,
...
const COMPUTER_USE_DEBUG_SUBDIR: &str = ".ai00-x/computer_use_debug";
...
**`key_chord` / `type_text` / `scroll` / `wait`:** standard desktop automation without any screenshot step.\n",
    ...
    async fn move_to_text_disambiguation_response(
        host_ref: &dyn crate::agent::tools::computer_use_host::ComputerUseHost,
    ...
    fn require_multimodal_tool_output_for_screenshot(ctx: &ToolUseContext) -> Ai00XResult<()> {
    ...
    fn ensure_click_has_no_coordinate_fields(input: &Value) -> Ai00XResult<()> {
    ...
    async fn find_text_on_screen(
        host_ref: &dyn crate::agent::tools::computer_use_host::ComputerUseHost,
        text_query: &str,
        region_native: Option<crate::agent::tools::computer_use_host::OcrRegionNative>,
    ) -> Ai00XResult<Vec<ScreenOcrTextMatch>> {
    ...
    ) -> Ai00XResult<(Value, ToolImageAttachment, String)> {
...
pub(crate) async fn computer_use_execute_mouse_click_tool(
    ...
    let act = input
        .get("action")
        .and_then(|v| v.as_str())
        .ok_or_else(|| Ai00XError::tool("action is required (click or wheel)".to_string()))?;
    match act {
        "click" => {
            let button = input
                .get("button")
...
fn parse_ocr_region_native(
    input: &Value,
) -> Ai00XResult<Option<crate::agent::tools::computer_use_host::OcrRegionNative>> {
    let v = input
    ...
    let x0 = o.get("x0").and_then(|x| x.as_i64()).ok_or_else(|| {
        Ai00XError::tool("ocr_region_native.x0 (integer) is required.".to_string())
    })? as i32;
    let y0 = o.get("y0").and_then(|x| x.as_i64()).ok_or_else(|| {
        Ai00XError::tool("ocr_region_native.y0 (integer) is required.".to_string())
    })? as i32;
    let width = o.get("width").and_then(|x| x.as_u64()).ok_or_else(|| {
        Ai00XError::tool("ocr_region_native.width (positive integer) is required.".to_string())
    })? as u32;
    let height = o.get("height").and_then(|x| x.as_u64()).ok_or_else(|| {
        Ai00XError::tool("ocr_region_native.height (positive integer) is required.".to_string())
    })? as u32;
    if width == 0 || height == 0 {
        return Err(Ai00XError::tool(
    ...
    Ok(Some(
        crate::agent::tools::computer_use_host::OcrRegionNative {
            x0,
            y0,
            width,
            height,
...
**`type_text`:** Type text; prefer clipboard for long content. Does **not** move the pointer — **Enter** **`key_chord`** may follow without a mandatory `screenshot` unless you moved the pointer since the last capture. If **`screenshot`** shows the correct chat is already open and the input may be focused, **try `type_text` first** before spending steps on `click_element` / `move_to_text`.",
    ...
    async fn is_enabled(&self) -> bool {
        ...
        let Ok(service) = get_global_config_service() else {
            return false;
        };
    ...
    async fn call_impl(
        ...
        let host = context.computer_use_host.as_ref().ok_or_else(|| {
            Ai00XError::tool(
                "Computer use is only available in the Ai00-X desktop app.".to_string(),
            )
        })?;
        ...
        let action = input
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::tool("action is required".to_string()))?;
        ...
        match action {
            ...
            "click_label" => {
                ...
                let label = input.get("label").and_then(|v| v.as_u64()).ok_or_else(|| {
                    Ai00XError::tool("click_label requires integer field `label`.".to_string())
                })? as u32;
                if label == 0 {
                    return Err(Ai00XError::tool(
                ...
                let matched = latest_shot
                    ...
                    .cloned()
                    .ok_or_else(|| Ai00XError::tool(format!(
            ...
            "key_chord" => {
                let keys: Vec<String> = input
                    ...
                    .ok_or_else(|| Ai00XError::tool("keys array is required".to_string()))?
                    .iter()
                    .filter_map(|v| v.as_str().map(|s| s.to_string()))
                    .collect();
                if keys.is_empty() {
                    return Err(Ai00XError::tool("keys must not be empty".to_string()));
                }
            ...
            "type_text" => {
                let text = input
                    .get("text")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| Ai00XError::tool("text is required".to_string()))?;
            ...
            "wait" => {
                let ms = input
                    .get("ms")
                    .and_then(|v| v.as_u64())
                    .ok_or_else(|| Ai00XError::tool("ms is required".to_string()))?;
            ...
            "open_app" => {
                let app_name = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        Ai00XError::tool("open_app requires `app_name` parameter.".to_string())
                    })?;
            ...
            "run_apple_script" => {
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    .await
                    .map_err(|e| Ai00XError::tool(format!("spawn: {}", e)))?
                    .map_err(|e| Ai00XError::tool(format!("osascript: {}", e)))?;
            ...
            _ => Err(Ai00XError::tool(format!("Unknown action: {}", action))),
        }
    }
}
...
fn req_i32(input: &Value, key: &str) -> Ai00XResult<i32> {
    input
        .get(key)
        .and_then(|v| v.as_i64())
        .map(|v| v as i32)
        .ok_or_else(|| Ai00XError::tool(format!("{} is required (integer)", key)))
}
