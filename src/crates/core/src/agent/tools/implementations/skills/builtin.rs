//! Built-in skills shipped with Ai00-X.
//!
//! These skills are embedded into the `ai00-x-core` binary and installed into the user skills
...
pub async fn ensure_builtin_skills_installed() -> Ai00XResult<()> {
...
async fn sync_dir(dir: &Dir<'_>, dest_root: &Path) -> Ai00XResult<SyncStats> {
...
fn safe_join(root: &Path, relative: &Path) -> Ai00XResult<PathBuf> {
    if relative.is_absolute() {
        return Err(crate::util::errors::Ai00XError::validation(format!(
