...
**Always use this tool before starting any real work** — research, multi-step tasks, file creation, or any workflow involving multiple steps or tool calls. The only exception is simple back-and-forth conversation or quick factual questions.

Even requests that sound simple are often underspecified. Asking upfront prevents wasted effort on the wrong thing. Examples of underspecified requests — always use this tool:
...
Key decision points that ALWAYS require user confirmation:
- Deleting files or directories
- Making changes to configuration files (build configs, CI/CD, deployment)
- Choosing between significantly different implementation approaches
- Executing commands that have irreversible effects
- Any action that could affect security or data integrity
...
- The task is straightforward with an obvious correct approach"#.to_string())
    }
    ...
    async fn call_impl(
        ...
        let tool_input: AskUserQuestionInput =
            serde_json::from_value(input.clone()).map_err(|e| {
                crate::util::errors::Ai00XError::Validation(format!(
        ...
        if let Err(error) = Self::validate_input(&tool_input) {
            return Err(crate::util::errors::Ai00XError::Validation(error));
        }
