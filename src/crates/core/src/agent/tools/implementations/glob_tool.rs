...
fn resolve_glob_config(pattern: &str) -> (bool, bool) {
    let is_whitelisted = pattern.starts_with(".ai00-x")
        || pattern.contains("/.ai00-x")
        || pattern.contains("\\.ai00-x");
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "path": {
                    "type": "string",
                    "description": "The directory to search in. If not specified, the current working directory will be used. IMPORTANT: Omit this field to use the default directory. DO NOT enter \"undefined\" or \"null\" - simply omit it for the default behavior. Must be a valid absolute path or exact ai00-x://runtime URI if provided."
                },
                "limit": {
                    "type": "number",
    ...
    async fn call_impl(
        ...
        let resolved = match input.get("path").and_then(|v| v.as_str()) {
            ...
            None => {
                ...
                crate::agent::tools::framework::ToolPathResolution {
                    requested_path: root.clone(),
                    logical_path: root.clone(),
                    resolved_path: root,
                    backend: if context.is_remote() {
                        crate::agent::tools::framework::ToolPathBackend::RemoteWorkspace
                    } else {
                        crate::agent::tools::framework::ToolPathBackend::Local
                    },
                    runtime_scope: None,
                    runtime_root: None,
                }
        ...
        if resolved.uses_remote_workspace_backend() {
            ...
            let (_stdout, _stderr, exit_code) = ws_shell
                ...
                .await
                .map_err(|e| Ai00XError::tool(format!("Failed to detect rg on remote: {}", e)))?;
            ...
            let (stdout, _stderr, _exit_code) = ws_shell
                ...
                .await
                .map_err(|e| {
                    Ai00XError::tool(format!("Failed to glob on remote with rg: {}", e))
                })?;
        ...
        .await
        .map_err(|err| Ai00XError::tool(format!("Glob tool task failed: {}", err)))?
...
mod tests {
    ...
    fn make_temp_dir(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("ai00-x-glob-tool-{name}-{unique}"));
    ...
    fn does_not_expand_walk_root_outside_search_path() {
        let root = std::env::temp_dir().join("ai00-x-glob-root");
