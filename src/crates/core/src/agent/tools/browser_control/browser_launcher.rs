...
impl BrowserLauncher {
    ...
    pub fn detect_default_browser() -> Ai00XResult<BrowserKind> {
        #[cfg(target_os = "macos")]
        {
    ...
    #[cfg(target_os = "macos")]
    fn detect_default_browser_macos() -> Ai00XResult<BrowserKind> {
    ...
    #[cfg(target_os = "windows")]
    fn detect_default_browser_windows() -> Ai00XResult<BrowserKind> {
        let output = process_manager::create_command("reg")
            .args([
                "query",
    ...
    #[cfg(target_os = "linux")]
    fn detect_default_browser_linux() -> Ai00XResult<BrowserKind> {
    ...
    pub async fn launch_with_cdp(kind: &BrowserKind, port: u16) -> Ai00XResult<LaunchResult> {
        ...
        let result = process_manager::create_command(&exe).arg(&flag).spawn();
    ...
    fn is_browser_running(kind: &BrowserKind) -> bool {
        ...
        {
            for name in &process_names {
                let output = process_manager::create_command("tasklist")
    ...
    #[cfg(target_os = "macos")]
    pub fn create_cdp_launcher_app(kind: &BrowserKind, port: u16) -> Ai00XResult<String> {
        ...
        std::fs::create_dir_all(&macos_dir)
            .map_err(|e| Ai00XError::tool(format!("Failed to create app bundle: {}", e)))?;
        ...
        std::fs::write(&script_path, &script)
            .map_err(|e| Ai00XError::tool(format!("Failed to write launcher script: {}", e)))?;
        ...
        #[cfg(unix)]
        {
            ...
            std::fs::set_permissions(&script_path, std::fs::Permissions::from_mode(0o755))
                .map_err(|e| {
                    Ai00XError::tool(format!("Failed to set executable permission: {}", e))
                })?;
        }
...
<dict>
    ...
    <string>com.ai00-x.browser-debug-launcher</string>
</dict>
</plist>"#,
            app_name
        );
        ...
        std::fs::write(format!("{}/Contents/Info.plist", app_dir), &plist)
            .map_err(|e| Ai00XError::tool(format!("Failed to write Info.plist: {}", e)))?;
