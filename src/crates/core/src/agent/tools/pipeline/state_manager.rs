...
use crate::agent::core::ToolExecutionState;
use crate::agent::events::{AgentEvent, EventQueue, ToolEventData};
use dashmap::DashMap;
use log::debug;
use std::sync::Arc;
...
impl ToolStateManager {
    ...
    async fn emit_state_change_event(&self, task: ToolTask) {
        let tool_event = match &task.state {
            ...
            ToolExecutionState::Completed {
                ...
                result_for_assistant: match result {
                    crate::agent::tools::framework::ToolResult::Result {
        ...
        let event = AgentEvent::ToolEvent {
