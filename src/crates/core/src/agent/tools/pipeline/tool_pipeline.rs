...
use super::types::*;
use crate::agent::core::{ToolCall, ToolExecutionState, ToolResult as ModelToolResult};
use crate::agent::events::types::ToolEventData;
use crate::agent::tools::computer_use_host::ComputerUseHostRef;
use crate::agent::tools::framework::{ToolResult as FrameworkToolResult, ToolUseContext};
use crate::agent::tools::permission::PermissionOutcome;
use crate::agent::tools::registry::ToolRegistry;
use crate::agent::tools::tool_repair::repair_tool_call;
...
impl ToolPipeline {
    ...
    fn extract_file_path_from_tool(tool_name: &str, input: &serde_json::Value) -> Option<String> {
        input
            .get("file_path")
            .and_then(|v| v.as_str())
            .map(|s| {
    ...
    ) -> Ai00XResult<Vec<ToolExecutionResult>> {
    ...
    /// Execute single tool
    async fn execute_single_tool(&self, tool_id: String) -> Ai00XResult<ToolExecutionResult> {
        ...
        let task = self
            .state_manager
            .get_task(&tool_id)
            .ok_or_else(|| Ai00XError::NotFound(format!("Tool task not found: {}", tool_id)))?;
        ...
        if tool_name.is_empty() || tool_is_error {
            ...
            return Err(Ai00XError::Validation(error_msg));
        }
        ...
        if let Some(ref policy) = task.context.permission_policy {
            match policy.authorize(&tool_name, &task.tool_call.arguments) {
                PermissionOutcome::Deny { reason } => {
                    ...
                    self.state_manager
                        ...
                        .await;
                    return Err(Ai00XError::Validation(reason));
                }
                PermissionOutcome::Allow => {}
            }
        }

        // Workflow phase path restriction: in Planning phase, write tools can only write to plan files
        if let Some(ref phase) = task.context.workflow_phase {
            if phase == "planning" {
                let restricted_tools = ["Write", "Edit"];
                if restricted_tools.contains(&tool_name.as_str()) {
                    if let Some(file_path) =
                        Self::extract_file_path_from_tool(&tool_name, &task.tool_call.arguments)
                    {
                        let plans_dir = task.context.plans_dir.as_deref().unwrap_or("");
                        if !plans_dir.is_empty() {
                            let normalized_file_path = file_path.replace('\\', "/");
                            let normalized_plans_dir = plans_dir.replace('\\', "/");
                            if !normalized_file_path.starts_with(&normalized_plans_dir) {
                                let error_msg = format!(
                                    "Planning phase restriction: tool '{}' can only write to plan files under '{}', got '{}'",
                                    tool_name, plans_dir, file_path
                                );
                                warn!("{}", error_msg);
                                self.state_manager
                                    .update_state(
        ...
        let tool = {
            let registry = self.tool_registry.read().await;
            if let Some(t) = registry.get_tool(&task.tool_call.tool_name) {
                t
            } else {
                let tool_names: Vec<String> = registry.get_tool_names();
                let name_strs: Vec<&str> = tool_names.iter().map(|s| s.as_str()).collect();
                if let Some(repaired) = repair_tool_call(&task.tool_call.tool_name, &name_strs) {
                    info!(
                        "Tool name repaired: {} -> {}",
                        task.tool_call.tool_name, repaired
                    );
                    registry.get_tool(&repaired).ok_or_else(|| {
                        let error_msg = format!(
                            "Tool '{}' is not registered or enabled (repaired to '{}' but still not found).",
                            task.tool_call.tool_name, repaired,
                        );
                        error!("{}", error_msg);
                        Ai00XError::tool(error_msg)
                    })?
                } else {
                    let error_msg = format!(
                    ...
                    error!("{}", error_msg);
                    return Err(Ai00XError::tool(error_msg));
                }
            }
        };
        ...
        if needs_confirmation {
            ...
            match confirmation_result {
                ...
                Some(Err(_)) => {
                    ...
                    return Err(Ai00XError::service("Confirmation channel closed"));
                }
                None => {
                    self.state_manager
                        .update_state(
                    ...
                    return Err(Ai00XError::Timeout(format!(
    ...
    async fn execute_with_retry(
        ...
        tool: Arc<dyn crate::agent::tools::framework::Tool>,
    ) -> Ai00XResult<ModelToolResult> {
        let mut attempts = 0;
    ...
    pub async fn cancel_dialog_turn_tools(&self, dialog_turn_id: &str) -> Ai00XResult<()> {
        info!(
