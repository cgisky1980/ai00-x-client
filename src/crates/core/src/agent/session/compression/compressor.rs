...
impl ContextCompressor {
    ...
    ) -> Ai00XResult<Vec<TurnWithTokens>> {
        debug!(
    ...
    pub async fn preprocess_turns(
        ...
        messages: Vec<Message>,
    ) -> Ai00XResult<(usize, Vec<TurnWithTokens>)> {
        debug!(
        ...
        // Enforce head protection: the first N turns (head_protect_turns) must never be
        // compressed. If the keep zone starts after the head zone, ensure the head turns
        // are excluded from the compression zone.
        if turn_index_to_keep > 0 {
            let head_count = self.config.head_protect_turns.min(turns_count);
            if turn_index_to_keep < head_count {
                turn_index_to_keep = head_count;
                debug!(
                    "Head protection enforced: session_id={}, head_protect_turns={}, adjusted_keep_from={}",
                    session_id, head_count, turn_index_to_keep
                );
            }
        }
        debug!(
    ...
    pub async fn compress_turns(
        ...
        let head_count = self.config.head_protect_turns.min(turns.len());
        let head_turns: Vec<TurnWithTokens> = if head_count > 0 && head_count < turns.len() {
            turns.drain(0..head_count).collect()
        } else {
            Vec::new()
        };
        let turn_index_to_keep = turn_index_to_keep.saturating_sub(head_count);
        let turns_to_keep = turns.split_off(turn_index_to_keep);

        let mut compressed_messages = Vec::new();
        let mut has_model_summary = false;

        // Head zone: protected turns, kept verbatim
        for head_turn in head_turns {
            compressed_messages.extend(head_turn.messages);
        }
    ...
    async fn execute_compression_with_fallback(
        ...
        context_window: usize,
    ) -> Ai00XResult<CompressionSummaryArtifact> {
        let summary_result = match get_global_ai_client_factory().await {
                ...
                .await
            {
                ...
                Err(err) => Err(Ai00XError::AIClient(format!(
    ...
    async fn execute_compression(
        ...
        turns_to_compress: Vec<TurnWithTokens>,
        context_window: usize,
    ) -> Ai00XResult<(String, Vec<GeminiUsage>)> {
...
Be thorough and precise. Do not lose important technical details from either the previous summary or the new conversation."#,
        ...
        let mut usages: Vec<GeminiUsage> = Vec::new();
    ...
    async fn generate_summary_with_retry(
        ...
        for attempt in 0..max_tries {
            ...
            match result {
                Ok(response) => {
                    ...
                    return Ok((response.text, response.usage));
                }
                Err(e) => {
                    warn!(
        ...
        warn!("{}", error_msg);
        Err(Ai00XError::AIClient(error_msg))
    }
...
#[cfg(test)]
mod tests {
    use super::{CompressionConfig, CompressionTailPolicy, ContextCompressor, TurnWithTokens};
    use crate::agent::core::{
    ...
    async fn collapse_all_creates_closed_compression_turn() {
        ...
        let boundary_text = match &result.messages[0].content {
            crate::agent::core::MessageContent::Text(text) => text,
    ...
    async fn preserve_live_frontier_keeps_last_user_after_summary_turn() {
        ...
        assert_eq!(
            ...
            crate::agent::core::MessageRole::User
        );
    ...
    fn synthetic_summary_turn_payload_remains_atomic_on_recompression() {
        ...
        let summary_artifact =
            crate::agent::session::compression::fallback::build_structured_compression_summary(
                vec![vec![marker, summary]],
                &crate::agent::session::compression::fallback::CompressionFallbackOptions {
                    max_tokens: 10_000,
                    user_chars: 120,
                    assistant_chars: 120,
                    tool_arg_chars: 80,
