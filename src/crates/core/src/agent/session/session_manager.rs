...
use crate::agent::session::SessionContextStore;
...
impl SessionManager {
    fn normalize_session_title_input(title: &str) -> Ai00XResult<String> {
    ...
    pub async fn create_session_with_id_and_details(
        ...
        let _workspace_path = Self::session_workspace_from_config(&config).ok_or_else(|| {
            Ai00XError::Validation("Session workspace_path is required".to_string())
        })?;
    ...
    pub async fn update_session_title(&self, session_id: &str, title: &str) -> Ai00XResult<()> {
        ...
        if self.config.enable_persistence {
            let Some(workspace_path) = workspace_path.as_ref() else {
                return Err(Ai00XError::Session(format!(
    ...
    /// Update session workflow phase (in-memory + persistence)
    ...
    async fn start_persisted_turn(
        ...
        let workspace_path = Self::effective_workspace_path_from_config(&session.config)
            .await
            .ok_or_else(|| {
                Ai00XError::Validation(format!("Session workspace_path is missing: {}", session_id))
            })?;
    ...
    pub async fn start_subagent_turn(
        ...
        self.start_persisted_turn(
            session_id,
            DialogTurnKind::SubagentTask,
            user_input,
            turn_id,
            Some(user_message),
    ...
    pub async fn complete_dialog_turn(
        ...
        let turn_index = self
            ...
            .ok_or_else(|| Ai00XError::NotFound(format!("Dialog turn not found: {}", turn_id)))?;
        let mut turn = self
            .persistence_manager
    ...
    pub async fn fail_dialog_turn(
        ...
        {
            let err_msg: &str = &error;
            error!(
                "[AI00X-DIAG] fail_dialog_turn called: session={}, turn={}, err={}",
                session_id, turn_id, err_msg
            );
        }
        let workspace_path = self
            .effective_session_workspace_path(session_id)
    ...
    pub async fn persist_btw_turn(
        ...
        let session = self.sessions.get(child_session_id).ok_or_else(|| {
            Ai00XError::NotFound(format!("Session not found: {}", child_session_id))
        })?;
    ...
    pub async fn get_context_messages(&self, session_id: &str) -> Ai00XResult<Vec<Message>> {
        let context_messages = self.context_store.get_context_messages(session_id);
    ...
    pub async fn add_message(&self, session_id: &str, message: Message) -> Ai00XResult<()> {
    ...
    pub async fn update_compression_state(
        ...
        if let Some(mut session) = self.sessions.get_mut(session_id) {
            ...
            Err(Ai00XError::NotFound(format!(
                "Session not found: {}",
                session_id
            )))
        }
    ...
    async fn try_generate_session_title_with_ai(
        ...
        let ai_client_factory = get_global_ai_client_factory()
            .await
            .map_err(|e| Ai00XError::AIClient(format!("Failed to get AI client factory: {}", e)))?;

        let ai_client = ai_client_factory
            .get_client_by_func_agent("session-title-func-agent")
            .await
            .map_err(|e| Ai00XError::AIClient(format!("Failed to get AI client: {}", e)))?;

        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| Ai00XError::ai(format!("AI call failed: {}", e)))?;
    ...
    /// Compact a wallpaper session by removing turns before the second-to-last
    /// Write/Edit tool call. Keeps only the dialog from that turn onwards.
    ///
    /// Returns (compacted: bool, removed_turns: usize).
    /// If fewer than 2 Write/Edit turns exist, no compaction is performed.
    pub async fn compact_to_last_file_write(
        &self,
        workspace_path: &Path,
        session_id: &str,
    ) -> Ai00XResult<(bool, usize)> {
        // Ensure session is in memory
        if !self.sessions.contains_key(session_id) && self.config.enable_persistence {
            let _ = self.restore_session(workspace_path, session_id).await;
        }

        // Load persisted turns to inspect tool calls
        let turns = self
            .persistence_manager
            .load_session_turns(workspace_path, session_id)
            .await?;
        ...
        // Find all turn indices that contain Write or Edit tool calls
        let file_write_turn_indices: Vec<usize> = turns
            .iter()
            .enumerate()
            .filter(|(_, turn)| {
                turn.model_rounds.iter().any(|round| {
                    round
                        .tool_items
                        .iter()
                        .any(|tool| tool.tool_name == "Write" || tool.tool_name == "Edit")
                })
            })
            .map(|(idx, _)| idx)
            .collect();

        // Need at least 2 Write/Edit turns to compact
        if file_write_turn_indices.len() < 2 {
            return Ok((false, 0));
        }

        // Second-to-last Write/Edit turn index
        let keep_from_index = file_write_turn_indices[file_write_turn_indices.len() - 2];
        let removed_count = keep_from_index;
        ...
        info!(
            "Compacting wallpaper session {}: removing {} turns before index {}",
            session_id, removed_count, keep_from_index
        );

        // Rollback context to the start of the keep_from_index turn
        // This truncates turns and snapshots from keep_from_index onwards,
        // but we want to KEEP from keep_from_index onwards.
        // So we need a different approach: rebuild context from turns[keep_from_index..]
        // and truncate the session's turn list.

        // 1) Load the context snapshot at the turn just before keep_from_index
        let messages = if keep_from_index == 0 {
            Vec::new()
        } else {
            self.persistence_manager
                .load_turn_context_snapshot(workspace_path, session_id, keep_from_index - 1)
                .await?
                .unwrap_or_default()
        };

        // 2) Replace in-memory context
        self.context_store.replace_context(session_id, messages);

        // 3) Truncate session turn list to remove turns before keep_from_index
        let mut removed = 0;
        if let Some(mut session) = self.sessions.get_mut(session_id) {
            if session.dialog_turn_ids.len() > keep_from_index {
                removed = keep_from_index;
                session.dialog_turn_ids = session.dialog_turn_ids.split_off(keep_from_index);
            }
            session.state = SessionState::Idle;
            session.updated_at = SystemTime::now();
            session.last_activity_at = SystemTime::now();
        ...
        // 4) Delete turn files and snapshots before keep_from_index
        if self.config.enable_persistence {
            // Delete turn files for removed turns
            for i in 0..keep_from_index {
                let _ = self
                    .persistence_manager
                    .delete_turn_data(workspace_path, session_id, i)
                    .await;
            }
            // Delete context snapshots for removed turns
            self.persistence_manager
                .delete_turn_context_snapshots_before(workspace_path, session_id, keep_from_index)
                .await?;

            // Renumber remaining turn and snapshot files to sequential indices
            self.persistence_manager
                .renumber_turns_after_compaction(workspace_path, session_id, removed)
                .await?;
        }
