...
use crate::agent::events::{
    AgentEvent, EventPriority, EventQueue, SubagentParentInfo as EventSubagentParentInfo,
    ToolEventData,
};
use crate::agent::tools::SubagentParentInfo;
...
pub struct StreamProcessError {
    pub error: Ai00XError,
    pub has_effective_output: bool,
}

impl StreamProcessError {
    fn new(error: Ai00XError, has_effective_output: bool) -> Self {
        Self {
            error,
...
impl StreamContext {
    ...
    fn finalize_pending_tool_call(
        ...
        if finalized.is_error {
            warn!(
                "Dropping invalid tool call at boundary={}: tool_id={}, tool_name={}, raw_len={}",
                boundary.as_str(),
                finalized.tool_id,
                finalized.tool_name,
...
impl StreamProcessor {
    ...
    async fn check_cancellation(
        ...
        if cancellation_token.is_cancelled() {
            ...
            Some(Err(StreamProcessError::new(
                Ai00XError::Cancelled("Stream processing cancelled".to_string()),
    ...
    async fn handle_tool_call_chunk(
        ...
        debug!(
            "Tool call chunk received: id={:?}, name={:?}, arguments_len={}, is_snapshot={}",
            id,
            name,
        ...
        let effective_tool_id = match id {
            Some(tid) if !tid.is_empty() => tid,
            _ => {
                if ctx.pending_tool_call.has_pending() {
                    ctx.pending_tool_call.tool_id().to_string()
        ...
        if !effective_tool_id.is_empty() {
            ctx.has_effective_output = true;
            let is_new_tool = ctx.pending_tool_call.tool_id() != effective_tool_id;
            if is_new_tool {
                ...
                ctx.pending_tool_call
                    .start_new(effective_tool_id.clone(), name.clone());
    ...
    pub async fn process_stream(
            ...
            tokio::select! {
                ...
                next_result = tokio::time::timeout(chunk_timeout, stream.next()) => {
                    ...
                    if let Some(text) = text {
                        eprintln!("[TRACE] PROC_TEXT len={}", text.len());
                        self.send_thinking_end_if_needed(&mut ctx).await;
                    ...
                    if let Some(tool_call) = tool_call {
                        eprintln!(
                            "[TRACE] PROC_TOOL_CALL id={:?} name={:?} args_len={} is_snap={}",
                            tool_call.id,
                            tool_call.name,
                            tool_call.arguments.as_ref().map(|a| a.len()).unwrap_or(0),
                    ...
                    if finish_reason.is_some() {
                        eprintln!("[TRACE] FINISH_REASON finalizing pending");
        ...
        eprintln!(
            "[TRACE] STREAM_END finalizing pending, has_pending={}",
            ctx.pending_tool_call.has_pending()
        );
        let _ = ctx.finalize_pending_tool_call(ToolCallBoundary::StreamEnd);

        eprintln!(
            "[TRACE] STREAM_END tool_calls_count={} full_text_len={}",
            ctx.tool_calls.len(),
            ctx.full_text.len()
        );
...
mod tests {
    ...
    use crate::agent::events::{EventQueue, EventQueueConfig};
