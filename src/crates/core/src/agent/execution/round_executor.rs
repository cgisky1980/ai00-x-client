...
use crate::agent::core::{Message, ToolCall};
...
use crate::agent::tools::pipeline::{ToolExecutionContext, ToolExecutionOptions, ToolPipeline};
use crate::agent::tools::registry::get_global_tool_registry;
...
impl RoundExecutor {
    ...
    pub async fn execute_round(
    ...
    ) -> Ai00XResult<RoundResult> {
        eprintln!("[TRACE] EXECUTE_ROUND_START round={}", context.round_number);
        ...
        let stream_result = loop {
            ...
            eprintln!(
                "[TRACE] BEFORE_SEND_MESSAGE_STREAM round={}",
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    return Err(Ai00XError::AIClient(err_msg));
                }
            };
            ...
            if cancel_token.is_cancelled() {
                ...
                return Err(Ai00XError::Cancelled("Execution cancelled".to_string()));
            }
        ...
        eprintln!(
            "[TRACE] STREAM_RESULT text_len={} tool_calls={} thinking_len={}",
            stream_result.full_text.len(),
            stream_result.tool_calls.len(),
            stream_result.full_thinking.len()
        );
        ...
        self.emit_event(
            AgentEvent::ModelRoundCompleted {
                session_id: context.session_id.clone(),
                turn_id: context.dialog_turn_id.clone(),
                round_id: round_id.clone(),
                ...
                input_tokens: stream_result
                    .usage
                    .as_ref()
                    .map(|u| u.prompt_token_count as usize),
                output_tokens: stream_result
                    .usage
                    .as_ref()
                    .map(|u| u.candidates_token_count as usize),
                total_tokens: stream_result
                    .usage
                    .as_ref()
        ...
        let tool_results = if let Some(tool_pipeline) = &self.tool_pipeline {
            ...
            let tool_context = ToolExecutionContext {
                ...
                workflow_phase: context.workflow_phase.clone(),
                plans_dir: context.plans_dir.clone(),
            };
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    stream_result
                        .tool_calls
                        .iter()
                        .map(|tc| crate::agent::tools::pipeline::ToolExecutionResult {
                            tool_id: tc.tool_id.clone(),
                            tool_name: tc.tool_name.clone(),
                            result: crate::agent::core::ToolResult {
                                tool_id: tc.tool_id.clone(),
                                tool_name: tc.tool_name.clone(),
    ...
    pub async fn execute_single_tool(
    ...
    ) -> Ai00XResult<Option<crate::agent::core::ToolResult>> {
        let pipeline = match self.tool_pipeline.as_ref() {
            Some(p) => p,
            None => return Ok(None),
        };
        ...
        Ok(results.into_iter().next().map(|r| r.result))
    }
