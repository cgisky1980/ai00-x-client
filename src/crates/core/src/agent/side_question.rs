...
use crate::agent::coordination::ConversationCoordinator;
use crate::agent::core::{Message as CoreMessage, MessageContent, MessageRole};
...
- Reply concisely, matching the question's language.\n"
    ...
    pub async fn ask(
        ...
        if session_id.trim().is_empty() {
            return Err(Ai00XError::Validation("session_id is required".to_string()));
        }
        if question.trim().is_empty() {
            return Err(Ai00XError::Validation("question is required".to_string()));
        }
        ...
        let client = self
            ...
            .await
            .map_err(|e| Ai00XError::service(format!("Failed to create AI client: {}", e)))?;
    ...
    ) -> Ai00XResult<mpsc::UnboundedReceiver<SideQuestionStreamEvent>> {
        if request.request_id.trim().is_empty() {
            return Err(Ai00XError::Validation("request_id is required".to_string()));
        }
