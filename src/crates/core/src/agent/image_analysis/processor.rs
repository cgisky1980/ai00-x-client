...
impl ImageAnalyzer {
    ...
    ) -> Ai00XResult<Vec<ImageAnalysisResult>> {
    ...
    async fn analyze_single_image(
        ...
        workspace_path: Option<PathBuf>,
        ai_client: Arc<AIClient>,
    ) -> Ai00XResult<ImageAnalysisResult> {
        ...
        let ai_response = ai_client.send_message(messages, None).await.map_err(|e| {
            ...
            Ai00XError::service(format!("Image analysis AI call failed: {}", e))
        })?;
    ...
    async fn load_image_from_context(
    ...
    ) -> Ai00XResult<(Vec<u8>, Option<String>)> {
        ...
        Err(Ai00XError::validation("Image context missing path or data"))
    }
