...
pub fn resolve_vision_model_from_ai_config(
    ...
    let model = ai_config
        ...
        .find(|m| m.id == id)
        .cloned()
        .ok_or_else(|| Ai00XError::service(format!("Model not found: {}", id)))?;

    if !model.enabled {
        return Err(Ai00XError::service(format!("Model is disabled: {}", id)));
    }
...
pub async fn resolve_vision_model_from_global_config() -> Ai00XResult<AIModelConfig> {
    let config_service = get_global_config_service()?;
    let ai_config: ServiceAIConfig = config_service
        .get_config(Some("ai"))
        .await
        .map_err(|e| Ai00XError::service(format!("Failed to get AI config: {}", e)))?;

    resolve_vision_model_from_ai_config(&ai_config)
}

pub fn resolve_image_path(path: &str, workspace_path: Option<&Path>) -> Ai00XResult<PathBuf> {
...
pub async fn load_image_from_path(
    ...
    fs::read(path)
        .await
        .map_err(|e| Ai00XError::io(format!("Failed to read image: {}", e)))
}

pub fn decode_data_url(data_url: &str) -> Ai00XResult<(Vec<u8>, Option<String>)> {
    if !data_url.starts_with("data:") {
        return Err(Ai00XError::validation("Invalid data URL format"));
    }

    let parts: Vec<&str> = data_url.splitn(2, ',').collect();
    if parts.len() != 2 {
        return Err(Ai00XError::validation("Data URL format error"));
    }
    ...
    let image_data = BASE64
        .decode(base64_data)
        .map_err(|e| Ai00XError::parse(format!("Base64 decode failed: {}", e)))?;
...
pub fn optimize_image_with_size_limit(
    ...
    let dynamic = image::load_from_memory(&image_data)
        .map_err(|e| Ai00XError::validation(format!("Failed to decode image data: {}", e)))?;
...
) -> Ai00XResult<Vec<ProcessedImage>> {
    let limits = ImageLimits::for_provider(provider);
    ...
    match target_format {
        ImageFormat::Png => {
            ...
            encoder
                ...
                .map_err(|e| Ai00XError::tool(format!("PNG encode failed: {}", e)))?;
        }
        ImageFormat::Jpeg => {
            let mut encoder = JpegEncoder::new_with_quality(&mut buffer, jpeg_quality);
            encoder
                .encode_image(image)
                .map_err(|e| Ai00XError::tool(format!("JPEG encode failed: {}", e)))?;
        }
