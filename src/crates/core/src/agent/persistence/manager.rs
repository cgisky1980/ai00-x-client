...
struct StoredSessionStateFile {
    ...
    workflow_phase: Option<crate::agent::core::WorkflowPhaseState>,
}
...
impl PersistenceManager {
    pub fn new(path_manager: Arc<PathManager>) -> Ai00XResult<Self> {
        Ok(Self {
    ...
    async fn ensure_runtime_for_write(&self, workspace_path: &Path) -> Ai00XResult<()> {
    ...
    async fn ensure_session_dir(
        ...
        fs::create_dir_all(&dir)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to create session directory: {}", e)))?;
        Ok(dir)
    }
    ...
    async fn ensure_turns_dir(
        ...
        fs::create_dir_all(&dir)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to create turns directory: {}", e)))?;
        Ok(dir)
    }
    ...
    async fn ensure_snapshots_dir(
        ...
        fs::create_dir_all(&dir)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to create snapshots directory: {}", e)))?;
        Ok(dir)
    }
    ...
    async fn ensure_artifacts_dir(
        ...
        fs::create_dir_all(&dir)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to create artifacts directory: {}", e)))?;
        Ok(dir)
    }

    async fn read_json_optional<T: DeserializeOwned>(&self, path: &Path) -> Ai00XResult<Option<T>> {
        if !path.exists() {
            return Ok(None);
        }
        ...
        let value = serde_json::from_str::<T>(&content).map_err(|e| {
            Ai00XError::Deserialization(format!(
    ...
    async fn write_json_atomic<T: Serialize>(&self, path: &Path, value: &T) -> Ai00XResult<()> {
        ...
        fs::create_dir_all(parent)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to create parent directory: {}", e)))?;

        let json = serde_json::to_string_pretty(value)
            .map_err(|e| Ai00XError::serialization(format!("Failed to serialize JSON: {}", e)))?;
    ...
    fn build_temp_json_path(path: &Path, attempt: usize) -> Ai00XResult<PathBuf> {
        let parent = path.parent().ok_or_else(|| {
            Ai00XError::io(format!(
                "Target path has no parent directory: {}",
                path.display()
    ...
    async fn build_session_metadata(
        ...
        SessionMetadata {
            ...
            plan_rating: existing.and_then(|v| v.plan_rating),
            plan_feedback: existing.and_then(|v| v.plan_feedback.clone()),
            complete_rating: existing.and_then(|v| v.complete_rating),
            complete_feedback: existing.and_then(|v| v.complete_feedback.clone()),
            rated_at: existing.and_then(|v| v.rated_at),
            workspace_path: Some(workspace_root),
            workspace_hostname,
            sandbox_branch: session
                .config
                .sandbox_branch
                .clone()
                .or_else(|| existing.and_then(|value| value.sandbox_branch.clone())),
        }
    }
    ...
    fn transcript_fingerprint(
        ...
        let bytes = serde_json::to_vec(&payload).map_err(|e| {
            Ai00XError::serialization(format!("Failed to serialize transcript fingerprint: {}", e))
        })?;
    ...
    fn parse_transcript_turn_selectors(
        selectors: &[String],
    ) -> Ai00XResult<Vec<ParsedTranscriptTurnSelector>> {
    ...
    fn parse_transcript_turn_selector(selector: &str) -> Ai00XResult<ParsedTranscriptTurnSelector> {
    ...
    fn parse_transcript_turn_value(value: &str, selector: &str) -> Ai00XResult<isize> {
    ...
    async fn scan_session_metadata_dirs(
        &self,
        workspace_path: &Path,
    ) -> Ai00XResult<Vec<SessionMetadata>> {
        ...
        let mut entries = fs::read_dir(&sessions_root)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to read sessions root: {}", e)))?;

        while let Some(entry) = entries
            .next_entry()
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to read session directory entry: {}", e)))?
        {
            let file_type = entry
                .file_type()
                .await
                .map_err(|e| Ai00XError::io(format!("Failed to get file type: {}", e)))?;
            if !file_type.is_dir() {
                continue;
            }
    ...
    async fn upsert_index_entry_locked(
        ...
        index
            .sessions
            .sort_by_key(|b| std::cmp::Reverse(b.last_active_at));
    ...
    async fn remove_index_entry(&self, workspace_path: &Path, session_id: &str) -> Ai00XResult<()> {
        let lock = self.get_session_index_lock(workspace_path).await;
        let _guard = lock.lock().await;
    ...
    pub async fn load_session_metadata(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> Ai00XResult<Option<SessionMetadata>> {
    ...
    async fn load_stored_session_state(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> Ai00XResult<Option<StoredSessionStateFile>> {
    ...
    pub async fn load_turn_context_snapshot(
        ...
        session_id: &str,
        turn_index: usize,
    ) -> Ai00XResult<Option<Vec<Message>>> {
        let snapshot = self
    ...
    pub async fn load_latest_turn_context_snapshot(
        ...
        let mut rd = fs::read_dir(&dir)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to read snapshots directory: {}", e)))?;

        while let Some(entry) = rd
            .next_entry()
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to iterate snapshots directory: {}", e)))?
        {
            let path = entry.path();
    ...
    /// Save session
    pub async fn save_session(&self, workspace_path: &Path, session: &Session) -> Ai00XResult<()> {
        self.ensure_runtime_for_write(workspace_path).await?;
        ...
        let state = StoredSessionStateFile {
            ...
            workflow_phase: session.workflow_phase.clone(),
        };
    ...
    pub async fn load_session(
        ...
        let metadata = self
            ...
            .await?
            .ok_or_else(|| {
                Ai00XError::NotFound(format!("Session metadata not found: {}", session_id))
            })?;
        ...
        let workflow_phase = stored_state
            .as_ref()
            .and_then(|value| value.workflow_phase.clone());
    ...
    pub async fn delete_session(&self, workspace_path: &Path, session_id: &str) -> Ai00XResult<()> {
        ...
        if dir.exists() {
            fs::remove_dir_all(&dir).await.map_err(|e| {
                Ai00XError::io(format!("Failed to delete session directory: {}", e))
            })?;
        }
    ...
    pub async fn list_sessions(&self, workspace_path: &Path) -> Ai00XResult<Vec<SessionSummary>> {
        ...
        for metadata in metadata_list {
            ...
            summaries.push(SessionSummary {
                ...
                sandbox_branch: metadata.sandbox_branch.clone(),
    ...
    pub async fn save_dialog_turn(
        ...
        let mut metadata = self
            ...
            .await?
            .ok_or_else(|| {
                Ai00XError::NotFound(format!("Session metadata not found: {}", turn.session_id))
            })?;
    ...
    pub async fn load_dialog_turn(
        ...
        session_id: &str,
        turn_index: usize,
    ) -> Ai00XResult<Option<DialogTurnData>> {
        Ok(self
    ...
    pub async fn load_session_turns(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> Ai00XResult<Vec<DialogTurnData>> {
        ...
        let mut entries = fs::read_dir(&turns_dir)
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to read turns directory: {}", e)))?;

        while let Some(entry) = entries
            .next_entry()
            .await
            .map_err(|e| Ai00XError::io(format!("Failed to iterate turns directory: {}", e)))?
        {
            let path = entry.path();
            if path.extension().and_then(|value| value.to_str()) != Some("json") {
                continue;
        ...
        for (_, path) in indexed_paths {
            if let Some(file) = self
                ...
                .await?
            {
                let mut turn = file.turn;
                if turn.status == TurnStatus::InProgress {
                    let now_ms = SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap_or_default()
                        .as_millis() as u64;
                    let staleness_ms = now_ms.saturating_sub(turn.start_time);
                    if staleness_ms < 30_000 {
                        info!(
                            "Skipping stale recovery for recent turn: session={}, turn_index={}, age_ms={}",
                            session_id, turn.turn_index, staleness_ms
                        );
                        turns.push(turn);
                        continue;
                    }
                    let had_end_time = turn.end_time.is_some();
                    let had_error = turn.error.is_some();
                    warn!(
                        "Stale InProgress turn detected: session={}, turn_index={}, had_end_time={}, had_error={}",
                        session_id, turn.turn_index, had_end_time, had_error
                    );
                    turn.status = TurnStatus::Error;
                    if turn.error.is_none() {
                        turn.error =
                            Some("Stale InProgress turn recovered after restart".to_string());
                    }
                    if turn.end_time.is_none() {
                        turn.end_time = Some(
                            SystemTime::now()
                                .duration_since(std::time::UNIX_EPOCH)
                                .unwrap_or_default()
                    ...
                    let stale_file = StoredDialogTurnFile {
                        schema_version: SESSION_SCHEMA_VERSION,
                        turn: turn.clone(),
                    };
                    if let Err(e) = self.write_json_atomic(&path, &stale_file).await {
                        warn!(
                            "Failed to save sanitized stale turn: session={}, turn_index={}, error={}",
    ...
    pub async fn export_session_transcript(
        ...
        session_id: &str,
        options: &SessionTranscriptExportOptions,
    ) -> Ai00XResult<SessionTranscriptExport> {
        if self
            .load_session_metadata(workspace_path, session_id)
            .await?
            .is_none()
        {
            return Err(Ai00XError::NotFound(format!(
    ...
    pub async fn delete_turns_after(
            ...
            .filter(|value| value.turn_index > turn_index)
        {
            ...
            if path.exists() {
                fs::remove_file(&path)
                    .await
                    .map_err(|e| Ai00XError::io(format!("Failed to delete turn file: {}", e)))?;
                deleted += 1;
            }
        }
    ...
    pub async fn touch_session(&self, workspace_path: &Path, session_id: &str) -> Ai00XResult<()> {
        if let Some(mut metadata) = self
            .load_session_metadata(workspace_path, session_id)
            .await?
        {
    ...
    /// Delete a single turn data file by index.
    pub async fn delete_turn_data(
        &self,
        workspace_path: &Path,
        session_id: &str,
        turn_index: usize,
    ) -> Ai00XResult<()> {
        let path = self.turn_path(workspace_path, session_id, turn_index);
        if path.exists() {
            fs::remove_file(&path)
                .await
                .map_err(|e| Ai00XError::io(format!("Failed to delete turn file: {}", e)))?;
    ...
    /// Delete context snapshot files with index strictly less than `before_index`.
    pub async fn delete_turn_context_snapshots_before(
        &self,
        workspace_path: &Path,
        session_id: &str,
        before_index: usize,
    ...
    pub async fn renumber_turns_after_compaction(
        &self,
        workspace_path: &Path,
        session_id: &str,
        removed_count: usize,
        ...
        for (seq, (old_index, old_path)) in indexed_paths.iter().enumerate() {
            let new_index = seq;
            ...
            let Some(file) = self
                .read_json_optional::<StoredDialogTurnFile>(old_path)
                .await?
            else {
                continue;
            };
            ...
            let mut turn = file.turn;
            turn.turn_index = new_index;

            let new_file = StoredDialogTurnFile {
                schema_version: SESSION_SCHEMA_VERSION,
                turn,
            };

            let new_path = self.turn_path(workspace_path, session_id, new_index);
            self.write_json_atomic(&new_path, &new_file).await?;
        ...
        if renumbered > 0 {
            info!(
                "Renumbered {} turn files after compaction: session={}",
                renumbered, session_id
            );
        }

        let snapshots_dir = self.snapshots_dir(workspace_path, session_id);
        if snapshots_dir.exists() {
            let mut snap_paths = Vec::new();
            let mut snap_entries = fs::read_dir(&snapshots_dir).await.map_err(|e| {
                Ai00XError::io(format!("Failed to read snapshots directory: {}", e))
            })?;

            while let Some(entry) = snap_entries.next_entry().await.map_err(|e| {
                Ai00XError::io(format!("Failed to iterate snapshots directory: {}", e))
            })? {
                let path = entry.path();
                if path.extension().and_then(|v| v.to_str()) != Some("json") {
                    continue;
                ...
                snap_paths.push((index, path));
            }

            snap_paths.sort_by_key(|(index, _)| *index);

            for (seq, (old_index, old_path)) in snap_paths.iter().enumerate() {
                let new_index = seq;
                if *old_index == new_index {
                    continue;
                }

                let new_path = snapshots_dir.join(format!("context-{:04}.json", new_index));
    ...
    impl TestWorkspace {
        fn new() -> Self {
            let path = std::env::temp_dir()
                .join(format!("ai00-x-session-transcript-test-{}", Uuid::new_v4()));
