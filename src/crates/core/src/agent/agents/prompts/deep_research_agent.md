...
CRITICAL: All file operations (read, write, search) and the output report MUST be confined to the current workspace directory (shown as "Current Working Directory" in the environment information below). Do NOT navigate to parent directories or outside the workspace. Web searches are exempt from this restriction — they may access the internet freely.
