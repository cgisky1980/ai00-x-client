use super::{
    Agent, CodeAgent, CodeReviewAgent, DebugMode, DeepResearchAgent, ExploreAgent, FileFinderAgent,
    GenerateDocAgent, InitAgent, PlanMode, RouterAgent, TaskAgent, WallpaperAgent,
};
use crate::agent::agents::custom_subagents::{
    CustomSubagent, CustomSubagentKind, CustomSubagentLoader,
};
use crate::agent::tools::get_all_registered_tool_names;
...
pub struct AgentInfo {
    ...
    /// whether enabled (Core always true, others from configuration)
    pub enabled: bool,
...
fn default_model_id_for_builtin_agent(agent_type: &str) -> &'static str {
    match agent_type {
        "Code" | "Task" | "Wallpaper" | "Core" | "Plan" | "DeepResearch" => "primary",
        "debug" => "primary",
        // TODO: fast/RWKV temporarily disabled for sub-agents — re-enable and verify individually later
        "Router" | "Explore" | "FileFinder" | "CodeReview" | "GenerateDoc" => "primary",
        _ => "primary",
    }
}
...
impl AgentRegistry {
    ...
    pub fn new() -> Self {
        ...
        let modes: Vec<Arc<dyn Agent>> = vec![
            ...
            Arc::new(TaskAgent::new()),
            Arc::new(WallpaperAgent::new()),
        ];
        for mode in modes {
        ...
        let hidden_subagents: Vec<Arc<dyn Agent>> = vec![Arc::new(InitAgent::new())];
    ...
    pub async fn get_modes_info(&self) -> Vec<AgentInfo> {
        let mode_configs = get_mode_configs().await;
        let top_level_ids = ["Code", "Task", "Wallpaper"];
        ...
        let mut result: Vec<AgentInfo> = map
            ...
            .map(|e| {
                ...
                agent_info.enabled = if top_level_ids.contains(&agent_type.as_str()) {
                    true
                } else {
                    mode_configs
                        .get(agent_type)
    ...
    pub fn update_and_save_custom_subagent_config(
        ...
        let entry = if let Some(entry) = map.get_mut(agent_id) {
            ...
            project_maps
                ...
                .ok_or_else(|| Ai00XError::agent(format!("Subagent not found: {}", agent_id)))?
        };

        if entry.category != AgentCategory::SubAgent {
            return Err(Ai00XError::agent(format!(
        ...
        let config = entry.custom_config.as_mut().ok_or_else(|| {
            Ai00XError::agent(format!("Subagent '{}' is not a custom subagent", agent_id))
        })?;
    ...
    pub async fn get_custom_subagent_detail(
        ...
        agent_id: &str,
        workspace_root: Option<&Path>,
    ) -> Ai00XResult<CustomSubagentDetail> {
    ...
    fn replace_custom_subagent_entry(
        ...
        let root = workspace_root.ok_or_else(|| {
            Ai00XError::agent("Workspace path is required to update project subagent".to_string())
        })?;
        let mut pm = self.write_project_subagents();
        let entries = pm.get_mut(root).ok_or_else(|| {
            Ai00XError::agent("Project subagent cache not loaded for this workspace".to_string())
        })?;
    ...
    pub fn remove_subagent(&self, agent_id: &str) -> Ai00XResult<Option<String>> {
        let mut map = self.write_agents();
...
mod tests {
    ...
    #[test]
    fn core_mode_defaults_to_primary() {
        assert_eq!(default_model_id_for_builtin_agent("Core"), "primary");
    }

    #[test]
    fn router_defaults_to_primary() {
        assert_eq!(default_model_id_for_builtin_agent("Router"), "primary");
    }

    #[test]
    fn subagent_modes_default_to_primary() {
        assert_eq!(default_model_id_for_builtin_agent("Plan"), "primary");
        assert_eq!(default_model_id_for_builtin_agent("debug"), "primary");
        assert_eq!(
            default_model_id_for_builtin_agent("DeepResearch"),
            "primary"
        );
        assert_eq!(default_model_id_for_builtin_agent("CodeReview"), "primary");
        assert_eq!(default_model_id_for_builtin_agent("GenerateDoc"), "primary");
    }

    #[test]
    fn explorer_and_filefinder_default_to_primary() {
        assert_eq!(default_model_id_for_builtin_agent("Explore"), "primary");
        assert_eq!(default_model_id_for_builtin_agent("FileFinder"), "primary");
    }
