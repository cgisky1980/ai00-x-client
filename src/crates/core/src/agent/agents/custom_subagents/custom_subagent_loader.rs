...
/// Project subagent directory names (relative to workspace root, each item is in [".ai00-x", "agents"] format)
...
impl CustomSubagentLoader {
    ...
    /// - Project subagents: .ai00-x/agents, .claude/agents, .cursor/agents, .codex/agents under workspace
    /// - User subagents: agents under ai00-x user config, ~/.claude/agents, ~/.cursor/agents, ~/.codex/agents
    pub fn get_possible_paths(workspace_root: &Path) -> Vec<SubagentDirEntry> {
        ...
        // User subagents: agents under ai00-x user config
        let pm = get_path_manager_arc();
        let ai00x_agents = pm.user_agents_dir();
        if ai00x_agents.exists() && ai00x_agents.is_dir() {
            entries.push(SubagentDirEntry {
                path: ai00x_agents,
        ...
        if let Some(home) = dirs::home_dir() {
            for (parent, sub) in PROJECT_AGENT_SUBDIRS {
                if *parent == ".ai00-x" {
                    continue; // ai00-x user path already handled by path_manager
                }
