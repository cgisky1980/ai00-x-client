...
use crate::agent::agents::{PromptBuilder, PromptBuilderContext};
...
impl CustomSubagent {
    ...
    pub fn from_file(path: &str, kind: CustomSubagentKind) -> Ai00XResult<Self> {
        let (metadata, content) = FrontMatterMarkdown::load(path)?;
        let name = metadata
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::Agent("Missing name field".to_string()))?
            .to_string();
        let description = metadata
            .get("description")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Ai00XError::Agent("Missing description field".to_string()))?
            .to_string();
    ...
    pub fn save_to_file(&self, enabled: Option<bool>, model: Option<&str>) -> Ai00XResult<()> {
        ...
        FrontMatterMarkdown::save(&self.path, &metadata, &self.prompt).map_err(Ai00XError::Agent)
    }
}
