...
use crate::agent::core::{ProcessingPhase, SessionState};
...
impl SessionStateManager {
    ...
    async fn emit_state_change_event(&self, session_id: &str, state: SessionState) {
        let event = AgentEvent::SessionStateChanged {
            session_id: session_id.to_string(),
            new_state: crate::agent::events::types::session_state_to_string(&state),
        };
