...
use crate::git_types::*;
use crate::git_utils::*;
use crate::graph::{build_git_graph, build_git_graph_for_branch, GitGraph};
...
impl GitService {
    ...
    pub fn init_repository<P: AsRef<Path>>(path: P) -> Result<(), GitError> {
    ...
    fn parse_worktree_list(output: &str) -> Result<Vec<GitWorktreeInfo>, GitError> {
    ...
    /// - `base_branch`: When creating a new branch, base it on this branch (e.g. "main")
    ///
    /// # Returns
    ...
    pub async fn add_worktree<P: AsRef<Path>>(
        ...
        let worktree_dir = path.as_ref().join(".tasks");
        let worktree_path = worktree_dir.join(branch_to_dir_name(branch));
        ...
        let mut args = if create_branch {
            vec!["worktree", "add", "-b", branch]
        } else {
            vec!["worktree", "add"]
        };
    ...
    /// - `target_branch`: Optional target branch to merge into (defaults to current branch)
    ///
    /// # Returns
    /// - Operation result
    pub async fn merge_branch<P: AsRef<Path>>(
        repo_path: P,
        branch: &str,
        target_branch: Option<&str>,
    ) -> Result<GitOperationResult, GitError> {
        let start_time = Instant::now();
        let repo_path_str = repo_path.as_ref().to_string_lossy();

        if let Some(target) = target_branch {
            let args = &["checkout", target];
            execute_git_command(&repo_path_str, args).await?;
        }
        ...
        let output = execute_git_command(&repo_path_str, args).await?;
        let duration = start_time.elapsed().as_millis() as u64;
    ...
    /// Checks whether the repository has merge conflicts.
    ///
    /// # Parameters
    /// - `repo_path`: Repository path
    ///
    /// # Returns
    /// - `true` if there are unmerged paths (conflicts)
    pub async fn has_conflicts<P: AsRef<Path>>(repo_path: P) -> Result<bool, GitError> {
        let repo_path_str = repo_path.as_ref().to_string_lossy();
    ...
    /// # Returns
    /// - Operation result
    pub async fn abort_merge<P: AsRef<Path>>(repo_path: P) -> Result<GitOperationResult, GitError> {
        let start_time = Instant::now();
        let repo_path_str = repo_path.as_ref().to_string_lossy();
    ...
    pub async fn stash<P: AsRef<Path>>(
        repo_path: P,
        message: Option<&str>,
    ) -> Result<GitOperationResult, GitError> {
        let start_time = Instant::now();
        ...
        let args = if let Some(msg) = message {
            execute_git_command(&repo_path_str, &["stash", "push", "-m", msg]).await?
        } else {
            execute_git_command(&repo_path_str, &["stash", "push"]).await?
        };
        let duration = start_time.elapsed().as_millis() as u64;
    ...
    pub async fn stash_pop<P: AsRef<Path>>(repo_path: P) -> Result<GitOperationResult, GitError> {
        let start_time = Instant::now();
        let repo_path_str = repo_path.as_ref().to_string_lossy();

        let output = execute_git_command(&repo_path_str, &["stash", "pop"]).await?;
        let duration = start_time.elapsed().as_millis() as u64;
