 ...
 */
use crate::git_types::{GitError, GitFileStatus};
...
fn create_command<S: AsRef<std::ffi::OsStr>>(program: S) -> std::process::Command {
    let cmd = std::process::Command::new(program.as_ref());
...
fn create_tokio_command<S: AsRef<std::ffi::OsStr>>(program: S) -> tokio::process::Command {
    let cmd = tokio::process::Command::new(program.as_ref());
...
/// Returns the current branch name for a given path, or None if not a git repo.
pub fn get_current_branch_from_path<P: AsRef<Path>>(path: P) -> Option<String> {
...
pub async fn execute_git_command(repo_path: &str, args: &[&str]) -> Result<String, GitError> {
    let output = create_tokio_command("git")
