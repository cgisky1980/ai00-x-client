...
pub fn resolve_request_url(base_url: &str, provider: &str, model_name: &str) -> String {
    ...
    match provider.trim().to_ascii_lowercase().as_str() {
        "openai" | "nvidia" | "openrouter" | "xfyun" => {
            append_endpoint(&trimmed, "chat/completions")
        }
        ...
        // Ai00-Salvo: chat 走 Rust handler `/api/v1/ai/chat`（脚本引擎不支持 SSE 流式）
        // 其他端点（me/models 等）仍走 `/ai00-s/api/*` 脚本
        "ai00s" => append_endpoint(&trimmed, "api/v1/ai/chat"),
        "rwkv" => String::new(),
        _ => trimmed,
    }
}
...
mod tests {
    ...
    fn resolves_gemini_request_url_bare_host() {
        assert_eq!(
            resolve_request_url("https://example.com", "gemini", "gemini-2.5-pro"),
