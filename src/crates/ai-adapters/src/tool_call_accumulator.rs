...
impl PendingToolCall {
                    ...
                    match serde_json::from_str::<Value>(&repaired_arguments) {
                        Ok(arguments) => {
                            if arguments.is_object() {
                                ...
                                Err(format!(
                                    "Tool call arguments must be a JSON object, got {} (repaired)",
                                    match &arguments {
                                        Value::Null => "null",
                                        Value::Bool(_) => "a boolean",
                                        Value::Number(_) => "a number",
    ...
    pub fn finalize(&mut self, boundary: ToolCallBoundary) -> Option<FinalizedToolCall> {
        ...
        if is_error {
            eprintln!(
                "[TRACE] TOOL_CALL_ACCUM_FINALIZE_ERROR tool_name={} raw_args_len={} raw_args={:?}",
                tool_name,
                raw_arguments.len(),
            ...
            eprintln!(
                "[TRACE] TOOL_CALL_ACCUM_FINALIZE_OK tool_name={} raw_args_len={}",
                tool_name,
                raw_arguments.len()
            );
        }
...
mod tests {
    ...
    #[test]
    fn non_object_arguments_become_error() {
        let mut pending = PendingToolCall::default();
        pending.start_new("call_1".to_string(), Some("tool_a".to_string()));
        pending.append_arguments("202");
    ...
    #[test]
    fn string_arguments_become_error() {
        let mut pending = PendingToolCall::default();
        pending.start_new("call_1".to_string(), Some("tool_a".to_string()));
        pending.append_arguments("\"hello\"");
    ...
    #[test]
    fn array_arguments_become_error() {
        let mut pending = PendingToolCall::default();
        pending.start_new("call_1".to_string(), Some("tool_a".to_string()));
        pending.append_arguments("[1,2,3]");
