...
impl OpenAIMessageConverter {
    /// Returns true for fabricated tool_call_ids that do not correspond to any
    /// assistant tool_call. Historically the execution engine emitted a synthetic
    /// "MultiToolMerge" tool result with id like "merged-WebFetch-WebFetch-<ts>"
    /// which violates the OpenAI Chat Completions contract (every tool message's
    /// tool_call_id must match a preceding assistant tool_call id). Filter those
    /// out here so already-corrupted session histories still send successfully.
    fn is_orphan_merged_tool_id(tool_call_id: &Option<String>) -> bool {
        match tool_call_id {
            Some(id) => id.starts_with("merged-"),
            None => false,
        }
    }
    ...
    pub fn convert_messages(messages: Vec<Message>) -> Vec<Value> {
        messages
            .into_iter()
            .filter(|msg| {
                // Drop synthetic merged tool results whose fabricated tool_call_id
                // ("merged-...") would not match any assistant tool_call, causing
                // OpenAI 400 "tool_call_ids did not have response messages".
                if msg.role == "tool" && Self::is_orphan_merged_tool_id(&msg.tool_call_id) {
                    warn!(
                        "Dropping orphan merged tool message from history (tool_call_id={:?})",
                        msg.tool_call_id
                    );
                    return false;
                }
    ...
    fn convert_tool_message_to_responses_item(msg: Message) -> Option<Value> {
        ...
        // Drop synthetic merged tool results (see is_orphan_merged_tool_id doc).
        if call_id.starts_with("merged-") {
            warn!(
                "Dropping orphan merged tool message from Responses input (call_id={})",
                call_id
            );
            return None;
        }
