    ...
    ),
{
    ...
    for attempt in 0..max_tries {
        ...
        eprintln!(
            "[TRACE] SSE_HTTP_SEND url={} attempt={}/{}",
            _url,
            attempt + 1,
            max_tries
        );
        ...
        let response_result = tokio::time::timeout(
            Duration::from_secs(AIClient::HTTP_RESPONSE_TIMEOUT_SECS),
            build_request().json(request_body).send(),
        )
        .await
        .map_err(|_elapsed| {
            anyhow!(
                "{} response timeout after {}s",
                label,
                AIClient::HTTP_RESPONSE_TIMEOUT_SECS
            )
        })
        .and_then(|r| r.map_err(|e| anyhow!("{} request failed: {}", label, e)));

        let response = match response_result {
            Ok(resp) => {
                let connect_time = request_start_time.elapsed().as_millis();
                eprintln!(
                    "[TRACE] SSE_HTTP_RESPONSE status={} time={}ms",
                    resp.status(),
                    connect_time
                );
                ...
                if status.as_u16() == 429 {
                    ...
                    let rate_limit_base_ms = 3000;
                    let delay_ms = if retry_after_secs > 0 {
                        retry_after_secs * 1000
                    } else {
                        rate_limit_base_ms * (1 << attempt.min(3))
                    };
                    warn!(
                        "{} rate limited (429), attempt {}/{}, retrying after {}ms: {}",
                        label,
                        attempt + 1,
                        max_tries,
                        delay_ms,
                    ...
                    last_error = Some(anyhow!("{} rate limited (429): {}", label, error_text));
            ...
            Err(e) => {
                let connect_time = request_start_time.elapsed().as_millis();
                eprintln!("[TRACE] SSE_HTTP_ERROR time={}ms error={}", connect_time, e);
        ...
        eprintln!("[TRACE] SSE_SPAWN_HANDLER");
