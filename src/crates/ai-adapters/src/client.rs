...
use crate::providers::{ai00s, anthropic, gemini, openai, rwkv};
...
impl AIClient {
    ...
    pub async fn send_message_stream_with_extra_body(
        ...
        match ApiFormat::parse(&self.config.format)? {
            ApiFormat::Ai00S => {
                ai00s::request::send_stream(self, messages, tools, extra_body, max_tries).await
            }
            ApiFormat::OpenAIChat => {
                openai::chat::send_stream(self, messages, tools, extra_body, max_tries).await
            }
            ApiFormat::Rwkv => {
                rwkv::request::send_stream(self, messages, tools, extra_body, max_tries).await
            }
