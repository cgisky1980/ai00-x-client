...
Shared AI protocol adapters used by both `ai00-x-core` and the installer.
...
This crate intentionally does **not** own Ai00-X runtime concerns such as:
...
Those remain in `ai00-x-core`, which maps app config into the shared `AIConfig`
...
installer, it belongs here. If it depends on Ai00-X runtime state or services,
