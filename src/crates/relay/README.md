...
WebSocket relay server for Ai00-X Remote Connect. Bridges desktop (WebSocket) and mobile (HTTP) clients with E2E encryption support.
...
git clone https://github.com/GCWing/Ai00-X
cd Ai00-X/src/crates/relay
...
### What URL should I fill in Ai00-X Desktop?
...
```bash
# From project root
cargo build --release -p ai00-x-relay-server

# Run
RELAY_PORT=9700 ./target/release/ai00-x-relay-server
```
...
| `RELAY_ROOM_WEB_DIR` | `/tmp/ai00-x-room-web` | Directory for per-room uploaded mobile-web files |
...
Run these commands on the target server inside `src/crates/relay/`:
...
# First-time setup (creates /opt/ai00-x-relay, copies static/)
...
4. Fill the same URL into Ai00-X Desktop "Custom Server"
