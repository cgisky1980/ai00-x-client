...
bool load_wav(const std::string& path, std::vector<float>& samples, int& sample_rate) {
    ...
    while (file.good()) {
        ...
        if (strncmp(chunk_id, "fmt ", 4) == 0) {
            ...
            // WAVE_FORMAT_EXTENSIBLE (0xFFFE): read the extension to extract
            // the real sub-format GUID. KSDATAFORMAT_SUBTYPE_PCM =
            // {00000001-0000-0010-8000-00AA00389B71}, whose first 4 bytes
            // (little-endian) equal 1 (WAVE_FORMAT_PCM).
            if (audio_format == 0xFFFE && chunk_size >= 40) {
                uint16_t cb_size;
                uint16_t valid_bits;
                uint32_t channel_mask;
                uint8_t sub_format[16];
                file.read(reinterpret_cast<char*>(&cb_size), 2);
                file.read(reinterpret_cast<char*>(&valid_bits), 2);
                file.read(reinterpret_cast<char*>(&channel_mask), 4);
                file.read(reinterpret_cast<char*>(sub_format), 16);

                // First 4 bytes of the GUID hold the format code (little-endian).
                uint32_t sub_format_code = static_cast<uint32_t>(sub_format[0]) |
                                           (static_cast<uint32_t>(sub_format[1]) << 8) |
                                           (static_cast<uint32_t>(sub_format[2]) << 16) |
                                           (static_cast<uint32_t>(sub_format[3]) << 24);
                if (sub_format_code == 1) {
                    is_extensible_pcm = true;
                }

                // Skip any remaining extension bytes
                if (chunk_size > 40) {
                    file.seekg(chunk_size - 40, std::ios::cur);
                }
            ...
            // Found data chunk
            bool is_pcm = (audio_format == 1) || is_extensible_pcm;
            if (!is_pcm) {
