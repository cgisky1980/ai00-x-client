...
mod imp {
    ...
    pub async fn print_page<R: Runtime>(
        ...
        let css = format!(
            r#"(function() {{
                let style = document.getElementById('__ai00_x_webdriver_print_style');
                if (!style) {{
                    style = document.createElement('style');
                    style.id = '__ai00_x_webdriver_print_style';
        ...
        let _ = webview.eval(
            "(() => { document.getElementById('__ai00_x_webdriver_print_style')?.remove(); })();",
        );
        response
    }
