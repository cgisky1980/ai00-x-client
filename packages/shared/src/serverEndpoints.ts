...
/** 主后端 Ai00-Salvo 基地址（正式 = app.ai00-x.com；默认编译指向正式服）（生产域名，XOR 混淆存储） */
export const DEFAULT_AI00_S_BASE_URL = decodeXor([
  50, 46, 46, 42, 41, 96, 117, 117,
  59, 42, 42, 116, 59, 51, 106, 106,
  119, 34, 116, 57, 53, 55
])

/** 测试服务器基地址（ai00-x.com 老机；客户端启动参数 --server=test 时覆盖 ai00_s_base_url，不落盘）（生产域名，XOR 混淆存储） */
export const AI00_S_BASE_URL_TEST = decodeXor([
  50, 46, 46, 42, 41, 96, 117, 117,
...
export const AI00_S_RELAY_URL_TEST = decodeXor([
  50, 46, 46, 42, 41, 96, 117, 117,
  59, 51, 106, 106, 119, 34, 116, 57,
  53, 55
])
...
/** 测试服务器 web 应用地址（--server=test 时覆盖 ai00_s_web_app_url）（生产域名，XOR 混淆存储） */
export const AI00_S_WEB_APP_URL_TEST = decodeXor([
  50, 46, 46, 42, 41, 96, 117, 117,
  59, 51, 106, 106, 119, 34, 116, 57,
  53, 55
])
