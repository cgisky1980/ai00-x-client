/**
 * 内联简体中文语言包（源：src/js/i18n/zh_CN.js）
 *
 * 目的：Vditor 构造函数默认从 CDN 异步拉取 i18n 脚本，init 被推迟到网络加载完成之后，
 * 消费方在 `new Vditor()` 后立即调用 getValue/enable 等方法会命中未初始化的
 * `vditor[mode]` 而抛 "Cannot read properties of undefined (reading 'element')"。
 * 内联后 init 同步执行，消除该竞态；其余语言仍走 CDN 兜底。
 */
export const zhCN: ITips = {
    "alignCenter": "居中",
    "alignLeft": "居左",
    "alignRight": "居右",
    "alternateText": "替代文本",
