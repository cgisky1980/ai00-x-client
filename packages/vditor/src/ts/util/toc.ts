...
export const clickToc = (event: MouseEvent & { target: HTMLElement }, vditor: IVditor) => {
    ...
    if (spanElement && hasClosestByClassName(spanElement, "vditor-toc")) {
        const headingElement = vditor.wysiwyg.element.querySelector("#" + spanElement.getAttribute("data-target-id")) as HTMLElement;
        if (headingElement) {
            if (vditor.options.height === "auto") {
                ...
                vditor.wysiwyg.element.scrollTop = headingElement.offsetTop;
            }
        }
        return;
    }
...
export const keydownToc = (blockElement: HTMLElement, vditor: IVditor, event: KeyboardEvent, range: Range) => {
    ...
    if (blockElement.previousElementSibling &&
        ...
        if (event.key === "Backspace" &&
            getSelectPosition(blockElement, vditor.wysiwyg.element, range).start === 0) {
    ...
    if (blockElement.nextElementSibling &&
        ...
        if (event.key === "Delete" &&
            getSelectPosition(blockElement, vditor.wysiwyg.element, range).start
