...
export const getEditorRange = (vditor: IVditor) => {
    ...
    const lastRange = vditor.wysiwyg.range;
    if (lastRange) {
        return lastRange;
    }
    element.focus();
...
export const insertHTML = (html: string, vditor: IVditor) => {
    ...
    if (tempBlockElement.length === 1 && !tempBlockElement[0].previousSibling && !tempBlockElement[0].nextSibling &&
        vditor.wysiwyg.element.children.length > 0 && tempElement.firstElementChild.tagName === "P") {
    ...
    if (pasteElement.firstElementChild &&
        ...
        if (!blockElement) {
            vditor.wysiwyg.element.insertAdjacentHTML("beforeend", pasteElement.innerHTML);
        } else {
