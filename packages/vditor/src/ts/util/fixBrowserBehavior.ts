...
export const fixGSKeyBackspace = (event: KeyboardEvent, vditor: IVditor, startContainer: Node) => {
    if (event.keyCode === 229 && event.code === "" && event.key === "Unidentified") {
        ...
        if (blockElement && blockElement.textContent.trim() === "") {
            vditor.wysiwyg.composingLock = true;
            return false;
        }
    }
    return true;
...
export const fixCJKPosition = (range: Range, vditor: IVditor, event: KeyboardEvent) => {
    ...
    if (pLiElement && getSelectPosition(pLiElement, vditor.wysiwyg.element, range).start === 0) {
...
export const insertAfterBlock = (vditor: IVditor, event: KeyboardEvent, range: Range, element: HTMLElement,
    ...
    const position = getSelectPosition(element, vditor.wysiwyg.element, range);
    ...
    if (cancel && itemElement) {
        ...
        if (!itemElement) {
            ...
            if (selectedBlockElements.length === 0) {
                ...
                const blockElement = vditor.wysiwyg.element.querySelector("p");
...
export const listIndent = (vditor: IVditor, liElement: HTMLElement, range: Range) => {
    ...
    if (liElement && previousElement) {
        ...
        if (tempTopListElement) {
            tempTopListElement.querySelectorAll(`.vditor-wysiwyg__preview[data-render='2']`)
...
export const fixList = (range: Range, vditor: IVditor, pElement: HTMLElement | false, event: KeyboardEvent) => {
    ...
    if (liElement) {
        ...
        if (!isCtrl(event) && !event.shiftKey && !event.altKey && event.key === "Backspace" &&
            ...
            getSelectPosition(liElement, vditor.wysiwyg.element, range).start === 0) {
...
export const fixMarkdown = (event: KeyboardEvent, vditor: IVditor, pElement: HTMLElement | false, range: Range) => {
    ...
    if (range.collapsed && pElement.previousElementSibling && event.key === "Backspace" &&
        ...
        getSelectPosition(pElement, vditor.wysiwyg.element, range).start === 0) {
...
export const fixCodeBlock = (vditor: IVditor, event: KeyboardEvent, codeRenderElement: HTMLElement, range: Range) => {
    ...
    if (event.key === "Backspace" && !isCtrl(event) && !event.shiftKey && !event.altKey) {
        const codePosition = getSelectPosition(codeRenderElement, vditor.wysiwyg.element, range);
...
export const fixBlockquote = (vditor: IVditor, range: Range, event: KeyboardEvent, pElement: HTMLElement | false) => {
    ...
    if (blockquoteElement && range.toString() === "") {
        ...
        if (event.key === "Backspace" && !isCtrl(event) && !event.shiftKey && !event.altKey &&
            getSelectPosition(blockquoteElement, vditor.wysiwyg.element, range).start === 0) {
        ...
        if (pElement && event.key === "Enter" && !isCtrl(event) && !event.shiftKey && !event.altKey
            ...
            if (pElement.innerHTML.replace(Constants.ZWSP, "") === "\n" ||
                ...
                getSelectPosition(pElement, vditor.wysiwyg.element, range).start ===
...
export const fixDelete = (vditor: IVditor, range: Range, event: KeyboardEvent, pElement: HTMLElement | false) => {
    ...
    if (pElement) {
        ...
        if (previousElement && getSelectPosition(pElement, vditor.wysiwyg.element, range).start === 0 &&
...
export const paste = async (vditor: IVditor, event: (ClipboardEvent | DragEvent) & {target: HTMLElement}, callback: {
    ...
    if (vditor.wysiwyg.element.getAttribute("contenteditable") !== "true") {
        return;
    }
                ...
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        ...
                        const imgElement: HTMLImageElement =
                            vditor.wysiwyg.element.querySelector(`img[src="${original}"]`);
    ...
    // process code
    const height = vditor.wysiwyg.element.scrollHeight;
    const code = processPasteCode(textHTML, textPlain);
    const codeElement = hasClosestByMatchTag(event.target, "CODE");
    const cellElement = hasClosestByMatchTag(getEditorRange(vditor).startContainer, "TD") ||
    ...
    if (cellElement && /[\r\n]/.test(textPlain)) {
        ...
        // 粘贴在代码位置
        const position = getSelectPosition(event.target, vditor.wysiwyg.element);
        ...
        if (codeElement.parentElement?.nextElementSibling.classList
            .contains(`vditor-wysiwyg__preview`)) {
        ...
        if (textHTML.trim() !== "") {
            ...
            const selectedText = getEditorRange(vditor).toString();
    ...
    execAfterRender(vditor);
    if (vditor.wysiwyg.element.scrollHeight - height >
        Math.min(vditor.wysiwyg.element.clientHeight, window.innerHeight) / 2) {
        scrollCenter(vditor);
    }
};
