...
export const initUI = (vditor: IVditor) => {
  ...
  // 单模式精简：初始化直接渲染 wysiwyg 内容
  renderDomByMd(vditor, afterRender(vditor), {
...
export const setPadding = (vditor: IVditor) => {
  ...
  if (vditor.preview.element.style.display !== "block") {
    vditor.toolbar.element.style.paddingLeft = Math.max(5,
      parseInt(vditor.wysiwyg.element.style.paddingLeft || "0", 10) +
...
export const setTypewriterPosition = (vditor: IVditor) => {
  ...
  vditor.wysiwyg.element.style.setProperty("--editor-bottom", editorBottom);
};
