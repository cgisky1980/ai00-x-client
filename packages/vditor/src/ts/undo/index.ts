...
class Undo {
    ...
    public resetIcon(vditor: IVditor) {
        ...
        if (this.wysiwyg.undoStack.length > 1) {
            enableToolbar(vditor.toolbar.elements, ["undo"]);
        } else {
            disableToolbar(vditor.toolbar.elements, ["undo"]);
        }

        if (this.wysiwyg.redoStack.length !== 0) {
    ...
    public undo(vditor: IVditor) {
        ...
        if (this.wysiwyg.undoStack.length < 2) {
            return;
        }
        const state = this.wysiwyg.undoStack.pop();
        if (!state) {
            return;
        }
        this.wysiwyg.redoStack.push(state);
        this.renderDiff(state, vditor);
        this.wysiwyg.hasUndo = true;
        // undo 操作后，需要关闭 hint
    ...
    public redo(vditor: IVditor) {
        ...
        const state = this.wysiwyg.redoStack.pop();
        if (!state) {
            return;
        }
        this.wysiwyg.undoStack.push(state);
    ...
    public recordFirstPosition(vditor: IVditor, event: KeyboardEvent) {
        ...
        if (this.wysiwyg.undoStack.length !== 1 || this.wysiwyg.undoStack[0].length === 0 ||
            this.wysiwyg.redoStack.length > 0) {
            return;
        }
        ...
        if (text.replace("<wbr>", "").replace(" vditor-ir__node--expand", "")
            !== this.wysiwyg.undoStack[0][0].diffs[0][1].replace("<wbr>", "")) {
            // 当还不没有存入 undo 栈时，按下 ctrl 后会覆盖 lastText
            return;
        }
        this.wysiwyg.undoStack[0][0].diffs[0][1] = text;
        this.wysiwyg.lastText = text;
    ...
    public addToUndoStack(vditor: IVditor) {
        ...
        const diff = this.dmp.diff_main(text, this.wysiwyg.lastText, true);
        const patchList = this.dmp.patch_make(text, this.wysiwyg.lastText, diff);
        if (patchList.length === 0 && this.wysiwyg.undoStack.length > 0) {
            return;
        }
        this.wysiwyg.lastText = text;
        this.wysiwyg.undoStack.push(patchList);
        if (this.wysiwyg.undoStack.length > this.stackSize) {
            this.wysiwyg.undoStack.shift();
        }
        if (this.wysiwyg.hasUndo) {
            this.wysiwyg.redoStack = [];
            this.wysiwyg.hasUndo = false;
            disableToolbar(vditor.toolbar.elements, ["redo"]);
        }
    ...
    private renderDiff(state: DiffMatchPatch.patch_obj[], vditor: IVditor, isRedo: boolean = false) {
        ...
        if (isRedo) {
            ...
            text = this.dmp.patch_apply(redoPatchList, this.wysiwyg.lastText)[0];
        } else {
            text = this.dmp.patch_apply(state, this.wysiwyg.lastText)[0];
        }

        this.wysiwyg.lastText = text;
        vditor.wysiwyg.element.innerHTML = text;
        vditor.wysiwyg.element.querySelectorAll(`.vditor-wysiwyg__preview`)
        ...
        if (!vditor.wysiwyg.element.querySelector("wbr")) {
            ...
            range.setEndBefore(vditor.wysiwyg.element);
            range.collapse(false);
        } else {
            setRangeByWbr(
                vditor.wysiwyg.element, vditor.wysiwyg.element.ownerDocument.createRange());
            scrollCenter(vditor);
        }
    ...
    private addCaret(vditor: IVditor, setFocus = false) {
        ...
        const cloneElement = vditor.wysiwyg.element.cloneNode(true) as HTMLElement;
        cloneElement.querySelectorAll(`.vditor-wysiwyg__preview[data-render='1']`)
            .forEach((item: HTMLElement) => {
        ...
        vditor.wysiwyg.element.querySelectorAll(".vditor-wbr").forEach((item) => {
            item.remove();
