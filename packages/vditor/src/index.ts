...
import {zhCN} from "./ts/i18n/zh_CN";
import {enUS} from "./ts/i18n/en_US";
...
class Vditor extends VditorMethod {
    ...
    constructor(id: string | HTMLElement, options?: IOptions) {
        ...
        if (!mergedOptions.i18n) {
            if (!["de_DE", "en_US", "es_ES", "fr_FR", "ja_JP", "ko_KR", "pt_BR", "ru_RU", "sv_SE", "vi_VN", "zh_CN", "zh_TW"].includes(mergedOptions.lang)) {
                ...
                // zh_CN / en_US 使用打包内联语言包：跳过 CDN 脚本加载，init 同步执行。
                // 否则 init 被 addScript 推迟到网络完成后，消费方在构造后立即调用
                // getValue/enable 等方法会命中未初始化的 vditor[mode]（reading 'element' 崩溃）。
                const bundledI18n = mergedOptions.lang === "zh_CN" ? zhCN : mergedOptions.lang === "en_US" ? enUS : undefined;
                if (bundledI18n) {
                    window.VditorI18n = bundledI18n;
    ...
    public disabled() {
        ...
        disableToolbar(
            ...
            Constants.EDIT_TOOLBARS.concat(["undo", "redo", "fullscreen"]),
        );
