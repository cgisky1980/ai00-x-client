...
export const Select: React.FC<SelectProps> = ({
  ...
  const resolvedPlaceholder = placeholder ?? tLabel('components.select.placeholder', '请选择');
  const resolvedSearchPlaceholder = searchPlaceholder ?? tLabel('components.select.search', '搜索选项');
  const resolvedEmptyText = emptyText ?? tLabel('components.select.emptyText', '无匹配选项');
  const resolvedCustomValueHint = customValueHint ?? tLabel('components.select.customValueHint', '使用该值');
  ...
  // E3a a11y：listbox id 供触发器 aria-controls / aria-activedescendant 关联
  ...
  useEffect(() => {
    if (highlightedIndex >= 0 && dropdownRef.current && isKeyboardNavigation.current) {
      // E3a：按 id 查找（分组模式下 option 嵌套于 group 容器，nth-child 会错位）
      const highlightedElement = document.getElementById(optionId(highlightedIndex));
        ...
        >
          ...
          {multiple && showSelectAll && filteredOptions.length > 0 && (
            <div className="select__select-all" onClick={handleSelectAll}>
              ...
              <span>{tLabel('components.select.selectAll', '全选')}</span>
            </div>
          )}
          ...
          <div className="select__options">
            {filteredOptions.length === 0 ? (
              ...
              // E3a 修复：分组渲染此前用渲染序号 globalIndex 递增，与键盘导航所用的
              // filteredOptions 索引错位（高亮错行/scrollIntoView 错标）；改为真实索引
              <>
                ...
                {allowCustomValue && !multiple && searchQuery.trim() && 
                 !filteredOptions.some(opt => opt.label.toLowerCase() === searchQuery.trim().toLowerCase()) && (
                  <div 
                    ...
                    onClick={() => handleCustomValueSubmit()}
                  >
                    <span className="select__custom-value-text">{tLabel('components.select.useCustomValue', '使用 "{value}"').replace('{value}', searchQuery.trim())}</span>
                  </div>
                )}
              </>
            )}
