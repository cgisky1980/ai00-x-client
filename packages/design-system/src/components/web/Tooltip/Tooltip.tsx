...
export const Tooltip: React.FC<TooltipProps> = ({
  ...
  // E3a a11y：tooltip 节点 id，供触发器 aria-describedby 关联（读屏播报）
  ...
  const handleFocus = (e: React.FocusEvent) => {
    // E3a a11y：键盘焦点（:focus-visible）也展示 tooltip——与 Radix 默认行为对齐；
    // 鼠标点击获得的焦点不匹配 :focus-visible，不触发（保持原有点击隐藏 UX）
    if (trigger === 'focus' || (e.target instanceof HTMLElement && e.target.matches(':focus-visible'))) {
      showTooltip();
    }
  ...
  // E3a a11y：Escape 关闭（保留子元素自身 onKeyDown）
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape' && visible) {
      hideTooltip();
    }
    if (typeof childProps.onKeyDown === 'function') {
      (childProps.onKeyDown as (e: React.KeyboardEvent) => void)(e);
    }
  };
  ...
  const triggerElement = React.cloneElement(children as React.ReactElement<any>, {
    ...
    // E3a a11y：可见时关联 tooltip，读屏聚焦触发器即播报内容
    'aria-describedby': visible ? tooltipId : undefined,
  });

  const tooltipClass = [
    'ai00-x-tooltip',
    `ai00-x-tooltip--${actualPlacement}`,
    visible && positionReady && 'ai00-x-tooltip--visible',
    interactive && 'ai00-x-tooltip--interactive',
    className
  ].filter(Boolean).join(' ');
  ...
  return (
    <>
      ...
      {visible && createPortal(
        ...
        >
          <div className="ai00-x-tooltip__content">{content}</div>
        </div>,
        portalTarget
      )}
    </>
