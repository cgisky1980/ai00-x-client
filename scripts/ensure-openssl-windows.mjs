 ...
 * - Cached under .ai00-x/cache/firedaemon-openssl-<version>/x64 (gitignored).
 * - Skips download if OPENSSL_DIR already points at a valid tree, or cache hit, or AI00_X_SKIP_OPENSSL_BOOTSTRAP=1.
...
export const CACHE_ROOT = join(ROOT, '.ai00-x', 'cache', `firedaemon-openssl-${OPENSSL_VERSION}`);
...
export async function ensureOpenSslWindows() {
  ...
  if (process.env['AI00_X_SKIP_OPENSSL_BOOTSTRAP'] === '1') {
    console.log(`${LOG} AI00_X_SKIP_OPENSSL_BOOTSTRAP=1, skipping bootstrap`);
    return;
  }
