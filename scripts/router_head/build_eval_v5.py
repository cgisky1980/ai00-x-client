"""Build the v5 boundary stress evaluation set.

Same construction as v4 (ambiguous follow-up × tier-pooled eval summaries,
prev_tier = summary's scenario background tier), with:
  - pools from eval_summary_pools_v5.json (v5 redo, regenerated summaries),
  - BOUNDARIES expanded 26 -> 40 (keep all v4 entries + new zh/en edges).

Each sample: {text: "Summary: {s}\\nRequest: {req}", tier, prev_tier}.
"""
...
OUT = DATA / "boundary_eval_v5.jsonl"
...
BOUNDARIES = [
    ...
    # ---- v5 新增 14 条（zh/en 边界扩充）----
    ("继续上面的实现", 3, 2, 2),
    ("continue implementing the rest", 3, 2, 2),
    ("换一个思路", 2, 2, 1),
    ("try another angle", 2, 2, 1),
    ("还是不对", 3, 3, 2),
    ("still wrong, check again", 3, 3, 2),
    ("帮我总结下", 1, 1, 0),
    ("give me a quick recap", 1, 1, 0),
    ("谢谢，没事了", 0, 0, 0),
    ("thanks, that's all", 0, 0, 0),
    ("直接给最终代码", 2, 2, 2),
    ("just give me the final code", 2, 2, 2),
    ("有报错你修复下", 3, 3, 2),
    ("there's an error, fix it", 3, 3, 2),
]
...
def main() -> None:
    ...
    print(f"[eval-v5] wrote {len(samples)} samples -> {OUT}")
    print(f"[eval-v5] tier dist: R0={tiers.count(0)} R1={tiers.count(1)} "
          f"R2={tiers.count(2)} R3={tiers.count(3)}")
    print(f"[eval-v5] pools: { {k: len(v) for k, v in sorted(pools.items())} }")
