...
// web-ui vite.config.ts outputs to <root>/dist/main (outDir: '../../dist/main'),
// so Monaco public assets land under dist/main/monaco-editor/vs.
const DIST_DIR = path.join(ROOT_DIR, 'dist', 'main', 'monaco-editor', 'vs');
...
if (missingFiles.length > 0) {
  ...
  for (const relativePath of missingFiles) {
    console.error(`  - dist/main/monaco-editor/vs/${relativePath.replace(/\\/g, '/')}`);
  }
  console.error(
