...
- **RWKV 本地推理**：优先在本地运行 RWKV 模型完成 LLM 文本推理，**零 API 费用、数据不出本机、隐私安全**。RWKV 能力持续提升，本地模型能做到的事情越来越多。此外本地还提供 Qwen3 ASR / TTS（语音识别与合成）、音乐与音频生成等能力。
...
| 组件 | 模型 | 用途 | 来源 |
| ---- | ---- | ---- | ---- |
| llama.cpp | RWKV（LLM）、Qwen3（ASR / TTS） | 本地推理引擎 | 源码编译（按平台选 CUDA / Vulkan / Metal 后端） |
